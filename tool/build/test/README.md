...
- `pending/compile_ok`: 当前设计期望 `do build` 成功但实现尚未满足 `.expect` WAT pattern 的红灯用例; 默认回归不执行, 仅在 `RUN_PENDING=1` 时复现缺口.
- `lib`: fixture 专用导入根, 由回归脚本通过 `DO_LIB_ROOT=tool/build/test/lib` 使用; 它不是公开标准库入口.
...
- `compile_ok/259_*` 到 `268_*` 与 `compiled_ok/65_*` 到 `73_*` 锁定阶段 I2 Tuple 子集: 位置构造与数字索引 `@get`、struct field、return/param multi-value ABI、嵌套叶子 ABI、标量叶子 `[Tuple<...>]` storage put/get/set/literal pack、尾逗号位置构造。反例: `err/330` 小写 `tuple` 源码误用 (`InvalidTypeRef`)、`compile_err/331` 越界索引 (`InvalidPathIndex`)、`332`/`333` arity 下限 (`InvalidTypeRef`)、`334`/`err/334` 位置构造 arity 不匹配 (`InvalidTypedLiteral`)、`335` 命名字段构造 (`InvalidStructLiteral`)、`336` 非字面量索引 (`InvalidPathIndex`)、`337` 字面量元素类型不匹配 (`InvalidTypedLiteral`)、`338` 嵌套位置构造 arity 不匹配 (`InvalidTypedLiteral`)。静态 `do test` runner 尚不解释 `Tuple`, 正例走 `compile_ok` / `compiled_ok` / `do test --compiled`。后置边界: managed payload / `text` 叶子 storage、`@get(storage, i, j)` path chaining、loop 绑定上的 `@get(v, N)`。
...
- 标准库源码扫描: `run_tests.sh` 会对 `src/*.do` 做 metadata 检查; 其中没有顶层 `test` 声明的库模块返回 `NoTestDecl` 时计为 `[PASS] std src <module> (NoTestDecl metadata only)`, 不计入 skip. 标准库公开行为应通过 `ok`、`compile_ok`、`compiled_ok`、`check` 等 fixture 覆盖, 不把库源码自身当测试入口。
...
./tool/build/test/run_release_smoke.sh
```
...
- `RUN_PENDING=1 SKIP_BUILD=1 ./tool/build/test/run_tests.sh`
