...
当前覆盖 `do build`、`do test`、第一版 `do run` 产品命令和第一版 `do fmt` 产品命令。
...
- `run_tests.sh`: 编译 `tool` 下的编译器, 然后执行 `do test`、编译模式和 `do run` 黑盒用例.
...
`do run` 用例还需要可用的 `wasm-tools` 和 `node`; 脚本会额外覆盖二者缺失时的 `error[MissingExternalTool]` 诊断。
