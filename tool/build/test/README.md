...
- `compile_ok/242_*` 到 `258_*` 锁定阶段 I 当前递归 / self-tail TCO build 子集: 普通递归、factorial、`if/else` 递归、imported recursion、scalar / `if/else` / guard / generic / imported self-tail loop lowering, 以及 `defer`、storage local、managed struct、多返回、guard+defer、`if/else+defer` 的“不优化”边界。
- `compile_ok/259_*` 到 `262_*` 与 `compiled_ok/65_*` 到 `68_*` 锁定阶段 I2 当前 Tuple 子集: 位置构造与数字索引 `@get`、struct field 嵌套、单返回 multi-value ABI、参数 flatten, 以及 arity / 越界索引反例 (`compile_err/331` 到 `333`, `err/330`)。
