...
- `compiled_ok`: 期望 `do test <input.do> --compiled -o out.wat` 成功生成 compiled test WAT 的用例; 当前覆盖普通 block 函数调用、单表达式箭头函数调用、私有函数声明调用、单返回调用结果的标量推断绑定、导入函数调用和 guard return. `RUN_WASM=1` 时会额外 parse, 并通过 `run_compiled_test_case.mjs` 逐个执行 `__do_test_N` export, 输出 `test "name" ... ok` 与汇总.
