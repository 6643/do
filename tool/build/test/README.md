...
- `compile_ok/259_*` 到 `267_*` 与 `compiled_ok/65_*` 到 `73_*` 锁定阶段 I2 Tuple 子集: 位置构造与数字索引 `@get`、struct field、return/param multi-value ABI、嵌套叶子 ABI、标量叶子 `[Tuple<...>]` storage put/get/set/literal pack。反例: `err/330` 小写 `tuple` 源码误用 (`InvalidTypeRef`)、`compile_err/331` 越界索引 (`InvalidPathIndex`)、`332`/`333` arity 下限 (`InvalidTypeRef`)、`334`/`err/334` 位置构造 arity 不匹配 (`InvalidTypedLiteral`)、`335` 命名字段构造 (`InvalidStructLiteral`)、`336` 非字面量索引 (`InvalidPathIndex`)、`337` 元素类型不匹配 (`NoMatchingCall`)。静态 `do test` runner 尚不解释 `Tuple`, 正例走 `compile_ok` / `compiled_ok` / `do test --compiled`。后置边界: managed payload / `text` 叶子 storage、`@get(storage, i, j)` path chaining、loop 绑定上的 `@get(v, N)`。
