...
- `src/build/`: Zig compiler and `do build` source, kept as flat single-purpose modules.
  ...
  - Gen domain (flat modules):
    - `gen.zig` — public entry (`emitWat` / `emitTestWat`) + unit tests
    - `gen_impl.zig` — expression/call/WASI/union emit and collect helpers
    - `gen_types.zig` — LocalSet, CodegenContext, decl types
    - `gen_util.zig` / `gen_wasi.zig` / `gen_union.zig` — token helpers, WASI tables, union layout
    - `gen_payload_wat.zig` / `gen_storage_wat.zig` — pure WAT fragments
    - plus `function_body_wat.zig` / `runtime_prelude_wat.zig` / `component_metadata_wat.zig` / `backend_ir.zig`
