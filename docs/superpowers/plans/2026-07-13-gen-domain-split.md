...
| `zig test src/build/gen.zig` | 69 passed |
| `./src/build/test/run_tests.sh` | pass=933 fail=0 skip=3 |
| Leaf `@import("gen_lower")` | none |
| `gen_lower` size | ~3029 lines (was ~19.3k) |
| Cycle break | `gen_hooks` late-bound emit/collect |

Domain files: `gen_collect`, `gen_wasi_emit`, `gen_ownership`, `gen_storage`, `gen_struct`, `gen_union_emit`, `gen_expr`, `gen_ctrl`, `gen_hooks`.


> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 把 `src/build/gen_lower.zig`（~19k 行）按功能拆成高内聚、单向依赖的 `gen_*` 模块，使 `gen_lower` 收敛为编排核；不引入 `build/gen/` 子目录，不大面积抽象 emit 虚表。
...
**Tech Stack:** Zig 0.16 编译器源码；回归 `./src/build/test/run_tests.sh`；单测 `zig test src/build/gen.zig`；对外入口保持 `@import("gen.zig")`。

## Global Constraints

- 路径根：仓库 `/home/_/._/do`；编译器源在 `src/build/`（**不是** `tool/`）。
- 扁平文件：新建一律 `src/build/gen_<domain>.zig`；**禁止**新建 `src/build/gen/` 子目录（本阶段）。
- 对外契约：调用方继续 `@import("gen.zig")` 的 `emitWat` / `emitTestWat` / `emitWatWithOptions`；行为与诊断字符串零漂移。
...
- 每刀验收：`cd src && zig build -Doptimize=Debug`；`zig test src/build/gen.zig`（69 tests）；`./src/build/test/run_tests.sh` → `pass=933 fail=0 skip=3`（skip 数若基线变化以当时 summary 为准，**fail 必须为 0**）。
- 文档同刀：`AGENTS.md` Gen 列表、`doc/start_here.md` Gen 表、`CHANGELOG.md` 首条；禁止只改代码不同步索引。
- 单刀目标：一次 PR/提交只完成 **一个域**；`gen_lower` 行数应单调不增（允许 re-export 别名行）。
...
|------|------|------|
| `gen.zig` | 668 | 公开入口 + 单测 |
| `gen_lower.zig` | **19326** | 主 lowering（待削） |
| `gen_types.zig` | 1118 | LocalSet / CodegenContext / free* / ExprCallHead |
| `gen_import.zig` | 802 | 模块解析 / reach / string-data |
...
| expr/ctrl/编排 | 其余 | `emitExpr*`, `emitBody`, `emitIf*`, `emitLoop*`, `emitWat*` |
...
```text
     ...
     gen_util   gen_union   gen_payload_wat   gen_storage_wat   gen_wasi(表)
         ...
         gen_collect ──────────────────────────────────┐
                                      ↑                 │
    gen_ownership   gen_struct   gen_storage   gen_union_emit   gen_wasi_emit
                                      ↑                 │
...
**不允许：** emit 域互相 `import` 对方的私有 helper；`gen_wasi_emit` import 整个 `gen_lower`。

---

## 成功度量

| 里程碑 | `gen_lower` 行数（目标） | 新增/加厚模块 | 验收 |
|--------|--------------------------|---------------|------|
...
| M4 ownership + union emit | **≤ 7k** | `gen_ownership`, `gen_union_emit` | 同上 |
| M5 expr/ctrl 收口 | **编排核 ≤ 3k**（理想）或过渡 **≤ 5k** | `gen_expr`, `gen_ctrl` | 同上 |
| 终态 | `gen_lower` 仅 `emitWat*` / 组装 `CodegenContext` / 调域入口 | 各域 1–3k | 全绿 |
...
| `src/build/gen_ownership.zig` | **Create** | release plan emit；last-use 能搬则搬，不能则留 lower 并写明 |
...
| `src/build/gen_lower.zig` | Modify | 删除已迁出符号；`const x = gen_*.x` 或 `pub const` re-export |
| `src/build/gen.zig` | Modify | 仅当单测需要的符号改为 `pub` re-export 路径变化时 |
...
1. **列符号清单**：从 `gen_lower` 用 `rg -n '^pub fn <Name>'` 列出本域 `pub fn` / 仅本域用的 `fn`。
2. **算闭包**：对本域种子函数求 gen_lower 内调用闭包；**只搬闭包内且不反向依赖 emitExpr 全家桶的符号**。闭包爆炸 → 缩小种子或先上移纯 helper 到 `gen_util`/`gen_types`。
3. **定 import 边**：新文件顶部只 `@import` 允许的下层；若需要 `emitExpr`，在新文件定义：
   ...
   ```
   由 `gen_lower` 传入实际 `emitExpr`（签名以当时源码为准，搬迁时对照 `emitExpr` 定义改，禁止臆造参数）。
4. **机械搬家**：保持函数体字节级行为不变；`pub` 性与 `gen.zig` 单测所需 `pub const` re-export 对齐。
5. **接线**：`gen_lower` 删除原体，加 `const foo = gen_xxx.foo`（文件内用）或 `pub const foo = gen_xxx.foo`（`gen.zig`/单测需要）。
6. **编译**：`cd src && zig build -Doptimize=Debug`
7. **单测**：`zig test src/build/gen.zig` → All 69 tests passed
8. **回归**：`./src/build/test/run_tests.sh` → `fail=0`
9. **文档**：更新三处索引 + CHANGELOG 一条
10. **提交**（若用户要求 commit）：主题形如 `refactor(gen): extract gen_wasi_emit from gen_lower`
...
```bash
wc -l src/build/gen_lower.zig src/build/gen*.zig | sort -n
cd src && zig build -Doptimize=Debug
zig test src/build/gen.zig 2>&1 | tail -3
...
- Modify: `src/build/gen_lower.zig` (~19326 → ~16946)
- Modify: `AGENTS.md`, `doc/start_here.md`, `CHANGELOG.md`
- Test: `zig test src/build/gen.zig` 69/69；`run_tests.sh` pass=933 fail=0 skip=3

**Interfaces:**
- Consumes: `gen_types` / `gen_util` / `gen_import` / `gen_union` / `gen_wasi` / `type_name` / `lexer` / `imports` / `test_runner`
- Produces: struct/enum/func/layout collect、parse helpers 闭包、`findStruct*`、pack leaf (`isPackManagedHandleLeaf` / `leafPayloadBytesForPack`) 等；**无** `emit*`；**不** import `gen_lower`
- **本刀未搬**: `collectGenericFuncInstances*`（除被 collectFunc 闭包拖入的部分外的大块 generic instance emit 侧）、body locals collect

- [x] **Step 1: 生成种子与闭包**

```bash
rg -n '^pub fn collect|^pub fn ensureStoragePack|^pub fn ensurePreopen|^pub fn appendStructFields|^pub fn managedLeafFieldName|^pub fn nextStructLayoutTypeId' src/build/gen_lower.zig
```

用脚本对种子求 gen_lower 内调用闭包；打印 `extra` 中含 `emit` 前缀的项。若存在 → 从种子中删除对应 collect 或先把被调用的纯函数上移 `gen_util`/`gen_types`。
...
// 仅添加闭包真正用到的 const 别名；禁止 import gen_lower
```

- [ ] **Step 3: 搬家 + `gen_lower` re-export**
...
```bash
cd src && zig build -Doptimize=Debug
zig test src/build/gen.zig 2>&1 | tail -5
./src/build/test/run_tests.sh 2>&1 | tail -5
wc -l src/build/gen_lower.zig src/build/gen_collect.zig
```

Expected: fail=0；`gen_lower` 行数 **明显下降**（目标合计收集域 ~2k 级迁出）；`gen_collect` 无 `@import("gen_lower.zig")`。
...
```bash
git add src/build/gen_collect.zig src/build/gen_lower.zig src/build/gen.zig AGENTS.md doc/start_here.md CHANGELOG.md
git commit -m "$(cat <<'EOF'
refactor(gen): extract gen_collect from gen_lower
...
- Modify: `src/build/gen_lower.zig`
...
- **不搬:** `collectWasiHostImports`（已在 `gen_wasi`）、reach validate（已在 `gen_import`）

- [ ] **Step 1: 列 WASI emit 符号**

```bash
rg -n '^pub fn emitWasi|^fn emitWasi' src/build/gen_lower.zig
```

- [ ] **Step 2: 检查对 `emitExpr` 的依赖**

```bash
rg -n 'emitExpr' src/build/gen_lower.zig | rg 'emitWasi|Wasi' || true
# 对每个 emitWasi* 函数体是否调用 emitExpr：
rg -n 'emitExpr\(' src/build/gen_lower.zig
```
...
`gen_lower` 调用处传入 `emitExpr`（或与签名匹配的包装）。**禁止** `gen_wasi_emit` import `gen_lower`。
...
zig test src/build/gen.zig 2>&1 | tail -5
./src/build/test/run_tests.sh 2>&1 | tail -5
wc -l src/build/gen_lower.zig src/build/gen_wasi_emit.zig
```

Expected: `gen_wasi_emit` ~1.2–1.8k 行；`gen_lower` ≤ 约 16k→14k 量级；fail=0。
...
```bash
wc -l src/build/gen_lower.zig src/build/gen_struct.zig src/build/gen_storage.zig
# 目标：gen_lower ≤ 10k 量级
```

---

### Task 4: 提取 `gen_ownership.zig` 与 `gen_union_emit.zig`

**Files:**
- Create: `src/build/gen_ownership.zig`
- Create: `src/build/gen_union_emit.zig`
- Modify: `src/build/gen_lower.zig`, docs

**Interfaces:**
- `gen_ownership` Consumes: `ownership`, `ownership_facts`, `gen_types`；**尽量不**依赖 emitExpr  
  ...
  - **可后置:** `directManagedLastUseMoveSource*`（闭包常拖入 findFunc/infer/call 匹配）——若闭包 >~1.5k 外部符号，**本 Task 明确留下 gen_lower**，在文件头注释 `// last-use move stays in gen_lower until gen_expr exists`
...
```bash
wc -l src/build/gen_lower.zig src/build/gen_ownership.zig src/build/gen_union_emit.zig
# 目标：gen_lower ≤ 7k 量级
```
...
- Modify: `src/build/gen_lower.zig`（应只剩 `emitWat*`、上下文组装、域调用）
- Modify: docs；可选 `src/main.zig` 若需 ` _ = @import` 拉单测（通常不需要）

**Interfaces:**
- `gen_expr`：拥有 `emitExpr` / `emitExprWithMoveContext` 及 call 分发；**可以** import `gen_wasi_emit` / `gen_struct` / `gen_storage` / `gen_union_emit` / `gen_ownership` 的 **pub**
- `gen_ctrl`：`emitBody`, `emitIf*`, `emitLoop*`, `emitDefer*`, `emitGuard*`；调用 `gen_expr.emitExpr` 与 ownership pub
- `gen_lower`：`emitWat` / `emitTestWat` 组装 imports/collect/layouts 后调 `emitUserFunc` / start
- **环处理：** 若 `gen_expr` 需要 ctrl 的 helper，把 helper 上移纯函数或保留在 `gen_lower` 编排层，**禁止** expr↔ctrl 双向 import

- [ ] **Step 1: 先画调用边（只读）**

```bash
rg -n 'emitExpr\(|emitBody\(|emitIfBlock\(|emitLoopBlock\(' src/build/gen_lower.zig | head -80
```
...
- [ ] **Step 4: 削 `gen_lower` 到编排核**

目标结构示意：

```zig
// gen_lower.zig — orchestration only
pub fn emitWatWithOptions(...) ![]u8 {
    // collect host/wasi/structs/funcs/layouts via gen_collect / gen_host / gen_wasi / gen_import
...
**Files:**
- Modify: `gen_lower.zig`（删除只转发的无意义 re-export 堆叠——**仅当**无外部/单测引用时）
...
```bash
# 不应出现
rg -n 'gen_lower' src/build/gen_collect.zig src/build/gen_wasi_emit.zig \
  src/build/gen_struct.zig src/build/gen_storage.zig src/build/gen_ownership.zig \
  src/build/gen_union_emit.zig src/build/gen_host.zig src/build/gen_import.zig \
...
| pub 丢失 | `zig test gen.zig` 报 not marked pub | `pub const x = mod.x` | 同上 |
| 静默行为变 | 回归 fail | 禁止“顺手重构”函数体 | 同上 |
| 闭包爆炸 | 一刀搬半个 lower | 停刀，改种子；勿强行 M5 | 不提交半截 |
| 文档漂移 | 新人仍改 gen_lower 找 collect | 每刀更新 AGENTS/start_here | 补文档 commit |
...
Task 0 基线
  ...
  → Task 4 gen_ownership → gen_union_emit
...
3. **Type consistency:** 模块名 `gen_collect` / `gen_wasi_emit` / `gen_struct` / `gen_storage` / `gen_ownership` / `gen_union_emit` / `gen_expr` / `gen_ctrl` 前后一致；WASI **表**在 `gen_wasi`，**emit** 在 `gen_wasi_emit`。
