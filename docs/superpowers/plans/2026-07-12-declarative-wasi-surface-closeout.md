...
| Single-line field clamp | `src/build/codegen.zig` `appendStructFieldsInBraceRange`; `sema.zig` `collectStructInfos` |
...
- Minimal change in `src/build/sema.zig`, `imports.zig`, `codegen.zig`, `parser.zig` as needed.
...
- Modify only if green breaks: `src/build/codegen.zig` `appendStructFieldsInBraceRange`, `src/build/sema.zig` `collectStructFieldInfos`
