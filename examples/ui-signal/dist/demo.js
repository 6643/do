...
function create_runtime_store(document2, doModule) {
  return {
    document: document2,
    doModule,
    nextScopeId: 1,
...
function createRuntime({ document: document2, module = null } = {}) {
  if (!document2 || typeof document2.createElement !== "function") {
    throw new TypeError("ui runtime requires a document adapter");
  }
  return create_runtime_api(create_runtime_store(document2, module));
}
...
function requireNode(selector) {
  ...
  if (!node)
    throw new Error(`missing demo node: ${selector}`);
  return node;
}
function requireRoot(scope) {
  if (!scope.root)
    throw new Error(`component did not create a root: ${scope.name}`);
  return scope.root;
}
...
var app = runtime.createScope("app", "app");
var mountNode = requireNode("#mount");
var status = requireNode("#status");
var removeSecond = requireNode("#remove-second");
var listNode = requireNode("#item-list");
var addItem = requireNode("#add-item");
var reverseItems = requireNode("#reverse-items");
var detailsNode = requireNode("#details");
var toggleDetails = requireNode("#toggle-details");
var deepOrderList = requireNode("#deep-order-list");
var deepAddOrder = requireNode("#deep-add-order");
var deepReverseOrders = requireNode("#deep-reverse-orders");
var first = runtime.mount("counter_render", "first", app);
var second = runtime.mount("counter_render", "second", app);
