...
if grep -Fq '(func (export "[async-lift]forward_stream")' "$core_path" ||
  grep -Fq '(func (export "[async-lift]middle_stream")' "$core_path"; then
  printf 'parameterized two-hop helper must not be exported\n' >&2
  exit 1
fi
...
printf 'parameterized two-hop forwarding helper producer count=0/1/3 value=90 pending/ready/error runtime passed\n'
