...
core_wat="$tmp_dir/response-body-three-read.wat"
...
output=$(cargo run --quiet --manifest-path "$runner_dir/Cargo.toml" \
  --bin do-p3-http-response-consume-body-host-runner -- "$component" 3)
for marker in \
...
printf 'WASI HTTP response consume-body three-read runtime passed\n'
