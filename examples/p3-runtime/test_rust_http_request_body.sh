...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-p3-http-request-body-runtime.XXXXXX")
...
DO_LIB_ROOT="$repo_root/lib" "$repo_root/bin/do" build \
  "$repo_root/examples/p3-runtime/http-request-body.do" \
...
WIT
"$toolchain_bin" embed-component "$wit_dir" "$core_wasm" http-request-body-probe \
...
ready_output=$(cd "$runner_dir" && env "${runner_env[@]}" DO_HTTP_BODY_COMPLETION_READY=1 \
  timeout 60s cargo run --quiet --bin do-p3-http-request-body-host-runner -- "$component")
...
case "$ready_output" in
  ...
  *)
    printf 'missing ready body completion marker\n%s\n' "$ready_output" >&2
    exit 1
    ;;
esac
for marker in 'body-future-drops=2' 'body-stream-drops=2' 'responses=1' 'table-empty=true'; do
  case "$ready_output" in
    *"$marker"*) ;;
    *)
...
printf 'WASI HTTP request body runtime passed\n'
