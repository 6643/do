...
for helper in outer_stream entry_stream forward_stream middle_stream finish_stream; do
  if grep -Fq "(func (export \"[async-lift]$helper\")" "$core_path"; then
    printf 'parameterized four-hop helper must not be exported\n' >&2
    exit 1
  fi
done
...
for mode in pending ready error; do
  cargo run --quiet --manifest-path "$runner_dir/Cargo.toml" \
    ...
    -- "$component_path" "$mode" parameterized-four-hop
done

printf 'parameterized four-hop forwarding helper producer count=0/1/3 value=90 pending/ready/error runtime passed\n'
