...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-p3-http-response-body-runtime.XXXXXX")
...
DO_LIB_ROOT="$repo_root/lib" "$repo_root/bin/do" build \
  "$repo_root/examples/p3-runtime/http-response-consume-body.do" \
...
output=$(cargo run --quiet --manifest-path "$runner_dir/Cargo.toml" \
  --bin do-p3-http-response-consume-body-host-runner -- "$component")
for marker in \
...
printf 'WASI HTTP response consume-body runtime passed\n'
