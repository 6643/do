...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-p3-http-response-body-eof-runtime.XXXXXX")
trap 'rm -rf -- "$tmp_dir"' EXIT

wit_dir="$tmp_dir/wit-package"
core_wat="$tmp_dir/response-body-eof.wat"
core_wasm="$tmp_dir/response-body-eof.wasm"
embedded="$tmp_dir/response-body-eof.embedded.wasm"
component="$tmp_dir/response-body-eof.component.wasm"
...
output=$(cargo run --quiet --manifest-path "$runner_dir/Cargo.toml" \
  --bin do-p3-http-response-consume-body-host-runner -- "$component" 0 0)
for marker in \
...
printf 'WASI HTTP response consume-body EOF runtime passed\n'
