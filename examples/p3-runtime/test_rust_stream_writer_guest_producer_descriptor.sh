...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-stream-writer-producer-descriptor-runtime.XXXXXX")
trap 'rm -rf -- "$tmp_dir"' EXIT

core_path="$tmp_dir/stream-writer-producer.wat"
wit_path="$tmp_dir/stream-writer-producer.wit"
core_wasm="$tmp_dir/stream-writer-producer.wasm"
embedded_path="$tmp_dir/stream-writer-producer.embedded.wasm"
component_path="$tmp_dir/stream-writer-producer.component.wasm"

DO_LIB_ROOT="$repo_root/lib" "$repo_root/bin/do" build --p3-async-component \
  --p3-wit-output "$wit_path" \
  "$repo_root/examples/p3-runtime/stream-probe-guest-producer-component.do" \
  -o "$core_path"
...
printf 'descriptor-owned guest producer pending/ready/error runtime passed\n'
