(module
  ...
  (import "[export]wasi:filesystem/probe@0.3.0-rc-2025-09-16"
    "[task-return]cancel"
    (func $task-return-cancel (type $noargs)))

  (memory $memory 1)
  (global $frame-next (mut i32) (i32.const 1024))
  (global $heap-next (mut i32) (i32.const 65536))
  (global $run-frame (mut i32) (i32.const 0))
  ;; [result-layout] result<unit,error-code> uses tag/payload words. The
  ;; cancellation endpoint is test-only and is not a Do registry surface.
  ...
  ;; [set-size-pending] retain the waitable, descriptor, and size until the callback.
  ...
  ;; Test-only control endpoint: cancel the host set-size subtask, then let the
  ;; run callback perform the ordinary descriptor/future cleanup.
  ;; [subtask-cancel] test-only cancellation endpoint.
  (func $cancel (type $async-cancel)
    (local $handle i32)
    (local $status i32)
  ...
  ;; [set-size-error] completion event=1 stores the unit/error arm before cleanup.
  ...
  (export "[callback][async-lift]wasi:filesystem/probe@0.3.0-rc-2025-09-16#run"
    (func $callback))
  (export "[async-lift]wasi:filesystem/probe@0.3.0-rc-2025-09-16#cancel"
    (func $cancel))
  (export "[callback][async-lift]wasi:filesystem/probe@0.3.0-rc-2025-09-16#cancel"
