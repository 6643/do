...
if grep -Fq '(func (export "[async-lift]forward_stream")' "$core_path"; then
  printf 'parameterized forwarding helper must not be exported\n' >&2
  exit 1
fi
...
printf 'parameterized forwarding helper producer count=0/1/3 value=90 pending/ready/error runtime passed\n'
