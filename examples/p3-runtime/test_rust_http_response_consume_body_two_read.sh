...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-p3-http-response-body-two-read-runtime.XXXXXX")
trap 'rm -rf -- "$tmp_dir"' EXIT

wit_dir="$tmp_dir/wit-package"
core_wat="$tmp_dir/response-body-two-read.wat"
core_wasm="$tmp_dir/response-body-two-read.wasm"
embedded="$tmp_dir/response-body-two-read.embedded.wasm"
component="$tmp_dir/response-body-two-read.component.wasm"

DO_LIB_ROOT="$repo_root/lib" "$repo_root/bin/do" build \
  "$repo_root/examples/p3-runtime/http-response-consume-body-two-read.do" \
...
output=$(cargo run --quiet --manifest-path "$runner_dir/Cargo.toml" \
  --bin do-p3-http-response-consume-body-host-runner -- "$component" 2)
for marker in \
...
printf 'WASI HTTP response consume-body two-read runtime passed\n'
