(module
  ...
  ;; measured from wasm-tools 1.255.0 for descriptor.metadata-hash.
  ...
  ;; result tag, lower u64, upper u64.
  (type $task-return-metadata-hash (func (param i32 i64 i64)))

  ;; [metadata-hash-call] descriptor handle and result-area pointer.
  ...
  ;; [metadata-hash-result-area] result tag is at frame+8 and the aligned
  ;; record payload begins at frame+16. Err uses the first payload slot.
  ...
  ;; [metadata-hash-pending] join the live subtask with the frame waitable.
  ...
  ;; [metadata-hash-ready] read the two u64 words before cleanup.
  ;; [metadata-hash-error] map the canonical error payload into the first
  ;; flat completion word and clear the second word.
