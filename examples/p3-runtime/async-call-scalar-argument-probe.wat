;; ABI probe for one scalar guest argument carried by the root-owned frame.
;; The helper is deliberately not a WIT export and has no task.return endpoint.
(module
  ...
  (import "do:generic-async-call-probe/host@0.1.0" "[async-lower]work"
  ...
  ;; scalar guest argument @12. The frame size is 20 bytes.
  ...
  (func (export "[async-lift]run") (type $async-run) (local $frame i32)
    ...
    i32.const 1
    i32.store
    ;; The guest call is helper(7); store and carry it through the frame.
    local.get $frame
    i32.const 7
    call $helper
  )
