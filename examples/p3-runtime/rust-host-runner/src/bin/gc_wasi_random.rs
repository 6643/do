...
fn main() -> Result<()> {
    ...
    map_wasmtime(random.func_wrap("get-random-bytes", |mut store, (len,): (u64,)| {
        if len != 16 {
            return Err(wasmtime::Error::msg(format!("expected bounded probe length 16, got {len}")));
        }
    ...
    if length != 16 || store.data().lengths.as_slice() != [16] ||
