...
fn run_component(engine: &Engine, component_path: &str) -> Result<(u32, u32, u32)> {
    ...
    let mut api = map_wasmtime(
        linker.instance("demo:marshal-record-mixed-text-byte-list-lift/api@1.0.0"),
    )?;
    ...
    map_wasmtime(api.func_new("read", move |_store, _ty, params, results| {
        if !params.is_empty() || results.len() != 1 {
            return Err(wasmtime::Error::msg(
                "unexpected mixed text/byte-list lift host signature",
            ));
        }
        results[0] = Val::Record(vec![
            ...
            (
                "payload".to_string(),
                Val::List(vec![Val::U8(10), Val::U8(20), Val::U8(5)]),
            ),
        ]);
...
fn main() -> Result<()> {
    ...
    let first_path = args.next().context(
        "usage: do-p3-gc-marshal-record-mixed-text-byte-list-lift <component> [arc-component]",
    )?;
    ...
    if let Some(second_path) = second_path {
        ...
        println!(
            "GC/ARC mixed text/byte-list record lift equivalence passed result=47/47 stats=34/34 read-calls=1/1"
        );
        return Ok(());
    }
    if first != (47, 34, 1) {
    ...
    println!(
        "GC mixed text/byte-list record lift host adapter passed code=7 label=hello payload=[10, 20, 5] result=47 stats=34 read-calls=1 allocations=2 frees=2"
    );
    Ok(())
}
