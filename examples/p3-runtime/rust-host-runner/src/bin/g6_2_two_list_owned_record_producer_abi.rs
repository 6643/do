...
const TYPES_INSTANCE: &str = "do:g6-2-owned-record-two-list-producer/types@0.1.0";
const SOURCE_INSTANCE: &str = "do:g6-2-owned-record-two-list-producer/source@0.1.0";
const SINK_INSTANCE: &str = "do:g6-2-owned-record-two-list-producer/sink@0.1.0";

const RECORD_OFFSET: u32 = 64;
const RECORD_BYTE_SIZE: u32 = 20;
const RECORD_ALIGNMENT: u32 = 4;
const FIRST_POINTER_OFFSET: u32 = 0;
const FIRST_LENGTH_OFFSET: u32 = 4;
const SECOND_POINTER_OFFSET: u32 = 8;
const SECOND_LENGTH_OFFSET: u32 = 12;
const TICKET_OFFSET: u32 = 16;
...
impl Mode {
    ...
    fn expected_received(self) -> Vec<(Vec<u32>, Vec<u32>, u32)> {
        match self {
            Self::Ready => vec![(vec![], vec![10], 111)],
            Self::Pending => vec![(vec![11, 22], vec![31], 222)],
            Self::SinkErrorBefore => vec![],
            Self::SinkErrorAfter => vec![(vec![12], vec![32, 33], 333)],
            Self::CancelBeforeTransfer => vec![],
            Self::CancelAfterTransfer => vec![(vec![16], vec![46], 555)],
            Self::EarlyDropBeforeTransfer => vec![],
            Self::EarlyDropAfterTransfer => vec![(vec![19, 20, 21], vec![79, 80], 777)],
            Self::Repeat => vec![
                (vec![22, 23], vec![82, 83, 84], 888),
                (vec![22, 23], vec![82, 83, 84], 888),
            ],
            Self::Invalid => vec![],
        }
...
struct Stats {
    created: u32,
    resource_drops: u32,
    received: Vec<(Vec<u32>, Vec<u32>, u32)>,
    host_calls: u32,
    pending_polls: u32,
    poll_calls: u32,
    stream_drops: u32,
...
impl StreamConsumer<State> for Sink {
    ...
    fn poll_consume(
        ...
        self.stats
            .lock()
                .expect("two-list owned-record sink stats mutex poisoned")
            .poll_calls += 1;
        if finish {
            self.stats
                .lock()
                .expect("two-list owned-record sink finish stats mutex poisoned")
                .cancel_calls += 1;
        ...
        if self.pending_once {
            ...
            self.stats
                .lock()
                .expect("two-list owned-record pending stats mutex poisoned")
                .pending_polls += 1;
        ...
        source.read(&mut store, &mut items).map_err(|error| {
            wasmtime::Error::msg(format!("two-list owned-record source lift failed: {error:#}"))
        })?;
        for entry in items {
            ...
            let mut stats = self
                .stats
                .lock()
                .expect("two-list owned-record resource stats mutex poisoned");
            stats.resource_drops += 1;
            stats.list_read_releases += 2;
            stats.received.push((entry.first, entry.second, seed));
        }
...
impl Drop for Sink {
    fn drop(&mut self) {
        self.stats
            .lock()
            .expect("two-list owned-record sink drop stats mutex poisoned")
            .stream_drops += 1;
    }
}
    ...
    F: Future<Output = HostFutureOutput> + Send,
{
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut TaskContext<'_>) -> Poll<Self::Output> {
        ...
        let mut stats = self
            .stats
            .lock()
            .expect("two-list owned-record host future stats mutex poisoned");
...
impl<F> Drop for ObservedHostFuture<F> {
    fn drop(&mut self) {
        let mut stats = self
            .stats
            .lock()
            .expect("two-list owned-record host future drop stats mutex poisoned");
...
fn install_types(linker: &mut Linker<State>, stats: Arc<Mutex<Stats>>) -> wasmtime::Result<()> {
    ...
    types.resource(
        ...
        move |mut store, rep| {
            ...
            stats
                .lock()
                .expect("two-list owned-record guest drop stats mutex poisoned")
                .resource_drops += 1;
            Ok(())
        },
    )?;
...
fn install_source(linker: &mut Linker<State>, stats: Arc<Mutex<Stats>>) -> wasmtime::Result<()> {
    ...
    source.func_wrap("make-ticket", move |mut store, (seed,): (u32,)| {
        ...
        stats
            .lock()
            .expect("two-list owned-record source stats mutex poisoned")
            .created += 1;
        Ok((ticket,))
    })?;
    Ok(())
...
fn install_sink(
    ...
    sink.func_wrap_concurrent(
        "consume-via-stream",
        move |accessor, (reader,): (StreamReader<TwoListEntry>,)| {
            host_stats
                .lock()
                .expect("two-list owned-record host callback stats mutex poisoned")
                .host_calls += 1;
...
async fn run(component_path: &Path, mode: Mode) -> Result<()> {
    ...
    let snapshot = stats
        .lock()
        .expect("two-list owned-record final stats mutex poisoned");
...
fn main() -> Result<()> {
    let component_path = std::env::args()
        .nth(1)
        .context("usage: g6_2_two_list_owned_record_producer_abi <component.wasm> <mode>")?;
    let mode = Mode::parse(
        &std::env::args()
            .nth(2)
            .context("missing two-list owned-record producer mode")?,
    )?;
