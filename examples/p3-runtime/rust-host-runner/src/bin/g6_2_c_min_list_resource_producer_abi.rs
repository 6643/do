...
const TYPES_INSTANCE: &str = "do:g6-2-c-min-producer/types@0.1.0";
const SOURCE_INSTANCE: &str = "do:g6-2-c-min-producer/source@0.1.0";
const SINK_INSTANCE: &str = "do:g6-2-c-min-producer/sink@0.1.0";
...
async fn run(component_path: &Path, mode: Mode) -> Result<()> {
    ...
    if !result_matches
        || (mode != Mode::InvalidMode && snapshot.received != mode.expected())
        || (mode == Mode::InvalidMode && snapshot.created != 0)
        || snapshot.host_calls != u32::from(mode != Mode::InvalidMode)
        || snapshot.stream_drops
            != if mode == Mode::CancelBeforeTransfer {
                0
            } else {
                u32::from(mode != Mode::InvalidMode)
            }
