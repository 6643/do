...
const TYPES_INSTANCE: &str = "do:g6-2-owned-record-pair-producer/types@0.1.0";
const SOURCE_INSTANCE: &str = "do:g6-2-owned-record-pair-producer/source@0.1.0";
const SINK_INSTANCE: &str = "do:g6-2-owned-record-pair-producer/sink@0.1.0";
...
const LEFT_TICKET_SEED: u32 = 111;
const RIGHT_TICKET_SEED: u32 = 222;
...
impl Mode {
    ...
    fn expected_received(self) -> &'static [u32] {
        match self {
            ...
            | Self::SinkErrorAfter
            | Self::CancelAfterTransfer
            | Self::EarlyDropAfterTransfer => &[LEFT_TICKET_SEED, RIGHT_TICKET_SEED],
            Self::SinkErrorBefore
            | Self::CancelBeforeTransfer
            | Self::EarlyDropBeforeTransfer
            | Self::Invalid => &[],
...
async fn call_produce(
    ...
    Ok(map_wasmtime(map_wasmtime(
        store
            .run_concurrent(async |accessor| produce.call_concurrent(&accessor, (input,)).await)
            .await,
    )?)?)
}
...
fn main() -> Result<()> {
    let component_path = std::env::args()
        .nth(1)
        .context("usage: g6_2_owned_record_pair_producer_abi <component.wasm> <mode>")?;
    let mode = Mode::parse(
        &std::env::args()
            .nth(2)
