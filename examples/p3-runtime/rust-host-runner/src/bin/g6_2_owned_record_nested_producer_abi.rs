...
const TYPES_INSTANCE: &str = "do:g6-2-owned-record-nested-producer/types@0.1.0";
const SOURCE_INSTANCE: &str = "do:g6-2-owned-record-nested-producer/source@0.1.0";
const SINK_INSTANCE: &str = "do:g6-2-owned-record-nested-producer/sink@0.1.0";

const RECORD_OFFSET: u32 = 64;
const RECORD_BYTE_SIZE: u32 = 4;
const RECORD_NESTED_TICKET_OFFSET: u32 = 0;
...
impl Mode {
    ...
    fn expected_received(self) -> Vec<u32> {
        match self {
            Self::Ready
            | Self::Pending
            | Self::SinkErrorAfter
            | Self::CancelAfterTransfer
            | Self::EarlyDropAfterTransfer => vec![TICKET_SEED],
            Self::SinkErrorBefore
            | Self::CancelBeforeTransfer
            | Self::EarlyDropBeforeTransfer
            | Self::Invalid => Vec::new(),
            Self::Repeat => vec![TICKET_SEED, TICKET_SEED],
        }
    }
    ...
    F: Future<Output = HostFutureOutput> + Send,
{
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut TaskContext<'_>) -> Poll<Self::Output> {
        ...
        let mut stats = self
            .stats
            .lock()
            .expect("owned record nested host future stats mutex poisoned");
...
impl<F> Drop for ObservedHostFuture<F> {
    fn drop(&mut self) {
        let mut stats = self
            .stats
            .lock()
            .expect("owned record nested host future drop stats mutex poisoned");
...
impl StreamConsumer<State> for Sink {
    ...
    fn poll_consume(
        ...
        self.stats
            .lock()
            .expect("owned record nested sink callback stats mutex poisoned")
            .poll_calls += 1;
        if finish {
            self.stats
                .lock()
                .expect("owned record nested sink finish stats mutex poisoned")
                .finish_calls += 1;
        ...
        if self.pending_once {
            ...
            self.stats
                .lock()
                .expect("owned record nested pending stats mutex poisoned")
                .pending_polls += 1;
        ...
        source.read(&mut store, &mut items).map_err(|error| {
            wasmtime::Error::msg(format!("owned record nested source lift failed: {error:#}"))
        })?;
        for entry in items {
            let ticket = Resource::<Ticket>::new_own(entry.inner.ticket.rep());
            let seed = store.data().table.get(&ticket)?.seed;
            store.data_mut().table.delete(ticket)?;
            self.stats
                .lock()
                .expect("owned record nested resource stats mutex poisoned")
                .resource_drops += 1;
            self.stats
                .lock()
                .expect("owned record nested received stats mutex poisoned")
                .received
                .push(seed);
        }
...
impl Drop for Sink {
    fn drop(&mut self) {
        let mut stats = self
            .stats
            .lock()
            .expect("owned record nested sink drop stats mutex poisoned");
...
fn install_types(linker: &mut Linker<State>, stats: Arc<Mutex<Stats>>) -> wasmtime::Result<()> {
    ...
    types.resource(
        ...
        move |mut store, rep| {
            ...
            stats
                .lock()
                .expect("owned record nested guest drop stats mutex poisoned")
                .resource_drops += 1;
            Ok(())
        },
    )?;
...
fn install_source(linker: &mut Linker<State>, stats: Arc<Mutex<Stats>>) -> wasmtime::Result<()> {
    ...
    source.func_wrap("make-ticket", move |mut store, (seed,): (u32,)| {
        ...
        stats
            .lock()
            .expect("owned record nested source stats mutex poisoned")
            .created += 1;
        Ok((ticket,))
    })?;
    Ok(())
...
fn install_sink(
    ...
    sink.func_wrap_concurrent(
        "consume-via-stream",
        move |accessor, (reader,): (StreamReader<Outer>,)| {
            {
                let mut stats = host_stats
                    .lock()
                    .expect("owned record nested host callback stats mutex poisoned");
                stats.host_calls += 1;
...
async fn call_produce(
    ...
    Ok(map_wasmtime(map_wasmtime(
        store
            .run_concurrent(async |accessor| produce.call_concurrent(&accessor, (mode,)).await)
            .await,
    )?)?)
}
...
async fn run(component_path: &Path, mode: Mode) -> Result<()> {
    ...
    let snapshot = stats
        .lock()
        .expect("owned record nested final stats mutex poisoned");
    let expected_result = mode.expected_result();
    let expected_invocations = mode.expected_invocations();
    let expected_created = expected_invocations;
    ...
    {
        bail!(
            "owned record nested ABI mismatch mode={} result={:?} expected={:?} received={:?} expected-received={:?} resource-created={} resource-drops={} host-calls={} callback-calls={} poll-calls={} finish-calls={} stream-drops={} future-drops={} future-polls={} future-completions={} pending-future-drops={} pending-polls={} cancel-calls={} table-empty={}",
            mode.label(),
            result.0,
            expected_result,
            snapshot.received,
...
fn main() -> Result<()> {
    let component_path = std::env::args()
        .nth(1)
        .context("usage: g6_2_owned_record_nested_producer_abi <component.wasm> <mode>")?;
    let mode = Mode::parse(
        &std::env::args()
            .nth(2)
            .context("missing owned record nested producer mode")?,
    )?;
