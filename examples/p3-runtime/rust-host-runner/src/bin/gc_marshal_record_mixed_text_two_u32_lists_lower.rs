...
fn main() -> Result<()> {
    let component_path = std::env::args().nth(1).context(
        "usage: do-p3-gc-marshal-record-mixed-text-two-u32-lists-lower-host-runner <component>",
    )?;
    ...
    map_wasmtime(api.func_new("write", move |_store, _ty, params, results| {
        if params.len() != 1 || !results.is_empty() {
            return Err(wasmtime::Error::msg(format!(
                "unexpected mixed text/two-u32-list lower signature params={params:?} results={results:?}"
            )));
        }
        let fields = match &params[0] {
            Val::Record(fields) => fields,
            other => return Err(wasmtime::Error::msg(format!(
                "mixed text/two-u32-list lower parameter was not a record: {other:?}"
            ))),
        };
        if fields.len() != 4 {
            return Err(wasmtime::Error::msg(format!(
                "expected four mixed text/two-u32-list lower fields, got {fields:?}"
            )));
        }
        let code = match fields.iter().find(|(name, _)| name == "code") {
            Some((_, Val::U32(value))) => *value,
            _ => return Err(wasmtime::Error::msg("mixed text/two-u32-list lower code was not u32")),
        };
    ...
    println!(
        "GC mixed text/two-u32-list record lower host adapter passed code=7 label=hello first=[10, 20, 5] second=[3, 4] write-calls={callback_count} allocations=3 frees=3"
    );
    Ok(())
}
