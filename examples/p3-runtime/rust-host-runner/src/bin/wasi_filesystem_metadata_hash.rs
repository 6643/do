...
struct RunTask {
    run: TypedFunc<(Resource<Descriptor>,), (std::result::Result<MetadataHash, ErrorCode>,)>,
...
impl Future for MetadataHashFuture {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut TaskContext<'_>) -> Poll<Self::Output> {
        {
            let mut stats = self
                .stats
                .lock()
                .expect("metadata-hash stats mutex poisoned");
...
impl Drop for MetadataHashFuture {
    fn drop(&mut self) {
        let mut stats = self
            .stats
            .lock()
...
async fn call_run(
    ...
    run: TypedFunc<(Resource<Descriptor>,), (std::result::Result<MetadataHash, ErrorCode>,)>,
    descriptor: Resource<Descriptor>,
) -> Result<(std::result::Result<MetadataHash, ErrorCode>,)> {
...
async fn run(component_path: &Path, mode: Mode) -> Result<()> {
    ...
    let probe = instance
        ...
        .context("missing metadata-hash probe export")?;
    let run_export = instance
        .get_export_index(&mut store, Some(&probe), "run")
        .context("missing metadata-hash probe.run export")?;
    ...
    let result = match mode {
        Mode::Cancel => {
            let cancel_export = instance
                ...
                .context("missing metadata-hash probe.cancel export")?;
            let cancel =
                ...
                store
                    .run_concurrent(async |_accessor| {
                        futures::future::poll_fn(|cx| {
                            let snapshot =
                                *stats.lock().expect("metadata-hash stats mutex poisoned");
        ...
        Mode::EarlyDrop => {
            ...
            let started = futures::future::poll_fn(|cx| {
                let snapshot = *stats.lock().expect("metadata-hash stats mutex poisoned");
            ...
            if dropped {
                ...
                {
                    bail!(
                        "unexpected metadata-hash early-drop store disposal: host-calls={} completion-polls={} completions={} future-drops={} pending-future-drops={} descriptor-drops={}",
                        snapshot.host_calls,
    ...
    if !store.data().table.is_empty() {
        bail!("unexpected metadata-hash ResourceTable contents at terminal state");
    }

    match mode {
        Mode::Ready | Mode::Pending => {
            let (value,) = result.context("missing metadata-hash result")?;
            if !check_hash(&value) {
                bail!("unexpected metadata-hash result: {value:?}");
            }
            println!(
                "mode={} result=Ok lower={} upper={} host-calls={} completion-polls={} external-wakes={} completions={} future-drops={} pending-future-drops={} descriptor-drops={} table-empty=true",
                mode.label(),
                expected_hash().lower,
                expected_hash().upper,
                snapshot.host_calls,
        ...
        Mode::Error => {
            ...
            if value != Err(ErrorCode::NoEntry) {
                bail!("unexpected metadata-hash error result: {value:?}");
            }
            println!(
                "mode=error result=Err(no-entry) host-calls={} completion-polls={} external-wakes={} completions={} future-drops={} pending-future-drops={} descriptor-drops={} table-empty=true",
                snapshot.host_calls,
                snapshot.completion_polls,
                snapshot.external_wakes,
                snapshot.completions,
        ...
        Mode::Cancel => {
            if snapshot.completions != 0 || snapshot.pending_future_drops != 1 {
                bail!(
                    "unexpected metadata-hash cancellation: completions={} pending-future-drops={}",
                    snapshot.completions,
                    snapshot.pending_future_drops,
                );
            }
        ...
        Mode::Repeat => {
            let (first, second) = repeat_result.context("missing metadata-hash repeat results")?;
            if !check_hash(&first.0) || !check_hash(&second.0) {
                bail!("unexpected metadata-hash repeat results: {first:?}, {second:?}");
            }
            println!(
    ...
    if snapshot.descriptor_drops != expected_drops {
        bail!(
...
fn main() -> Result<()> {
    ...
    let component_path = args
        .next()
        .context("usage: wasi-filesystem-metadata-hash <component.wasm> <mode>")?;
    let mode = args
        .next()
        .context("usage: wasi-filesystem-metadata-hash <component.wasm> <mode>")?;
    if args.next().is_some() {
        bail!("usage: wasi-filesystem-metadata-hash <component.wasm> <mode>");
    }
