...
fn run_component(engine: &Engine, component_path: &str) -> Result<u32> {
    ...
    let mut api = map_wasmtime(linker.instance("demo:marshal-record-nested-lift-deep/api@1.0.0"))?;
    map_wasmtime(api.func_new("read", |_store, _ty, params, results| {
        if !params.is_empty() || results.len() != 1 {
            return Err(wasmtime::Error::msg(
                "unexpected three-level nested record host signature",
            ));
        }
...
fn main() -> Result<()> {
    ...
    let gc_path = args.next().context(
        "usage: do-p3-gc-marshal-record-nested-lift-deep-host-equivalence-runner <gc> <arc>",
    )?;
    ...
    println!(
        "GC/ARC manifest three-level nested scalar record lift equivalence passed sums={gc_sum}/{arc_sum}"
    );
    Ok(())
}
