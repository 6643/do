    ...
    map_wasmtime(api.func_new("write", move |_store, _ty, params, results| {
        if params.len() != 1 || !results.is_empty() {
            return Err(wasmtime::Error::msg(
                "unexpected multi-managed lower signature",
            ));
        }
...
fn main() -> Result<()> {
    ...
    let gc_path = args.next().context(
        "usage: do-p3-gc-marshal-record-managed-lower-multi-equivalence-runner <gc> <arc>",
    )?;
    ...
    println!(
        "GC/ARC multi-managed-field record lower equivalence passed code=7 label=hello note=world allocations=2/2 frees=2/2 write-calls=1/1"
    );
    Ok(())
}
