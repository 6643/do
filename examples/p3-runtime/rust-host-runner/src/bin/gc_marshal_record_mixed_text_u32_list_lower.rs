...
fn main() -> Result<()> {
    let component_path = std::env::args().nth(1).context(
        "usage: do-p3-gc-marshal-record-mixed-text-u32-list-lower-host-runner <component>",
    )?;
    ...
    map_wasmtime(api.func_new("write", move |_store, _ty, params, results| {
        if params.len() != 1 || !results.is_empty() {
            return Err(wasmtime::Error::msg(format!(
                "unexpected mixed text/u32-list lower signature params={params:?} results={results:?}"
            )));
        }
        ...
        if fields.len() != 3 {
            return Err(wasmtime::Error::msg(format!(
                "expected three mixed text/u32-list lower fields, got {fields:?}"
            )));
        }
    ...
    println!(
        "GC mixed text/u32-list record lower host adapter passed code=7 label=hello payload=[10, 20, 5] write-calls={callback_count} allocations=2 frees=2"
    );
    Ok(())
}
