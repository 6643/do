...
fn run_component(engine: &Engine, component_path: &str) -> Result<u32> {
    ...
    let mut api = map_wasmtime(linker.instance("demo:marshal-record-host/api@1.0.0"))?;
    map_wasmtime(api.func_new("read", |_store, _ty, params, results| {
        if !params.is_empty() || results.len() != 1 {
            return Err(wasmtime::Error::msg("unexpected record host signature"));
        }
        results[0] = Val::Record(vec![
            ("code".to_string(), Val::U32(20)),
            ("count".to_string(), Val::U32(22)),
        ]);
        Ok(())
    }))?;
...
fn main() -> Result<()> {
    ...
    let gc_path = args
        .next()
        .context("usage: do-p3-gc-marshal-record-host-equivalence-runner <gc> <arc>")?;
    let arc_path = args
        .next()
        .context("usage: do-p3-gc-marshal-record-host-equivalence-runner <gc> <arc>")?;
    ...
    if gc_sum != 42 || arc_sum != 42 {
        bail!("expected GC/ARC sums 42/42, got {gc_sum}/{arc_sum}");
    }
    println!("GC/ARC manifest scalar record lift equivalence passed sums={gc_sum}/{arc_sum}");
    Ok(())
}
