    ...
    map_wasmtime(api.func_new("write", move |_store, _ty, params, results| {
        if params.len() != 1 || !results.is_empty() {
            return Err(wasmtime::Error::msg(
                "unexpected scalar record lower signature",
            ));
        }
...
fn main() -> Result<()> {
    ...
    let gc_path = args
        .next()
        .context("usage: do-p3-gc-marshal-record-lower-equivalence-runner <gc> <arc>")?;
    let arc_path = args
        .next()
        .context("usage: do-p3-gc-marshal-record-lower-equivalence-runner <gc> <arc>")?;
    ...
    println!(
        "GC/flat scalar record lower equivalence passed results={gc_result}/{arc_result} write-calls={gc_calls}/{arc_calls}"
    );
    Ok(())
}
