...
fn run_component(engine: &Engine, path: &str) -> Result<Outcome> {
    ...
    map_wasmtime(api.func_new("write", move |_store, _ty, params, results| {
        if params.len() != 1 || !results.is_empty() {
            return Err(wasmtime::Error::msg("unexpected mixed scalar-list lower signature"));
        }
        ...
        if fields.len() != 3 {
            return Err(wasmtime::Error::msg("expected three mixed scalar-list lower fields"));
        }
...
fn main() -> Result<()> {
    ...
    let gc_path = args.next().context(
        "usage: do-p3-gc-marshal-record-mixed-scalar-list-lower-equivalence-runner <gc> <arc>",
    )?;
    ...
    println!(
        "GC/ARC mixed scalar-list record lower equivalence passed code=7 label=hello payload=[10, 20, 5] allocations=2/2 frees=2/2 write-calls=1/1"
    );
    Ok(())
}
