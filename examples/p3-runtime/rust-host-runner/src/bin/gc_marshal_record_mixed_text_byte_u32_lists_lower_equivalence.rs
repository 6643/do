    ...
    map_wasmtime(api.func_new("write", move |_store, _ty, params, results| {
        if params.len() != 1 || !results.is_empty() {
            return Err(wasmtime::Error::msg(
                "unexpected mixed text/byte-u32-list lower signature",
            ));
        }
        let fields = match &params[0] {
            Val::Record(fields) => fields,
            _ => return Err(wasmtime::Error::msg(
                "mixed text/byte-u32-list lower parameter was not a record",
            )),
        };
        if fields.len() != 4 {
            return Err(wasmtime::Error::msg(
                "expected four mixed text/byte-u32-list lower fields",
            ));
        }
...
fn main() -> Result<()> {
    ...
    let gc_path = args.next().context(
        "usage: do-p3-gc-marshal-record-mixed-text-byte-u32-lists-lower-equivalence-runner <gc> <arc>",
    )?;
    ...
    println!(
        "GC/ARC mixed text/byte-u32-list record lower equivalence passed code=7 label=hello bytes=[10, 20, 5] values=[3, 4] allocations=3/3 frees=3/3 write-calls=1/1"
    );
    Ok(())
}
