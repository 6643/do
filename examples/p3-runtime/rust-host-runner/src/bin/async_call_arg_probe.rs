...
const HOST_INSTANCE: &str = "do:async-call-arg-probe/host@0.1.0";
...
struct Stats {
    ...
    last_argument: AtomicU32,
    argument_mismatches: AtomicU32,
    polls: AtomicU32,
    ...
    fn spawn(self: &Arc<Self>, stats: Arc<Stats>) -> JoinHandle<()> {
        ...
        thread::spawn(move || {
            let mut state = signal
                .state
                .lock()
                .expect("async argument wake mutex poisoned");
            while !state.armed && !state.cancelled {
                state = signal
                    .changed
                    .wait(state)
                    .expect("async argument wake condvar poisoned");
            }
            if state.cancelled {
                return;
            }
...
impl ControlledWork {
    ...
    fn pending(stats: Arc<Stats>, wake: bool) -> Self {
        ...
        let waker_thread = wake.then(|| signal.spawn(Arc::clone(&stats)));
        Self {
            stats,
            signal: Some(signal),
            waker_thread,
...
impl Drop for ControlledWork {
    fn drop(&mut self) {
        ...
        if let Some(thread) = self.waker_thread.take() {
            thread
                .join()
                .expect("async argument external waker should terminate");
        }
    }
}
...
async fn run(component_path: &Path, mode: Mode) -> Result<()> {
    ...
    map_wasmtime(
        host.func_wrap_concurrent("work", move |_accessor, (value,): (u32,)| {
            host_stats.host_calls.fetch_add(1, Ordering::SeqCst);
            host_stats.last_argument.store(value, Ordering::SeqCst);
            if value != 7 {
                host_stats
                    .argument_mismatches
                    .fetch_add(1, Ordering::SeqCst);
    ...
    let argument = stats.last_argument.load(Ordering::SeqCst);
    let mismatches = stats.argument_mismatches.load(Ordering::SeqCst);
    ...
    if calls != 1 || argument != 7 || mismatches != 0 || future_drops != 1 || !table_empty {
        bail!(
            "argument probe observations invalid: calls={calls} argument={argument} mismatches={mismatches} polls={polls} external-wakes={external_wakes} completions={completions} future-drops={future_drops} pending-drops={pending_drops} guest-completed={guest_completed} table-empty={table_empty}"
        );
    }
    match mode {
        Mode::Ready => {
            if polls != 1 || external_wakes != 0 || completions != 1 || pending_drops != 0 {
                bail!(
                    "ready observations invalid: calls={calls} argument={argument} polls={polls} external-wakes={external_wakes} completions={completions} future-drops={future_drops} pending-drops={pending_drops} guest-completed={guest_completed} table-empty={table_empty}"
                );
            }
        }
        Mode::Pending => {
            if polls < 2 || external_wakes != 1 || completions != 1 || pending_drops != 0 {
                bail!(
                    "pending observations invalid: calls={calls} argument={argument} polls={polls} external-wakes={external_wakes} completions={completions} future-drops={future_drops} pending-drops={pending_drops} guest-completed={guest_completed} table-empty={table_empty}"
                );
            }
        }
        Mode::Cancel => {
            if polls < 1 || external_wakes != 0 || completions != 0 || pending_drops != 1 {
                bail!(
                    "cancel observations invalid: calls={calls} argument={argument} polls={polls} external-wakes={external_wakes} completions={completions} future-drops={future_drops} pending-drops={pending_drops} guest-completed={guest_completed} table-empty={table_empty}"
                );
            }
        }
    }
    ...
    println!(
        "mode={} calls={} argument={} polls={} external-wakes={} completions={} future-drops={} pending-drops={} guest-completed={} table-empty={}",
        match mode {
            Mode::Ready => "ready",
...
fn main() -> Result<()> {
    ...
    let component_path = args.next().context(
        "usage: do-p3-async-call-arg-probe-host-runner <component.wasm> <ready|pending|cancel>",
    )?;
