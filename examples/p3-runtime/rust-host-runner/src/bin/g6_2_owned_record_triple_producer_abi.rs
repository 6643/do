...
const TYPES_INSTANCE: &str = "do:g6-2-owned-record-triple-producer/types@0.1.0";
const SOURCE_INSTANCE: &str = "do:g6-2-owned-record-triple-producer/source@0.1.0";
const SINK_INSTANCE: &str = "do:g6-2-owned-record-triple-producer/sink@0.1.0";

const RECORD_OFFSET: u32 = 64;
const RECORD_BYTE_SIZE: u32 = 12;
const RECORD_LEFT_OFFSET: u32 = 0;
const RECORD_MIDDLE_OFFSET: u32 = 4;
const RECORD_RIGHT_OFFSET: u32 = 8;
const STREAM_CAPACITY: u32 = 1;
const REPEAT_SECOND_LEFT_SEED: u32 = 444;
const REPEAT_SECOND_MIDDLE_SEED: u32 = 555;
const REPEAT_SECOND_RIGHT_SEED: u32 = 666;
...
impl Mode {
    ...
    fn expected_received(self, left_seed: u32, middle_seed: u32, right_seed: u32) -> Vec<u32> {
        match self {
            Self::Ready
            | Self::Pending
            | Self::SinkErrorAfter
            | Self::CancelAfterTransfer
            | Self::EarlyDropAfterTransfer => vec![left_seed, middle_seed, right_seed],
            Self::SinkErrorBefore
            | Self::CancelBeforeTransfer
            | Self::EarlyDropBeforeTransfer
    ...
    F: Future<Output = HostFutureOutput> + Send,
{
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut TaskContext<'_>) -> Poll<Self::Output> {
        ...
        let mut stats = self
            .stats
            .lock()
            .expect("owned record triple host future stats mutex poisoned");
...
impl<F> Drop for ObservedHostFuture<F> {
    fn drop(&mut self) {
        let mut stats = self
            .stats
            .lock()
            .expect("owned record triple host future drop stats mutex poisoned");
...
impl StreamConsumer<State> for Sink {
    ...
    fn poll_consume(
        ...
        self.stats
            .lock()
            .expect("owned record triple sink callback stats mutex poisoned")
            .poll_calls += 1;
        if finish {
            self.stats
                .lock()
                .expect("owned record triple sink finish stats mutex poisoned")
                .finish_calls += 1;
        ...
        if self.pending_once {
            ...
            self.stats
                .lock()
                .expect("owned record triple pending stats mutex poisoned")
                .pending_polls += 1;
        ...
        source.read(&mut store, &mut items).map_err(|error| {
            wasmtime::Error::msg(format!("owned record triple source lift failed: {error:#}"))
        })?;
        for entry in items {
            for ticket in [entry.left, entry.middle, entry.right] {
                let ticket = Resource::<Ticket>::new_own(ticket.rep());
                let seed = store.data().table.get(&ticket)?.seed;
                store.data_mut().table.delete(ticket)?;
                self.stats
                    .lock()
                    .expect("owned record triple resource stats mutex poisoned")
                    .resource_drops += 1;
                self.stats
                    .lock()
                    .expect("owned record triple received stats mutex poisoned")
                    .received
                    .push(seed);
            }
        }
...
impl Drop for Sink {
    fn drop(&mut self) {
        let mut stats = self
            .stats
            .lock()
            .expect("owned record triple sink drop stats mutex poisoned");
...
fn install_types(linker: &mut Linker<State>, stats: Arc<Mutex<Stats>>) -> wasmtime::Result<()> {
    ...
    types.resource(
        ...
        move |mut store, rep| {
            ...
            stats
                .lock()
                .expect("owned record triple guest drop stats mutex poisoned")
                .resource_drops += 1;
            Ok(())
        },
    )?;
...
fn install_source(linker: &mut Linker<State>, stats: Arc<Mutex<Stats>>) -> wasmtime::Result<()> {
    ...
    source.func_wrap("make-ticket", move |mut store, (seed,): (u32,)| {
        ...
        stats
            .lock()
            .expect("owned record triple source stats mutex poisoned")
            .created += 1;
        Ok((ticket,))
    })?;
    Ok(())
...
fn install_sink(
    ...
    sink.func_wrap_concurrent(
        "consume-via-stream",
        move |accessor, (reader,): (StreamReader<ResourceTriple>,)| {
            {
                let mut stats = host_stats
                    .lock()
                    .expect("owned record triple host callback stats mutex poisoned");
                stats.host_calls += 1;
...
async fn call_produce(
    ...
    produce: &wasmtime::component::TypedFunc<
        (u32, u32, u32, u32),
    ...
    Ok(map_wasmtime(map_wasmtime(
        store
            .run_concurrent(async |accessor| {
                produce
                    .call_concurrent(&accessor, (mode, left_seed, middle_seed, right_seed))
                    .await
            })
            .await,
    )?)?)
...
async fn run(
    ...
    let produce = map_wasmtime(
        instance.get_typed_func::<(u32, u32, u32, u32), (std::result::Result<(), ErrorCode>,)>(
            &mut store, "produce",
        ),
    )?;
    ...
    let snapshot = stats
        .lock()
        .expect("owned record triple final stats mutex poisoned");
    let expected_result = mode.expected_result();
    let expected_invocations = mode.expected_invocations();
    let expected_created = expected_invocations * 3;
    let expected_received = mode.expected_received(left_seed, middle_seed, right_seed);
    ...
    {
        bail!(
            "owned record triple ABI mismatch mode={} result={:?} expected={:?} received={:?} expected-received={:?} resource-created={} resource-drops={} host-calls={} callback-calls={} poll-calls={} finish-calls={} stream-drops={} future-drops={} future-polls={} future-completions={} pending-future-drops={} pending-polls={} cancel-calls={} table-empty={}",
            mode.label(),
            result.0,
            expected_result,
            snapshot.received,
...
fn main() -> Result<()> {
    let component_path = std::env::args()
        .nth(1)
        .context(
            "usage: g6_2_owned_record_triple_producer_abi <component.wasm> <mode> <left-seed> <middle-seed> <right-seed>",
        )?;
    let mode = Mode::parse(
        &std::env::args()
            .nth(2)
            .context("missing owned record triple producer mode")?,
    )?;
    ...
    let middle_seed = std::env::args()
        .nth(4)
        .context("missing middle ticket seed")?
        .parse::<u32>()
        .context("middle ticket seed must be u32")?;
