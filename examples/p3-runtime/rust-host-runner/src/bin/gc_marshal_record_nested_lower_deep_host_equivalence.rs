...
fn main() -> Result<()> {
    ...
    let gc_path = args.next().context(
        "usage: do-p3-gc-marshal-record-nested-lower-deep-host-equivalence-runner <gc> <arc>",
    )?;
    ...
    println!(
        "GC/ARC three-level nested scalar record lower equivalence passed results={gc_result}/{arc_result} write-calls={gc_calls}/{arc_calls}"
    );
    Ok(())
}
