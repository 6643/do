...
// Use the pinned package for all resource and error-code bindings. The test
// world adds only the probe export and is selected dynamically below.
...
fn main() -> Result<()> {
    let component_path = std::env::args()
        .nth(1)
        .context("usage: do-p3-http-client-send-host-runner <component.wasm>")?;
...
async fn run(component_path: &Path) -> Result<()> {
    ...
    {
        bail!(
            "unexpected HTTP client stats: consumed={} created={} dropped={} completions={}",
    ...
    println!("Rust P3 HTTP client send adapter passed");
    println!("requests={}", stats.request_consumed);
    println!("responses={}", stats.response_created);
    println!("drops={}", stats.response_dropped);
    Ok(())
}
