...
fn main() -> Result<()> {
    let component_path = std::env::args()
        .nth(1)
        .context("usage: do-p3-gc-marshal-record-mixed-host-runner <component>")?;
    ...
    let mut api = map_wasmtime(linker.instance("demo:marshal-record-mixed-host/api@1.0.0"))?;
    map_wasmtime(api.func_new("read", |_store, _ty, params, results| {
        if !params.is_empty() || results.len() != 1 {
            return Err(wasmtime::Error::msg(
                "unexpected mixed record host signature",
            ));
        }
    ...
    if sum != 37 {
        bail!("expected mixed scalar record sum 37, got {sum}");
    }
    println!("GC mixed scalar record lift host adapter passed sum={sum}");
    Ok(())
}
