...
impl Future for StatFuture {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut TaskContext<'_>) -> Poll<Self::Output> {
        {
            let mut stats = self.stats.lock().expect("stat-at stats mutex poisoned");
        ...
        move |accessor,
            ...
            let descriptor_path = match accessor.with(|mut store| {
                ...
                let mut stats = store
                    ...
                    .stats
                    .lock()
                    .expect("stat-at stats mutex poisoned");
                stats.host_calls += 1;
                stats.path_copies += 1;
...
fn snapshot(stats: &Arc<Mutex<Stats>>) -> Stats {
    stats.lock().expect("stat-at stats mutex poisoned").clone()
}
...
async fn run(component_path: &Path, mode: Mode) -> Result<()> {
    ...
    let probe = instance
        ...
        .context("missing stat-at probe export")?;
    let run_export = instance
        .get_export_index(&mut store, Some(&probe), "run")
        .context("missing stat-at probe.run export")?;
    ...
    let result = match mode {
        Mode::Cancel => {
            let cancel_export = instance
                ...
                .context("missing stat-at probe.cancel export")?;
            let cancel =
        ...
        Mode::EarlyDrop => {
            ...
            if dropped {
                ...
                {
                    bail!(
                        "unexpected stat-at early-drop store disposal: host-calls={} completion-polls={} completions={} future-drops={} pending-future-drops={} descriptor-drops={}",
                        snapshot.host_calls,
                ...
                println!(
                    ...
                    snapshot.host_calls,
                    snapshot.path_copies,
                    snapshot.observed_flags.unwrap_or(0),
                    observed_path(&snapshot),
                    snapshot.completion_polls,
        ...
        Mode::Ready | Mode::Pending | Mode::Error => {
            let descriptor = make_descriptor(&mut store)?;
            Some(call_run(&mut store, run, descriptor, flags, path.clone()).await?)
        }
    };
    ...
    if !table_empty {
        bail!("unexpected stat-at ResourceTable contents at terminal state");
    }
    ...
    match mode {
        Mode::Ready | Mode::Pending => {
            ...
            if !check_stat(&value) {
                bail!("unexpected stat-at result: {value:?}");
            }
            println!(
                "mode={} result=Ok descriptor-type=regular-file link-count=1 size=4096 access=Some(100,1) modification=Some(101,2) change=Some(102,3) host-calls={} path-copies={} observed-flags={} observed-path={} completion-polls={} external-wakes={} completions={} future-drops={} pending-future-drops={} descriptor-drops={} table-empty=true",
                mode.label(),
                snapshot.host_calls,
                snapshot.path_copies,
                snapshot.observed_flags.unwrap_or(0),
        ...
        Mode::Error => {
            ...
            if value != Err(ErrorCode::NoEntry) {
                bail!("unexpected stat-at error result: {value:?}");
            }
            println!(
                "mode=error result=Err(no-entry) options=none host-calls={} path-copies={} observed-flags={} observed-path={} completion-polls={} external-wakes={} completions={} future-drops={} pending-future-drops={} descriptor-drops={} table-empty=true",
                snapshot.host_calls,
                snapshot.path_copies,
                snapshot.observed_flags.unwrap_or(0),
                observed_path(&snapshot),
        ...
        Mode::Cancel => {
            if snapshot.completions != 0 || snapshot.pending_future_drops != 1 {
                bail!(
                    "unexpected stat-at cancellation: completions={} pending-future-drops={}",
                    snapshot.completions,
                    snapshot.pending_future_drops,
                );
            }
        ...
        Mode::Repeat => {
            ...
            if !check_stat(&first.0) || !check_stat(&second.0) {
                bail!("unexpected repeat stat-at results: {first:?}, {second:?}");
            }
            println!(
    ...
    {
        bail!(
            "unexpected stat-at descriptor drop count: {}",
            snapshot.descriptor_drops
        );
    }
    Ok(())
...
fn main() -> Result<()> {
    ...
    let component_path = args
        .next()
        .context("usage: wasi-filesystem-stat-at <component.wasm> <mode>")?;
    let mode = args
        .next()
        .context("usage: wasi-filesystem-stat-at <component.wasm> <mode>")?;
    if args.next().is_some() {
        bail!("usage: wasi-filesystem-stat-at <component.wasm> <mode>");
    }
