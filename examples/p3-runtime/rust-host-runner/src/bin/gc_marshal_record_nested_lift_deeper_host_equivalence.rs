...
fn main() -> Result<()> {
    ...
    let gc_path = args.next().context(
        "usage: do-p3-gc-marshal-record-nested-lift-deeper-host-equivalence-runner <gc> <arc>",
    )?;
    ...
    println!(
        "GC/ARC manifest four-level nested scalar record lift equivalence passed sums={gc_sum}/{arc_sum}"
    );
    Ok(())
}
