...
wasmtime::component::bindgen!({
    inline: r#"
        ...
        world async-resource-cancel-types {
          import http;
        }
    "#,
    world: "async-resource-cancel-types",
...
impl Future for PendingSend {
    ...
    fn poll(self: Pin<&mut Self>, _cx: &mut TaskContext<'_>) -> Poll<Self::Output> {
        self.stats
            .lock()
            .expect("resource cancellation stats mutex poisoned")
            .pending_polls += 1;
        Poll::Pending
    }
}
...
fn main() -> Result<()> {
    let component_path = std::env::args()
        .nth(1)
        .context("usage: do-p3-resource-result-cancel-host-runner <component.wasm>")?;
...
async fn run(component_path: &Path) -> Result<()> {
    ...
    {
        bail!(
            "unexpected resource cancellation stats: request-consumed={} response-created={} response-dropped={} pending-polls={} pending-future-drops={} table-empty={}",
    ...
    println!("Rust P3 async resource cancellation probe passed");
