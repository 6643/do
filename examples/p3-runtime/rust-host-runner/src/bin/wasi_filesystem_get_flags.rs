...
struct RunTask {
    run: TypedFunc<(Resource<Descriptor>,), (std::result::Result<DescriptorFlags, ErrorCode>,)>,
...
impl Mode {
    fn parse(value: &str) -> Result<Self> {
        match value {
            ...
            other => bail!("mode must be ready, pending, error, or cancel (got {other})"),
        }
    }
...
struct GetFlagsFuture {
    stats: Arc<Mutex<Stats>>,
    output: std::result::Result<DescriptorFlags, ErrorCode>,
    pending_once: bool,
    never_ready: bool,
    was_pending: bool,
    completed: bool,
...
impl Future for GetFlagsFuture {
    type Output = wasmtime::Result<(std::result::Result<DescriptorFlags, ErrorCode>,)>;

    fn poll(mut self: Pin<&mut Self>, cx: &mut TaskContext<'_>) -> Poll<Self::Output> {
        self.stats
            .lock()
            .expect("get-flags stats mutex poisoned")
            .completion_polls += 1;
        if self.never_ready {
...
impl Drop for GetFlagsFuture {
    fn drop(&mut self) {
        let mut stats = self.stats.lock().expect("get-flags stats mutex poisoned");
...
fn install_types(
    ...
    types.func_wrap_concurrent(
        ...
        move |accessor, (descriptor,): (Resource<Descriptor>,)| {
            let valid = accessor.with(|mut store| {
                let _ = store.data_mut().table.get(&descriptor)?;
                store
                    .data_mut()
                    .stats
                    .lock()
            ...
            let output = match mode {
                Mode::Error => Err(ErrorCode::NoEntry),
                Mode::Ready | Mode::Pending | Mode::Cancel => {
                    Ok(DescriptorFlags::READ | DescriptorFlags::WRITE)
                }
            };
            Box::pin(GetFlagsFuture {
...
async fn run(component_path: &Path, mode: Mode) -> Result<()> {
    ...
    let run = map_wasmtime(instance.get_typed_func::<
        (Resource<Descriptor>,),
        (std::result::Result<DescriptorFlags, ErrorCode>,),
            ...
            store
                .run_concurrent(async |_accessor| {
                    futures::future::poll_fn(|cx| {
                        let snapshot = *stats.lock().expect("get-flags stats mutex poisoned");
    ...
    if !table_empty || snapshot.host_calls != 1 || snapshot.descriptor_drops != 1 {
        bail!(
            "unexpected get-flags cleanup: host-calls={} descriptor-drops={} table-empty={table_empty}",
            snapshot.host_calls,
    ...
    let (value,) = result.context("missing get-flags result")?;
    let result_label = match value {
        Ok(flags) if flags.contains(DescriptorFlags::READ | DescriptorFlags::WRITE) => {
            "Ok(read|write)"
        }
        Ok(_) => "Ok(other)",
    ...
    let expected = match mode {
        Mode::Ready | Mode::Pending => "Ok(read|write)",
        Mode::Error => "Err(no-entry)",
        Mode::Cancel => unreachable!(),
    };
    if result_label != expected {
        bail!("unexpected get-flags result: expected {expected}, got {result_label}");
    }
    println!(
...
fn main() -> Result<()> {
    ...
    let component_path = args
        .next()
        .context("usage: do-p3-wasi-filesystem-get-flags-host-runner <component.wasm> <mode>")?;
    let mode = args
        .next()
        .context("usage: do-p3-wasi-filesystem-get-flags-host-runner <component.wasm> <mode>")?;
    if args.next().is_some() {
        bail!("usage: do-p3-wasi-filesystem-get-flags-host-runner <component.wasm> <mode>");
    }
