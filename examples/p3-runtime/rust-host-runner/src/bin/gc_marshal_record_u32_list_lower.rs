...
fn payload_values(fields: &[(String, Val)]) -> Result<Vec<u32>> {
    let payload = fields
        ...
        .map(|(_, value)| value)
        .context("u32-list record payload field is missing")?;
    let values = match payload {
        Val::List(values) => values,
        _ => bail!("u32-list record payload was not a list: {payload:?}"),
    };
    values
        .iter()
        .map(|value| match value {
            Val::U32(value) => Ok(*value),
            other => bail!("u32-list payload element was not u32: {other:?}"),
        })
        .collect()
}
...
fn main() -> Result<()> {
    let component_path = std::env::args()
        .nth(1)
        .context("usage: do-p3-gc-marshal-record-u32-list-lower-host-runner <component>")?;
    ...
    map_wasmtime(api.func_new("write", move |_store, _ty, params, results| {
        if params.len() != 1 || !results.is_empty() {
            return Err(wasmtime::Error::msg(format!(
                "unexpected u32-list lower signature params={params:?} results={results:?}"
            )));
        }
        let fields = match &params[0] {
            Val::Record(fields) => fields,
            other => return Err(wasmtime::Error::msg(format!(
                "u32-list lower parameter was not a record: {other:?}"
            ))),
        };
        ...
        let payload = payload_values(fields).map_err(|error| wasmtime::Error::msg(error.to_string()))?;
    ...
    println!(
        "GC u32-list record lower host adapter passed code=7 payload=[10, 20, 30] result={result} write-calls={callback_count} allocations=1 frees=1"
    );
    Ok(())
}
