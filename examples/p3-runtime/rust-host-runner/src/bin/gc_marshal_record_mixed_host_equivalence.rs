...
fn main() -> Result<()> {
    ...
    let gc_path = args
        .next()
        .context("usage: do-p3-gc-marshal-record-mixed-host-equivalence-runner <gc> <arc>")?;
    let arc_path = args
        .next()
        .context("usage: do-p3-gc-marshal-record-mixed-host-equivalence-runner <gc> <arc>")?;
    ...
    println!("GC/ARC manifest mixed scalar record lift equivalence passed sums={gc_sum}/{arc_sum}");
    Ok(())
}
