...
async fn run(component_path: &Path, mode: Mode) -> Result<()> {
    ...
    let result = if mode.cancellation() {
        ...
        match select(call, cancel_gate).await {
            Either::Left((result, _)) => {
                ...
                bail!(
                    "scalar-list promotion cancellation completed before cancellation mode={mode:?} result={result:?}"
                );
            }
    ...
    {
        bail!(
            "scalar-list promotion mismatch mode={} result={result:?} expected={expected_result:?} received={:?} expected-items={:?} host-calls={} pending-polls={} stream-drops={} cancel-calls={} list-releases={} table-empty={}",
            mode.label(),
            snapshot.received,
            mode.expected(),
            snapshot.host_calls,
...
fn main() -> Result<()> {
    let component_path = std::env::args()
        .nth(1)
        .context("usage: g6_2_scalar_list_producer <component.wasm> <mode>")?;
    let mode = Mode::parse(
        &std::env::args()
            .nth(2)
