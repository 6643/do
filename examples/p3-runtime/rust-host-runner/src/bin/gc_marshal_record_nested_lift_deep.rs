...
fn main() -> Result<()> {
    let component_path = std::env::args()
        .nth(1)
        .context("usage: do-p3-gc-marshal-record-nested-lift-deep-host-runner <component>")?;
    ...
    if sum != 42 {
        bail!("expected three-level nested scalar record sum 42, got {sum}");
    }
    println!("GC three-level nested scalar record lift host adapter passed sum={sum}");
    Ok(())
}
