...
fn main() -> Result<()> {
    ...
    let gc_path = args.next().context(
        "usage: do-p3-gc-marshal-record-managed-lift-host-equivalence-runner <gc> <arc>",
    )?;
    ...
    if gc_value != 12 || arc_value != 12 {
        bail!("expected GC/ARC values 12/12, got {gc_value}/{arc_value}");
    }
    println!(
        "GC/ARC manifest managed-field record lift equivalence passed values={gc_value}/{arc_value}"
    );
    Ok(())
}
