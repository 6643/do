...
impl Future for SyncFuture {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut TaskContext<'_>) -> Poll<Self::Output> {
        self.stats
            .lock()
            .expect("sync stats mutex poisoned")
            .completion_polls += 1;
        if self.never_ready {
...
impl Drop for SyncFuture {
    fn drop(&mut self) {
        let mut stats = self.stats.lock().expect("sync stats mutex poisoned");
...
fn sync_file(path: &Path) -> std::result::Result<(), ErrorCode> {
    ...
    file.sync_all().map_err(|_| ErrorCode::Io)
}
...
async fn run(component_path: &Path, mode: Mode) -> Result<()> {
            ...
            store
                .run_concurrent(async |_accessor| {
                    futures::future::poll_fn(|cx| {
                        let snapshot = *stats.lock().expect("sync stats mutex poisoned");
    ...
    {
        bail!(
            "unexpected sync cleanup: host-calls={} future-drops={} descriptor-drops={} table-empty={table_empty}",
            snapshot.host_calls,
    ...
    let (value,) = result.context("missing sync result")?;
    ...
    if result_label != expected {
        bail!("unexpected sync result: expected {expected}, got {result_label}");
    }
    println!(
...
fn main() -> Result<()> {
    ...
    let component_path = args
        .next()
        .context("usage: do-p3-wasi-filesystem-sync-host-runner <component.wasm> <mode>")?;
    let mode = args
        .next()
        .context("usage: do-p3-wasi-filesystem-sync-host-runner <component.wasm> <mode>")?;
    if args.next().is_some() {
        bail!("usage: do-p3-wasi-filesystem-sync-host-runner <component.wasm> <mode>");
    }
