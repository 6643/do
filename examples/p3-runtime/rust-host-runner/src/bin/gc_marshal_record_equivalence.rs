...
fn run_component(engine: &Engine, component_path: &str) -> Result<u32> {
    ...
    let mut api = map_wasmtime(linker.instance("demo:marshal-record-equivalence/api@1.0.0"))?;
...
fn main() -> Result<()> {
    ...
    let gc_path = args
        .next()
        .context("usage: do-p3-gc-marshal-record-equivalence-runner <gc> <arc>")?;
    let arc_path = args
        .next()
        .context("usage: do-p3-gc-marshal-record-equivalence-runner <gc> <arc>")?;
    ...
    println!("GC/ARC scalar record marshal equivalence passed sums={gc_sum}/{arc_sum}");
    Ok(())
}
