...
const HOST_INSTANCE: &str = "do:generic-async-scalar-probe/host@0.1.0";
...
impl Future for ScalarCompletion {
    type Output = wasmtime::Result<u32>;

    fn poll(mut self: Pin<&mut Self>, cx: &mut TaskContext<'_>) -> Poll<Self::Output> {
        if self.completed {
            panic!("scalar completion polled after terminal completion");
        }
        {
            let mut stats = self
                .stats
                .lock()
                .expect("scalar probe stats mutex poisoned");
            stats.polls += 1;
        }
...
async fn run(component_path: &Path, mode: Mode) -> Result<()> {
    ...
    let stats = stats.lock().expect("scalar probe stats mutex poisoned");
    ...
    {
        bail!(
            "unexpected scalar probe stats: host-calls={} polls={} wakes={} completions={} future-drops={} pending-future-drops={} table-empty={}",
            stats.host_calls,
            stats.polls,
            stats.wakes,
            stats.completions,
    ...
    if mode == Mode::Pending && (stats.polls != 3 || stats.wakes != 1) {
        bail!(
            "pending scalar ABI mismatch: polls={} wakes={}",
            stats.polls,
            stats.wakes
        );
    }
    if mode == Mode::Ready && (stats.polls != 2 || stats.wakes != 0) {
        bail!(
            "ready scalar ABI mismatch: polls={} wakes={}",
            stats.polls,
            stats.wakes
        );
    }
...
fn main() -> Result<()> {
    let component_path = std::env::args()
        .nth(1)
        .context("usage: do-p3-generic-async-scalar-probe-host-runner <component.wasm>")?;
