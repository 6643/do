...
fn main() -> Result<()> {
    let component_path = std::env::args()
        .nth(1)
        .context("usage: do-p3-gc-marshal-record-mixed-lower-host-runner <component.wasm>")?;
    ...
    map_wasmtime(api.func_new("write", move |_store, _ty, params, results| {
        if params.len() != 1 || !results.is_empty() {
            return Err(wasmtime::Error::msg(format!(
                "unexpected mixed scalar record signature params={params:?} results={results:?}"
            )));
        }
        ...
        if fields.len() != 3 {
            return Err(wasmtime::Error::msg(format!(
                "unexpected mixed record field count {}",
                fields.len()
            )));
        }
    ...
    println!(
        "GC mixed scalar record lower host adapter passed result={result} write-calls={callback_count}"
    );
    Ok(())
}
