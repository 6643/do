...
fn main() -> Result<()> {
    let component_path = std::env::args()
        .nth(1)
        .context("usage: do-p3-gc-marshal-record-managed-lift-multi-host-runner <component>")?;
    ...
    if value != 17 {
        bail!("expected managed multi-record code plus text lengths 17, got {value}");
    }
    println!("GC managed multi-field record lift host adapter passed value={value}");
    Ok(())
}
