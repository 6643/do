...
extra_stream(writer StreamWriter<u8>, count u64, value u8) -> Result<nil, ProbeError> {
...
outer_stream(writer StreamWriter<u8>, count u64, value u8) -> Result<nil, ProbeError> {
    pending Future<Result<nil, ProbeError>> = @async(extra_stream(writer, count, value))
    return @await(pending)
}
