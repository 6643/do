(module
  ;; The async method and root ABI are measured from wasm-tools 1.255.0 and
  ;; reproduced by the pinned 1.254.0 async assembler.
  ...
  ;; [sync-data-pending] retain the waitable and descriptor until the callback.
  (func $wait-on-sync-data (param $frame i32) (result i32)
    local.get $frame
    i32.const 16
    i32.add
    i32.load
  ...
  ;; [sync-data-ready] status=2 means the Result is already complete. The private
  ;; probe models unit success as tag=0, payload=0.
  (func $start-sync-data (param $frame i32) (result i32)
    (local $code i32)
    local.get $frame
    i32.const 4
    i32.add
  ...
  (func $run (type $async-run)
    ...
    i32.store
    local.get $frame
    call $start-sync-data
  )

  ;; [sync-data-error] completion event=1 with payload=error-code stores the error
  ;; arm; payload=2 is the unit-success marker used by the pinned probe.
