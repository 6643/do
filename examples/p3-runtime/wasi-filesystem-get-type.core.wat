(module
  ;; The type order and import names are measured from the pinned WIT dummy
  ;; generated by wasm-tools 1.255.0 and reproduced by 1.254.0.
  ...
  ;; [get-type-call] descriptor handle=i32, completion context=i32,
  ;; result status=i32.
  ...
  ;; [result-layout] result<descriptor-type,error-code> is a Component
  ;; variant. The Core completion buffer reserves one i32 tag/payload word;
  ;; the async task-return ABI carries the buffer address and status words.
  ;; [get-type-ready] status=0, tag/payload are read from result-buffer.
  ;; [get-type-pending] status=-1, retain waitable and descriptor ownership.
  ;; [get-type-error] status=1, tag selects error-code in result-buffer.
  ;; [get-type-cancel] the test-only control probe cancels the child; the root
  ;; callback releases the waitable, task, and descriptor.
