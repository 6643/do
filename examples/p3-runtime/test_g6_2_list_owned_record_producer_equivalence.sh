...
tmp_dir=$(mktemp -d "$tmp_root/g6-2-list-owned-record-equivalence.XXXXXX")
cleanup() {
  if [[ -d "${tmp_dir:-}" ]]; then
...
canonical_wat="$repo_root/examples/p3-runtime/g6-2-owned-record-list-producer-canonical.wat"
...
if grep -Fq '__arc_' "$generated_wat"; then
  printf 'list-owned-record generated WAT emitted an ARC symbol\n' >&2
  exit 1
fi
...
for mode in ready pending sink-error-before sink-error-after \
  ...
  printf 'PASS %s: canonical and generated list-owned-record lifecycle observations agree\n' "$mode"
done

printf 'canonical/generated list-owned-record producer equivalence passed modes=10 artifacts=byte-identical resources=1/1 drops=1/1 table-empty=true\n'
