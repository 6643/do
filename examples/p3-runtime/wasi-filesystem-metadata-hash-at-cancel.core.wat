(module
  ;; Cancellation variant of the descriptor.metadata-hash-at ABI probe.
  ...
  (func $finish (param $frame i32) (result i32)
    ...
    i32.eqz
    if
      local.get $frame i32.const 32 i32.add i64.load local.set $lower
      local.get $frame i32.const 40 i32.add i64.load local.set $upper
    else
      local.get $frame i32.const 32 i32.add i32.load8_u i64.extend_i32_u local.set $lower
      i64.const 0
      local.set $upper
    end
    local.get $frame i32.const 48 i32.add i32.load local.set $subtask
    local.get $subtask
    i32.eqz
    if
    else
  ...
  (func $start-metadata-hash-at (param $frame i32) (result i32)
    ...
    if (result i32)
      ...
      local.get $frame call $wait-on-metadata-hash-at
    end
  )
  ...
  (func $run (type $async-run)
    ...
    local.get $frame i32.const 48 i32.add i32.const 0 i32.store
    local.get $frame call $start-metadata-hash-at
  )
  ...
  (func $callback (type $async-callback)
    ...
    i32.eq
    if (result i32)
      local.get $frame i32.const 48 i32.add local.get 1 i32.store
      local.get 2
      i32.const 2
      i32.eq
      if (result i32)
