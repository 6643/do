(module
  ;; The async method and root ABI are measured from wasm-tools 1.255.0.
  ...
  ;; [result-layout] result<unit,error-code> is a Component variant. The
  ;; task-return ABI still carries tag and payload words; unit success ignores
  ;; payload, while error uses it as the error-code discriminant.
  ...
  ;; [set-size-pending] retain the waitable and descriptor until the callback.
  ...
  ;; [set-size-error] completion event=1 with payload=error-code stores the
  ;; error arm; payload=2 is the unit-success marker used by the pinned probe.
