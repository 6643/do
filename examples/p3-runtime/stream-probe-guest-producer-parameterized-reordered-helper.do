...
finish_stream(value u8, writer StreamWriter<u8>, count u64) -> Result<nil, ProbeError> {
    remaining u64 = count
    loop {
        if @eq(remaining, 0) {
            break
...
forward_stream(writer StreamWriter<u8>, count u64, value u8) -> Result<nil, ProbeError> {
    pending Future<Result<nil, ProbeError>> = @async(finish_stream(value, writer, count))
