(module
  ;; Cancellation variant of the descriptor.metadata-hash ABI probe.
  ...
  ;; [metadata-hash-task-return-layout]
  ...
  (import "wasi:filesystem/types@0.3.0-rc-2025-09-16"
    ...
    (func $metadata-hash (type $method)))
  ;; [descriptor-drop]
  (import "wasi:filesystem/types@0.3.0-rc-2025-09-16"
  ...
  (import "[export]wasi:filesystem/probe@0.3.0-rc-2025-09-16"
    "[task-return]run" (func $task-return-run (type $task-return-metadata-hash)))
  (import "[export]wasi:filesystem/probe@0.3.0-rc-2025-09-16"
  ...
  (func $wait-on-metadata-hash (param $frame i32) (result i32)
    local.get $frame
    i32.const 32
    i32.add
    i32.load
  ...
  ;; [metadata-hash-error] cancellation/error callbacks carry an error code
  ;; in the first result payload slot and clear the second u64 word.
  ...
  (func $finish (param $frame i32) (result i32)
    ...
    i32.eqz
    if
      local.get $frame i32.const 16 i32.add i64.load local.set $lower
      local.get $frame i32.const 24 i32.add i64.load local.set $upper
    else
      local.get $frame i32.const 16 i32.add i32.load8_u i64.extend_i32_u local.set $lower
      i64.const 0
      local.set $upper
    end
    local.get $frame i32.const 36 i32.add i32.load local.set $subtask
    local.get $subtask
    i32.eqz
    if
    else
  ...
  (func $start-metadata-hash (param $frame i32) (result i32)
    (local $status i32)
    local.get $frame i32.const 4 i32.add i32.load
    local.get $frame i32.const 8 i32.add
    call $metadata-hash
    local.set $status
    local.get $frame i32.const 32 i32.add local.get $status i32.store
    local.get $status
    i32.const 2
    i32.eq
    if (result i32)
      ...
      local.get $frame call $wait-on-metadata-hash
    end
  )
  ...
  (func $run (type $async-run)
    ...
    local.get $frame i32.const 32 i32.add i32.const 0 i32.store
    local.get $frame i32.const 36 i32.add i32.const 0 i32.store
    local.get $frame call $start-metadata-hash
  )

  (func $cancel (type $async-cancel) (result i32)
    (local $handle i32)
    (local $status i32)
    ;; [subtask-cancel] cleanup only; no host rollback is synthesized.
  ...
  (func $callback (type $async-callback)
    ...
    i32.eq
    if (result i32)
      local.get $frame i32.const 36 i32.add local.get 1 i32.store
      local.get 2
      i32.const 2
      i32.eq
      if (result i32)
