...
arc_core_wat=$repo_root/examples/gc-p3-runtime/marshal-record-mixed-text-u32-list-lift-arc.core.wat
runner_manifest=$repo_root/examples/p3-runtime/rust-host-runner/Cargo.toml
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-gc-default-mixed-text-u32-list-lift-equivalence.XXXXXX")
...
if rg -q '__arc_' "$tmp_dir/gc.core.wat"; then
  printf 'default mixed text/u32-list lift equivalence GC route still contains ARC symbols\n' >&2
  exit 1
fi
...
if rg -q '^\s*\(import.*\(ref' "$tmp_dir/gc.core.wat" "$arc_core_wat"; then
  printf 'GC reference crossed canonical mixed text/u32-list lift import\n' >&2
  exit 1
fi
...
output=$(
  ...
  "$cargo_bin" run --quiet --locked --manifest-path "$runner_manifest" \
    --bin do-p3-gc-marshal-record-mixed-text-u32-list-lift-equivalence-runner -- \
    "$tmp_dir/gc.component.wasm" "$tmp_dir/arc.component.wasm"
)
grep -Fq 'GC/ARC mixed text/u32-list record lift equivalence passed result=47/47 stats=34/34 read-calls=1/1' <<<"$output"
printf '%s\n' "$output"
printf 'default mixed text/u32-list lift ARC/GC equivalence passed\n'
