...
arc_core_wat=$repo_root/examples/gc-p3-runtime/marshal-record-u32-list-lift-arc.core.wat
runner_manifest=$repo_root/examples/p3-runtime/rust-host-runner/Cargo.toml
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-gc-marshal-record-u32-list-lift-equivalence.XXXXXX")
...
rg -q 'array.set \$do_u32' "$tmp_dir/gc.core.wat"
...
output=$(
  ...
  "$cargo_bin" run --quiet --locked --manifest-path "$runner_manifest" \
    --bin do-p3-gc-marshal-record-u32-list-lift-equivalence-runner -- \
    "$tmp_dir/gc.component.wasm" "$tmp_dir/arc.component.wasm"
)
grep -Fq 'GC/ARC u32-list record lift equivalence passed result=67/67 stats=17/17 read-calls=1/1' <<<"$output"
printf '%s\n' "$output"
printf 'manifest-backed u32-list record lift ARC/GC equivalence passed\n'
