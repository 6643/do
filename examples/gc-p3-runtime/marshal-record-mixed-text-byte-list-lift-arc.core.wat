;; Linear-memory ARC oracle for the mixed text/byte-list record lift route.
(module
  (type $canonical_lift (func (param i32)))
  (type $cabi_realloc_type (func (param i32 i32 i32 i32) (result i32)))
  (import "demo:marshal-record-mixed-text-byte-list-lift/api@1.0.0" "read" (func $canonical_call (type $canonical_lift)))
  ...
  (func $run (result i32)
    ...
    i32.add
    i32.load
    local.set $payload_length
    local.get $label_length
    i32.const 5
    i32.ne
    if unreachable end
    local.get $payload_length
    i32.const 3
    i32.ne
    if unreachable end
    local.get $code
