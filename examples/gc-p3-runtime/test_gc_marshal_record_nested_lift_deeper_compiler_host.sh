...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-gc-nested-lift-deeper-compiler-host.XXXXXX")
...
rg -q '\(import "demo:marshal-record-nested-lift-deeper/api@1.0.0" "read"' "$tmp_dir/core.wat"
rg -q 'struct.new \$do_leaf' "$tmp_dir/core.wat"
rg -q 'struct.new \$do_header' "$tmp_dir/core.wat"
rg -q 'struct.new \$do_detail' "$tmp_dir/core.wat"
...
printf '%s\n' "$output"
printf 'compiler-wired four-level nested scalar record lift Component host execution passed\n'
