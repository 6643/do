...
wit="$repo_root/examples/gc-p3-runtime/marshal-record-equivalence.wit"
probe="$repo_root/src/gc_marshal_record_probe_main.zig"
arc_core_wat="$repo_root/examples/gc-p3-runtime/marshal-record-equivalence-arc.core.wat"
runner_manifest="$repo_root/examples/p3-runtime/rust-host-runner/Cargo.toml"
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-gc-marshal-record-equivalence.XXXXXX")
...
"$zig_bin" run "$probe" -- "$wit" "$gc_core_wat" --host
...
output=$(
  ...
  "$cargo_bin" run --quiet --locked --manifest-path "$runner_manifest" \
    --bin do-p3-gc-marshal-record-equivalence-runner -- \
    "$tmp_dir/gc.component.wasm" "$tmp_dir/arc.component.wasm"
)
grep -Fq 'GC/ARC scalar record marshal equivalence passed sums=42/42' <<<"$output"
printf '%s\n' "$output"
