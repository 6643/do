...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-gc-default-mixed-text-two-u32-lists-lift-negative.XXXXXX")
...
DO_LIB_ROOT="$repo_root/lib" "$do_bin" build \
  "$repo_root/src/build/test/compile_ok/03_env_host_import_i32.do" -o "$unadmitted_wat"
...
printf 'default mixed text/two-u32-lists lift negative boundary gate passed\n'
