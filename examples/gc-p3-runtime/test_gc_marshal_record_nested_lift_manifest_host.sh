...
wit="$repo_root/examples/gc-p3-runtime/marshal-record-nested-lift-assembly.wit"
probe="$repo_root/src/gc_marshal_record_nested_lift_probe_main.zig"
runner_manifest="$repo_root/examples/p3-runtime/rust-host-runner/Cargo.toml"
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-gc-marshal-record-nested-lift-manifest-host.XXXXXX")
...
if grep -E '^[[:space:]]*\(import .*(\(param|\(result).*\(ref' "$tmp_dir/core.wat"; then
  printf 'GC reference crossed canonical nested lift import\n' >&2
  exit 1
fi
...
record_source_relative='examples/gc-p3-runtime/marshal-record-nested-lift-manifest-source.wit'
...
cp "$repo_root/doc/wit/gc_marshal_record_nested_lift_imports.wit" "$mutated_root/doc/wit/gc_marshal_record_nested_lift_imports.wit"
...
output=$(
  ...
  "$cargo_bin" run --quiet --locked --manifest-path "$runner_manifest" \
    --bin do-p3-gc-marshal-record-nested-host-runner -- "$tmp_dir/component.wasm"
)
grep -Fq 'GC nested scalar record lift host adapter passed sum=37' <<<"$output"
printf '%s\n' "$output"

printf 'manifest-backed nested scalar record lift Component host execution passed\n'
