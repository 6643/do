...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-gc-default-mixed-text-two-u32-lists-lower-host.XXXXXX")
...
if rg -q '__arc_' "$tmp_dir/core.wat"; then
    printf 'mixed text/two-u32-list lower route still contains ARC runtime symbols\n' >&2
    exit 1
fi
rg -q '\(type \$canonical_lower \(func \(param i32 i32 i32 i32 i32 i32 i32\)\)\)' "$tmp_dir/core.wat"
rg -q '\(import "demo:marshal-record-mixed-text-two-u32-lists-lower/api@1.0.0" "write"' "$tmp_dir/core.wat"
if rg -q '^\s*\(import .*\(ref' "$tmp_dir/core.wat"; then
    printf 'GC reference crossed the mixed text/two-u32-list canonical import\n' >&2
    exit 1
fi
test "$(rg -c 'array.get_s \$do_bytes' "$tmp_dir/core.wat")" -eq 1
...
if [[ -z "$call_line" || "$call_line" -ge "${realloc_lines[3]}" ]]; then
    printf 'mixed text/two-u32-list lower canonical call/free order is invalid\n' >&2
    exit 1
fi
...
output=$(
    ...
    "$cargo_bin" run --quiet --locked --manifest-path "$runner_manifest" \
        --bin do-p3-gc-marshal-record-mixed-text-two-u32-lists-lower-host-runner -- \
        "$tmp_dir/component.wasm"
)
grep -Fq 'GC mixed text/two-u32-list record lower host adapter passed code=7 label=hello first=[10, 20, 5] second=[3, 4] write-calls=1 allocations=3 frees=3' <<<"$output"
printf '%s\n' "$output"
printf 'mixed text/two-u32-list lower Component host execution passed\n'
