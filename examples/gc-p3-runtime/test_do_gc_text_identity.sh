...
"$toolchain_bin" compile-core-gc "$wat_path" -o "$tmp_dir/text-identity.compiled"

result=$("$toolchain_bin" invoke-core-gc "$wat_path" --export probe)
if [ "$result" != "27815" ]; then
  printf 'expected GC text identity result 27815, got %s\n' "$result" >&2
  exit 1
fi
printf 'Do GC text identity passed: %s\n' "$result"
