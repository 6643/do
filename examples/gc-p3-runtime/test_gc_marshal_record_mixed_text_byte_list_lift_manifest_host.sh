...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-gc-marshal-record-mixed-text-byte-list-lift-host.XXXXXX")
...
if rg -q '__arc_' "$tmp_dir/core.wat"; then
  printf 'explicit mixed text/byte-list record lift route still contains ARC symbols\n' >&2
  exit 1
fi
if rg -q '^\s*\(import.*\(ref' "$tmp_dir/core.wat"; then
  printf 'GC reference crossed canonical mixed text/byte-list record lift import\n' >&2
  exit 1
fi
test "$(rg -c 'array.new_default \$do_bytes' "$tmp_dir/core.wat")" -eq 2
test "$(rg -c 'array.set \$do_bytes' "$tmp_dir/core.wat")" -ge 2
...
if [ "${call_lines[0]}" -ge "${call_lines[1]}" ] || [ "${call_lines[1]}" -ge "${call_lines[2]}" ]; then
  printf 'mixed text/byte-list record lift call/free order is invalid\n' >&2
  exit 1
fi
...
printf '%s\n' "$output"
printf 'manifest-backed mixed text/byte-list record lift Component host execution passed\n'
