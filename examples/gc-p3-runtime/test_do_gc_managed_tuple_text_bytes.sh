...
"$toolchain_bin" parse-core "$wat_path" -o "$tmp_dir/managed-tuple-text-bytes.wasm"
"$toolchain_bin" compile-core-gc "$wat_path" -o "$tmp_dir/managed-tuple-text-bytes.compiled"

result=$("$toolchain_bin" invoke-core-gc "$wat_path" --export probe)
if [ "$result" != "27815" ]; then
  printf 'expected GC managed tuple persistent update result 27815, got %s\n' "$result" >&2
  exit 1
fi
printf 'Do GC managed tuple persistent update passed: %s\n' "$result"
