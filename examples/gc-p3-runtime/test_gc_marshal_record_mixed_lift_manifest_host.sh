...
wit="$repo_root/examples/gc-p3-runtime/marshal-record-mixed-lift-assembly.wit"
probe="$repo_root/src/gc_marshal_record_mixed_lift_probe_main.zig"
runner_manifest="$repo_root/examples/p3-runtime/rust-host-runner/Cargo.toml"
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-gc-marshal-record-mixed-lift-manifest-host.XXXXXX")
...
if grep -E '^[[:space:]]*\(import .*(\(param|\(result).*\(ref' "$tmp_dir/core.wat"; then
  printf 'GC reference crossed canonical mixed lift import\n' >&2
  exit 1
fi
...
record_source_relative='examples/gc-p3-runtime/marshal-record-mixed-lift-manifest-source.wit'
...
cp "$repo_root/doc/wit/gc_marshal_record_mixed_lift_imports.wit" "$mutated_root/doc/wit/gc_marshal_record_mixed_lift_imports.wit"
...
output=$(
  ...
  "$cargo_bin" run --quiet --locked --manifest-path "$runner_manifest" \
    --bin do-p3-gc-marshal-record-mixed-host-runner -- "$tmp_dir/component.wasm"
)
grep -Fq 'GC mixed scalar record lift host adapter passed sum=37' <<<"$output"
printf '%s\n' "$output"

printf 'manifest-backed mixed scalar record lift Component host execution passed\n'
