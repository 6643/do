...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-gc-marshal-record-mixed-text-byte-list-lift-equivalence.XXXXXX")
...
test "$(rg -c 'array.new_default \$do_bytes' "$tmp_dir/gc.core.wat")" -eq 2
...
printf '%s\n' "$output"
printf 'manifest-backed mixed text/byte-list record lift ARC/GC equivalence passed\n'
