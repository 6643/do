...
probe="$repo_root/src/gc_marshal_record_lift_probe_main.zig"
runner_manifest="$repo_root/examples/p3-runtime/rust-host-runner/Cargo.toml"
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-gc-marshal-record-lift-manifest-host.XXXXXX")
...
if grep -E '^[[:space:]]*\(import .*(\(param|\(result).*\(ref' "$tmp_dir/core.wat"; then
  printf 'GC reference crossed canonical lift import\n' >&2
  exit 1
fi
...
record_source_relative='examples/gc-p3-runtime/marshal-record-lift-manifest-source.wit'
...
cp "$repo_root/doc/wit/gc_marshal_record_lift_imports.wit" "$mutated_root/doc/wit/gc_marshal_record_lift_imports.wit"
...
printf 'manifest-backed scalar record lift Component host execution passed\n'
