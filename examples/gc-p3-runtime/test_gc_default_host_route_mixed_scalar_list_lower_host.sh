...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-gc-default-mixed-scalar-list-lower-host.XXXXXX")
...
if rg -q '__arc_' "$tmp_dir/core.wat"; then
  printf 'default mixed scalar-list lower route still contains ARC runtime symbols\n' >&2
  exit 1
fi
rg -q '\(type \$canonical_lower \(func \(param i32 i32 i32 i32 i32\)\)\)' "$tmp_dir/core.wat"
rg -q '\(import "demo:marshal-record-mixed-scalar-list-lower/api@1.0.0" "write"' "$tmp_dir/core.wat"
if rg -q '^\s*\(import "demo:marshal-record-mixed-scalar-list-lower/api@1.0.0" "write".*\(ref' "$tmp_dir/core.wat"; then
...
test "$(rg -c 'i32.store8' "$tmp_dir/core.wat")" -eq 2
...
if [[ -z "$call_line" || "$call_line" -ge "${realloc_lines[2]}" || "${realloc_lines[2]}" -ge "${realloc_lines[3]}" ]]; then
  printf 'mixed scalar-list lower canonical call/free order is invalid\n' >&2
  exit 1
fi
...
output=$(
  ...
  "$cargo_bin" run --quiet --locked --manifest-path "$runner_manifest" \
    --bin do-p3-gc-marshal-record-mixed-scalar-list-lower-host-runner -- "$tmp_dir/component.wasm"
)
grep -Fq 'GC mixed scalar-list record lower host adapter passed code=7 label=hello payload=[10, 20, 5] write-calls=1 allocations=2 frees=2' <<< "$output"
printf '%s\n' "$output"
printf 'default mixed scalar-list record lower Component host execution passed\n'
