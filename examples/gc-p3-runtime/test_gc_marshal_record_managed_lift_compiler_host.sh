...
input_do=$repo_root/src/build/test/compile_ok/566_gc_wit_managed_record_lift_host_boundary.do
wit=$repo_root/examples/gc-p3-runtime/marshal-record-managed-lift-assembly.wit
runner_manifest=$repo_root/examples/p3-runtime/rust-host-runner/Cargo.toml
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-gc-marshal-record-managed-lift-compiler-host.XXXXXX")
...
rg -q '\(import "demo:marshal-record-managed-lift/api@1.0.0" "read"' "$tmp_dir/core.wat"
rg -q '\(type \$do_record \(struct \(field \$field0 i32\) \(field \$field1 \(ref null \$do_text\)\)\)\)' "$tmp_dir/core.wat"
rg -q 'array.set \$do_bytes' "$tmp_dir/core.wat"
rg -q 'struct.get \$do_text \$length' "$tmp_dir/core.wat"
...
printf '%s\n' "$output"
printf 'compiler-wired managed-field record lift Component host execution passed\n'
