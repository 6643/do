...
input_do=$repo_root/examples/gc-p3-runtime/ordinary-host-record-byte-list-lower-call.do
descriptor=demo:marshal-record-byte-list-lower/api.write@1.0.0/lower
wit=$repo_root/examples/gc-p3-runtime/marshal-record-byte-list-lower-assembly.wit
runner_manifest=$repo_root/examples/p3-runtime/rust-host-runner/Cargo.toml
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-gc-marshal-record-byte-list-lower-host.XXXXXX")
...
rg -q '\(import "demo:marshal-record-byte-list-lower/api@1.0.0" "write"' "$tmp_dir/core.wat"
if rg -q '__arc_' "$tmp_dir/core.wat"; then
  printf 'explicit byte-list record lower route still contains ARC symbols\n' >&2
  exit 1
fi
if rg -q '^\s*\(import.*\(ref' "$tmp_dir/core.wat"; then
  printf 'GC reference crossed canonical byte-list record lower import\n' >&2
  exit 1
fi
rg -q 'array.get_s \$do_bytes' "$tmp_dir/core.wat"
rg -q 'i32.store8' "$tmp_dir/core.wat"
...
if [ -z "$call_line" ] || [ -z "$free_line" ] || [ "$call_line" -ge "$free_line" ]; then
  printf 'byte-list record lower call/free order is invalid\n' >&2
  exit 1
fi
...
output=$(
  ...
  "$cargo_bin" run --quiet --locked --manifest-path "$runner_manifest" \
    --bin do-p3-gc-marshal-record-byte-list-lower-host-runner -- "$tmp_dir/component.wasm"
)
grep -Fq 'GC byte-list record lower host adapter passed code=7 payload=[10, 20, 5] result=42 write-calls=1 allocations=1 frees=1' <<<"$output"
printf '%s\n' "$output"
printf 'manifest-backed byte-list record lower Component host execution passed\n'
