...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-gc-marshal-record-mixed-lower-equivalence.XXXXXX")
...
"$zig_bin" run "$probe" -- "$wit" "$tmp_dir/gc.core.wat" --lower-mixed-host
...
printf 'bounded GC/flat mixed scalar record lower equivalence passed\n'
