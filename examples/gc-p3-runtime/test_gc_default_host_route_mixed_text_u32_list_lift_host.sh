...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-gc-default-mixed-text-u32-list-lift-host.XXXXXX")
...
if rg -q '__arc_' "$tmp_dir/core.wat"; then
  printf 'default mixed text/u32-list lift route still contains ARC runtime symbols\n' >&2
  exit 1
fi
...
if rg -q '^\s*\(import.*\(ref' "$tmp_dir/core.wat"; then
  printf 'GC reference crossed default mixed text/u32-list lift import\n' >&2
  exit 1
fi
...
printf '%s\n' "$output"
printf 'default mixed text/u32-list lift Component host execution passed\n'
