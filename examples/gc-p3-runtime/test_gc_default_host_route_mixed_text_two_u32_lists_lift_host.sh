...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-gc-default-mixed-text-two-u32-lists-lift-host.XXXXXX")
...
if rg -q '__arc_' "$tmp_dir/core.wat"; then
  printf 'default mixed text/two-u32-lists lift route still contains ARC runtime symbols\n' >&2
  exit 1
fi
...
if rg -q '^\s*\(import.*\(ref' "$tmp_dir/core.wat"; then
  printf 'GC reference crossed default mixed text/two-u32-lists lift import\n' >&2
  exit 1
fi
...
test "$(rg -c 'array.set \$do_u32' "$tmp_dir/core.wat")" -eq 2
...
printf '%s\n' "$output"
printf 'default mixed text/two-u32-lists lift Component host execution passed\n'
