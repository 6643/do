...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-gc-marshal-record-mixed-lower-host.XXXXXX")
...
"$zig_bin" run "$probe" -- "$wit" "$tmp_dir/core.wat" --lower-mixed-host
...
if grep -E '^[[:space:]]*\(import .*(\(param|\(result).*\(ref' "$tmp_dir/core.wat"; then
  printf 'GC reference crossed canonical mixed lower import\n' >&2
  exit 1
fi
...
printf 'bounded GC mixed scalar record lower Component host execution passed\n'
