...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-gc-default-mixed-text-two-u32-lists-lower-negative.XXXXXX")
...
for input_do in \
    "$repo_root/src/build/test/compile_err/666_gc_wit_mixed_text_two_u32_lists_lower_async.do" \
    "$repo_root/src/build/test/compile_err/667_gc_wit_mixed_text_two_u32_lists_lower_member.do" \
    "$repo_root/src/build/test/compile_err/668_gc_wit_mixed_text_two_u32_lists_lower_reordered.do" \
    "$repo_root/src/build/test/compile_err/669_gc_wit_mixed_text_two_u32_lists_lower_first_byte_list.do" \
    "$repo_root/src/build/test/compile_err/670_gc_wit_mixed_text_two_u32_lists_lower_second_byte_list.do" \
    "$repo_root/src/build/test/compile_err/671_gc_wit_mixed_text_two_u32_lists_lower_extra_field.do" \
    "$repo_root/src/build/test/compile_err/672_gc_wit_mixed_text_two_u32_lists_lower_record_name.do"; do
    run_rejection "$input_do"
done

(
    cd "$repo_root/src"
    "$zig_bin" test main.zig --test-filter 'mixed text and two u32-list lower declaration rejects locator drift'
)
