...
rg -q '\(import "demo:marshal-record-byte-list-lift/api@1.0.0" "read"' "$tmp_dir/core.wat"
if rg -q '__arc_' "$tmp_dir/core.wat"; then
  printf 'explicit byte-list record lift route still contains ARC symbols\n' >&2
  exit 1
fi
...
if [ -z "$call_line" ] || [ -z "$free_line" ] || [ "$call_line" -ge "$free_line" ]; then
  printf 'byte-list record lift call/free order is invalid\n' >&2
  exit 1
fi
...
output=$( \
  ...
  "$cargo_bin" run --quiet --locked --manifest-path "$runner_manifest" \
    --bin do-p3-gc-marshal-record-byte-list-lift-host-runner -- "$tmp_dir/component.wasm"
)
grep -Fq 'GC byte-list record lift host adapter passed code=7 payload=[10, 20, 30] result=67 stats=17 read-calls=1 allocations=1 frees=1' <<<"$output"
printf '%s\n' "$output"
printf 'manifest-backed byte-list record lift Component host execution passed\n'
