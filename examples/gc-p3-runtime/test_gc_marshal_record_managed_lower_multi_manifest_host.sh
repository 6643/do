...
probe="$repo_root/src/gc_marshal_record_managed_lower_multi_probe_main.zig"
runner_manifest="$repo_root/examples/p3-runtime/rust-host-runner/Cargo.toml"
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-gc-marshal-record-managed-lower-multi-host.XXXXXX")
...
if [ -z "$call_line" ] || [ -z "$free_line" ] || [ "$call_line" -ge "$free_line" ]; then
  printf 'multi-managed-text lower call/free order is invalid\n' >&2
  exit 1
fi
...
record_source_relative='examples/gc-p3-runtime/marshal-record-managed-lower-multi-manifest-source.wit'
...
cp "$repo_root/doc/wit/gc_marshal_record_managed_lower_multi_imports.wit" "$mutated_root/doc/wit/gc_marshal_record_managed_lower_multi_imports.wit"
...
printf 'manifest-backed multi-managed-field record lower Component host execution passed\n'
