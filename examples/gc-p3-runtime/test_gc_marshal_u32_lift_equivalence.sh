...
probe="$repo_root/src/gc_marshal_u32_lift_probe_main.zig"
arc_core_wat="$repo_root/examples/gc-p3-runtime/marshal-u32-lift-host.core.wat"
runner_manifest="$repo_root/examples/p3-runtime/rust-host-runner/Cargo.toml"
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-gc-marshal-u32-lift-equivalence.XXXXXX")
...
if [ ! -f "$wit" ] || [ ! -f "$probe" ] || [ ! -f "$arc_core_wat" ]; then
  printf 'missing u32 list lift equivalence fixture\n' >&2
  exit 1
fi
...
output=$(
  ...
  "$cargo_bin" run --quiet --locked --manifest-path "$runner_manifest" \
    --bin do-p3-gc-marshal-u32-lift-host-runner -- \
    "$tmp_dir/gc.component.wasm" "$tmp_dir/arc.component.wasm"
)
grep -Fq 'GC/ARC list<u32> lift equivalence passed checksums=60/60' <<<"$output"
printf '%s\n' "$output"
