...
wit="$repo_root/examples/gc-p3-runtime/marshal-record-nested-lower-assembly.wit"
probe="$repo_root/src/gc_marshal_record_nested_lower_probe_main.zig"
runner_manifest="$repo_root/examples/p3-runtime/rust-host-runner/Cargo.toml"
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/do-gc-marshal-record-nested-lower-manifest-host.XXXXXX")
...
if rg -q '^\s*\(import .*(\(param|\(result).*\(ref' "$tmp_dir/core.wat"; then
  printf 'GC reference crossed canonical nested lower import\n' >&2
  exit 1
fi
rg -q '\(type \$canonical_lower \(func \(param i32 i64 i64\)\)\)' "$tmp_dir/core.wat"
rg -q 'struct\.get \$do_header \$field0' "$tmp_dir/core.wat"
...
record_source_relative='examples/gc-p3-runtime/marshal-record-nested-lower-manifest-source.wit'
...
cp "$repo_root/doc/wit/gc_marshal_record_nested_lower_imports.wit" "$mutated_root/doc/wit/gc_marshal_record_nested_lower_imports.wit"
...
output=$(
  ...
  "$cargo_bin" run --quiet --locked --manifest-path "$runner_manifest" \
    --bin do-p3-gc-marshal-record-nested-lower-host-runner -- "$tmp_dir/component.wasm"
)
grep -Fq 'GC nested scalar record lower host adapter passed result=42 write-calls=1' <<<"$output"
printf '%s\n' "$output"

printf 'manifest-backed nested scalar record lower Component host execution passed\n'
