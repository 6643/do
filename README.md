...
- 验证入口: 当前默认完整回归基线为 `pass=1398 fail=0 skip=3`，`zig test main.zig` 为 `686/686`，默认 GC gate 为 `86 fixtures`；`RUN_WASM=1` 的最后已核实 checkpoint 为 `pass=1269 fail=0 skip=3`（2026-08-20），不作为当前默认基线；发布前 smoke 入口是 `./src/build/test/run_release_smoke.sh`。
