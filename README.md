...
- 验证入口: 默认完整回归基线为 `pass=1108 fail=0 skip=3`; `RUN_WASM=1` 扩展回归基线需在当前环境单独复跑 (最近记录为 `pass=1100 fail=0 skip=3`, wasm run `pass=6 fail=0`); 发布前 smoke 入口是 `./src/build/test/run_release_smoke.sh`。
