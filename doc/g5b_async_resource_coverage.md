...
underlying compiler test aggregate is `zig test main.zig 1583/1583`. These are
...
| direct/paired/parameterized/triple/nested/single-list/two-list owned-record producers | PASS | `test_rust_g6_2_owned_record_producer.sh`, `test_rust_g6_2_owned_record_pair_producer.sh`, `test_rust_g6_2_owned_record_pair_parameterized_producer.sh`, `test_rust_g6_2_owned_record_triple_producer.sh`, `test_rust_g6_2_owned_record_nested_producer.sh`, `test_rust_g6_2_list_owned_record_producer.sh`, `test_rust_g6_2_two_list_owned_record_producer.sh` | Generated Component lifecycle scripts pass valid, error, cancel/early-drop, repeat, and invalid modes with exact resource cleanup plus `table-empty=true`. For the two-list route, static WAT checks lock two allocation call sites, the shared release-counter path, and four terminal cleanup call sites; transferred-list reads are observed at runtime, while printed allocation/release totals are derived expected cardinalities rather than direct Core-global observations. Canonical/generated parity is separate evidence. |
