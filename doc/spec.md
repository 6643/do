...
7. 查编译器模块边界: 流水线入口在 `src/build/lexer.zig` / `parser.zig` / `sema.zig` / `gen.zig`; sema 域模块为 `sema_util` / `sema_types` / `sema_func` / `sema_struct` / `sema_type` / `sema_import` / `sema_ctrl`; 纯类型/布局 SSOT 在 `src/build/type_name.zig`; storage 指针与 payload/Tuple pack 的纯 WAT 在 `src/build/gen_storage_wat.zig` / `src/build/gen_payload_wat.zig` (见 `AGENTS.md` 与 `doc/start_here.md`)。
