...
7. 查编译器模块边界: 流水线在 `src/build/lexer.zig` / `parser.zig` / `sema.zig` / `gen.zig`; 纯类型/布局 SSOT 在 `src/build/type_name.zig`; storage 指针与 payload/Tuple pack 的纯 WAT 在 `src/build/gen_storage_wat.zig` / `src/build/gen_payload_wat.zig` (见 `AGENTS.md` 与 `doc/start_here.md`)。
