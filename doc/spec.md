...
9. `doc/wit/wasi_p3_lowering.md`: 统一 `@host(locator, member, sig)` / WIT / component lowering 的 compiler-facing 合同和当前可验证产物。
...
7. 查编译器模块边界: 流水线入口在 `src/build/lexer.zig` / `parser.zig` / `sema.zig` / `codegen_api.zig`; sema 域模块为 `sema_util` / `sema_types` / `sema_func` / `sema_struct` / `sema_type` / `sema_import` / `sema_ctrl`; 纯类型/布局 SSOT 在 `src/build/type_name.zig`; storage 指针与 payload/Tuple pack 的纯 WAT 在 `src/build/gen_storage_wat.zig` / `src/build/gen_payload_wat.zig` (见 `AGENTS.md` 与 `doc/start_here.md`)。
...
| Host ABI / `@host` / WIT lowering | `doc/spec_rules.md` 第 3, 13-14 章 | `doc/wit/wasi_p3_lowering.md`, `doc/wit/wasi_registry.json` |
...
11. `@host` 的 WASI locator/member 形态声明的是 WIT binding, 不是普通 core Wasm import; 当前只开放已登记的 scalar/record/list<u8> 与少量 result-area wrapper 子集进入 lowering, 完整 component/resource/future/variant 支持仍后置。
