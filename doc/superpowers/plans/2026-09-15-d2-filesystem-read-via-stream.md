...
- [ ] **Step 3: Implement the adapter-based probe.**
...
- Create: `src/build/test/compile_err/451_wasi_filesystem_read_via_stream_wrong_host_kind.do`
- Create: `src/build/test/compile_err/452_wasi_filesystem_read_via_stream_wrong_offset.do`
- Create: `src/build/test/compile_err/453_wasi_filesystem_read_via_stream_wrong_item.do`
- Create: `src/build/test/compile_err/454_wasi_filesystem_read_via_stream_missing_completion.do`
...
- [ ] **Step 1: Add RED semantic and target tests.**
...
Assert that `@host_async_func`, `u32`, `Stream<u16>`, a non-pinned locator, and a completion future missing from the body are rejected with the established component diagnostic.
...
Expected: FAIL because no descriptor effect or target accepts this method.

- [ ] **Step 3: Define the private descriptor exactly.**
...
- [ ] **Step 4: Run focused green and fixture tests.**
...
Expected: PASS for the fixed fixture; every drift fixture is rejected.

- [ ] **Step 5: Commit fail-closed admission.**
...
- [ ] **Step 1: Add RED route-plan and WAT tests.**

Use a one-read source and a three-read source. Assert that the plan captures the descriptor variable, `u64` offset variable, tuple, reader, completion, and `read_count`. Assert WAT contains the synchronous method import, `i64` offset transport, one reusable byte-stream read site, one completion read site, and guarded drops for stream, future, and descriptor.
...
- [ ] **Step 3: Implement the route-specific parser and state machine.**
...
- [ ] **Step 4: Add assembly gate and run green.**
...
Expected: PASS for both fixed fixtures.

- [ ] **Step 5: Commit the dedicated emitter.**
...
- [ ] **Step 1: Add RED host-counter assertions.**
...
Expected: FAIL because the generated WIT sidecar, runner binary, and script do not exist.

- [ ] **Step 3: Implement a controlled byte producer and lifecycle modes.**
...
- [ ] **Step 4: Build the Component and run the full matrix.**
...
Expected: PASS; every live-store terminal mode prints exactly-once drops and `table-empty=true`, while early-drop reports disposal without a table claim.
...
- [ ] **Step 1: Add integration markers.**
...
- [ ] **Step 2: Run the integrated harness.**
...
- [ ] **Step 3: Document only the verified route.**
