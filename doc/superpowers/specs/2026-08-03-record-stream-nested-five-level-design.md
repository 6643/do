# Five-Level Nested Owned Resource Record Stream Design

## Goal

Extend the private descriptor-driven record-stream consumer to one bounded
five-level nested `own<ticket>` path:

`resource-entry.inner.deep.deeper.deepest.ultra.ticket`.

The path remains frame-owned, decoded recursively, and released exactly once.

## Boundary

- Admit exactly one top-level nested path with five container levels.
...
- Keep sixth-level paths, `borrow`, `list`, `variant`, mixed scalar/nested
...
Raise the manifest parser's explicit nested-depth ceiling from four to five.
