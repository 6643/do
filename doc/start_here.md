...
| G5c route consolidation | C14–C20 已统一消费一次 manifest-backed `LoadedRequest`、descriptor registry 与 measured `SyncValuePlan`；default/explicit route 不重复解析 request，已登记 locator 的 member drift 在 WAT 前 fail-closed。新鲜 focused route/residual gate、`run_tests.sh` `pass=1410 fail=0 skip=3`、`zig test main.zig` `692/692`、default `86 fixtures`、semantic-equivalence `26 rows; 0 pending`、ReleaseSmall/release smoke 已通过；inventory 仍为 `complete_rows=15 pending_rows=15`、exit 1。 |
...
| G5c residual capability matrix | 15 个 residual row 已逐行复核且仍全部 pending；14 行 blocked，唯一 candidate 为 `demo:marshal-record-mixed-text-byte-u32-lists-lower/api.write@1.0.0/lower`。该候选只接受同步、manifest-backed、ABI 可测量且 canonical ABI 无 GC reference 的固定 shape，已完成 spec/host/equivalence/negative/default/full verification；release-candidate maintenance、G6.2 双 owned-field producer gate 与固定三字段 `ResourceTriple` private design/probe 已完成，下一阶段若要 compiler admission 必须另立实现计划并获明确批准，不扩大默认 route。 |
...
| G6.2 fixed three-owned-field record producer probe | 私有 canonical WIT package `do:g6-2-owned-record-triple-producer@0.1.0`，hash 为 `73fd57dc8f34f13023b48f2b82e439b212eab52192886f0d07a37d86e63658b1`；`ResourceTriple` 为 12 bytes、alignment 4，`left/middle/right: own<Ticket>` 位于 offset `0/4/8`，stream capacity `1`，输入为 `(mode,left-seed,middle-seed,right-seed)` 四个 `u32` words。canonical WAT parse/embed/component-new/validate 与 Rust/Wasmtime 十模式 lifecycle gate 通过；valid 为 `3/3` ticket cleanup、repeat 为 `6/6`，所有模式 `table-empty=true`，四种取消/早退各有一次 cancel 与 pending-future drop，invalid 不创建资源。仅为 private design/probe evidence，不计入 ARC/GC 26-row 矩阵，不新增 manifest/compiler/Do/public ownership syntax；若要 admission 必须另立实现计划并获明确批准。 |
...
# 最新文档记录的验证 (2026-08-28): pass=1400 fail=0 skip=3; `zig test main.zig` `688/688`;
# default GC gate `86 fixtures`; semantic-equivalence `26 rows; 0 pending`;
# ReleaseSmall/release smoke 通过。G6.2 双 owned-field record producer 的十模式
# canonical/generated lifecycle gate 也已通过；inventory 仍按设计为 15/15 pending、exit 1。
...
`pass=1410 fail=0 skip=3`、`zig test main.zig` `692/692`、default GC gate
...
`ResourceTriple` probe 的 canonical ABI、Component assembly、negative、Rust/Wasmtime
lifecycle 与已有 route equivalence 十模式 gate。host、ARC/GC equivalence、negative、
...
direct owned-record、双 owned-field producer 与固定三字段 `ResourceTriple` probe 的独立
design/lifecycle gate 均已闭环；下一阶段若获明确批准，才按 design -> manifest/hash/ABI
-> compiler/source matcher -> negative -> Component -> Rust/Wasmtime -> exactly-once cleanup
顺序建立独立 compiler admission 计划。默认 route 不在实现计划批准前扩大，当前仍不开放通用 `list<T>`、aggregate inference、
async/resource lowering 或 `own<T>`/`borrow<T>`/`ref<T>`。
...
| ID | 说明 | 恢复条件 |
| --- | --- | --- |
| G6.2 | `descriptor.read-directory`、注册 record-stream consumer 与 bounded scalar/dynamic/batched producer | scalar/string、多-owned-resource、多个顶层 nested paths 与一层/两层/三层/四层/五层/六层 nested-owned-resource consumer、注册 `do:stream-probe` 的 capacity-one `StreamWriter<u8>` producer、固定/参数化 `u64` countdown producer、参数化 helper 的六跳 forwarding、typed 参数受限重排、其它同类型 async helper、bounded StreamMirror、私有 `variant-resource-stream`、private C-min list/resource producer、动态 count `0..3` producer、固定两批 list-resource producer 与固定三字段 `ResourceTriple` private design/probe 已验证；一般 producer lease、borrowed/list/通用 variant、第七跳 forwarding、第七层或更一般 nested resource gates、triple 的 compiler admission 与任意 filesystem async method 仍待单独推进 |
