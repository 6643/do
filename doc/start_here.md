...
# 最近扩展基线: pass=1197 fail=0 skip=3; wasm run summary: pass=6 fail=0
```

| 基线项 | 最近值 |
| --- | --- |
| 默认回归 (`SKIP_BUILD=1`) | `pass=1195 fail=0 skip=3` |
| WASM 扩展回归 (`RUN_WASM=1 SKIP_BUILD=1`) | `pass=1197 fail=0 skip=3`; smoke `6/6` |
| `zig test main.zig` | `343/343` |
...
私有 D2 filesystem async 当前只开放四个独立的有界方法：
`wasi:filesystem/types@0.3.0-rc-2025-09-16 / descriptor.get-type`、
`descriptor.sync`、`descriptor.get-flags` 与 `descriptor.sync-data`。`get-type` 的
...
Store-disposal early-drop 行均通过。四者都不意味着通用
