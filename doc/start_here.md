...
最新闭环: G6.2 private two-list owned-record producer 已通过 canonical ABI、
Do/Component、Rust/Wasmtime 十模式 lifecycle 与 canonical/generated byte
parity。布局为 record `20/4`, `first.ptr/len=0/4`,
`second.ptr/len=8/12`, `ticket=16`, list `stride/capacity=4/3`; WIT hash 为
`14ba67e070346a127e75c7dfd73c71d6082ad7d9385c7d803834319dabc6404a`。
静态 WAT 门禁固定两个 allocation call site、共享 release-counter path 与四个终态
cleanup call site; runner 直接观测 transferred-list read、ticket drop 和空
ResourceTable。其 list 总数按有效
调用数推导而非直接读取 Core global; repeat 为推导 `4/4` list 与直接观测 `2/2`
ticket, invalid 为 `0/0`, 所有模式 `table-empty=true`。完整 harness 为
`14/14 steps; 53/53 tests`, `zig test main.zig` 为 `1583/1583`,
ReleaseSmall build 通过。该 route 仍为 private fixed-shape evidence, 不开放
public ownership syntax 或 generic producer lowering。
...
| 当前验证基线 | 默认及 `RUN_WASM=1 RUN_GC_CORE=1` 为 `14/14 steps; 53/53 tests`；`zig test main.zig` 为 `1583/1583`；GC default gate `87 fixtures`、semantic-equivalence `26 rows; 0 pending`、ReleaseSmall/release smoke 通过 |
