...
- 当前主线已经推进到 `doc/roadmap_status.md` 的 `07. 生态工具`，阶段 A 工具链体验补齐已收口: `do run`、`do fmt` stdout/check/write、`do lsp` diagnostics/formatting/semantic tokens、`do check` 单/多文件和工具链收口检查均已完成。
...
- 新的总规划入口是 [doc/master_plan.md](/home/_/._/do/doc/master_plan.md); 默认下一步按其中“阶段 B: 语法和语义冻结审查”推进。B1 grammar / parser 差异审查已按用户选择落地并删除问题文件, 下一步优先 B2 spec_rules / sema 差异审查。
