...
- I1 当前缺口: 递归错误分支在 `ok/183_recursive_error_union.do` 上仍会被静态 runner skip; imported self-tail `if/else` 在 `ok/189_imported_self_tail_if_else_tco.do` 上也仍会被静态 runner skip。当前这两类先由 `.compiled_must_pass` 保证 compiled 路径执行通过。
