...
- `@wasi_func("package/interface/member", (DoOrWitTypes…) -> DoOrWitRet)`：函数绑定。已知 target 允许 do 侧糖（如 `i32` 代替 `descriptor`、`[u8]` 代替 `list<u8>`）；codegen 将 manifest 规范为 WIT 形。
...
`WitType` 只出现在 `@wasi_func(...)` 的 host import 签名里；其中 `char` 等尚未进入 Do 源码类型系统的 WIT 标量名只是 ABI 边界 token。WIT `string` 映射到 Do 源码 `text`，标准库在和原始字节交互时必须显式使用 `[u8]` 转换边界。
