...
- `@wasi_func("package/interface/member", (DoOrWitTypes…) -> DoOrWitRet)`：函数绑定。**推荐**在签名里写 do 类型：resource/record 名（`Dir`/`File`/`Datetime`）、`[u8]`/`text`/标量糖，以及排他联合 `Ok | Err` / `T | nil`（见 §23）。已知 target 的 do 侧糖由 codegen 规范为 WIT 写入 manifest。
