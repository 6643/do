...
28a. 普通函数的异步语义不向调用者传染：函数声明仍使用普通签名，函数体中的显式 `@await`/`@cancel` 不会把普通调用改成 `Future<T>`。普通 `helper()` 是 inline 调用；只有显式 `@async(helper())` 才创建用户函数 Future。`--p3-async-call-component` 当前只收窄两个 root-owned unit 形状：child-only 的 `@async(helper())`，以及其前恰好一个零参数 `helper()` 的 inline 形状；后者的 inline host phase、explicit child phase 和 terminal phase 都由 root frame 管理。两种形状都不公开 helper WIT export，不引入 `own<T>`、`borrow<T>`、`ref<T>`、指针、引用或生命周期语法。第二个 inline call、非 `nil` helper payload、参数、分支、循环、递归、多个 child、资源/Stream/list 或任意 producer expression 在 WAT 前报告 `UnsupportedP3AsyncCallComponent`；v1/default dispatch 与更一般 async-call composition 仍保持原边界。
