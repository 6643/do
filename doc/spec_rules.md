...
12. v1 不提供 `@field_type`、`@field_default_value` 或 `@field_default_type`。需要序列化时，使用 `fields(TypeOrTypeParam)` 枚举字段，再通过 `@field_get` 调用具体类型编码函数；泛型序列化函数可以写成 `#T stringify(value T) -> [u8]` 并在函数体内使用 `fields(T)`。
