...
当前结论: C5.2 已开始。C1 JSON stringify / from_json 已收口, C2 字段反射 API 收口结束, C3 bytes / text 边界已完成。C4 已完成集合 skip 收回与剩余 skip / `NoTestDecl` 分类。C5.1 已按直接 fixture 引用、compiled pass、compile_ok、static pass 和 skipped 五类盘点 `src/*.do` 覆盖状态。C5.2 已先为 `binary.do` 与 `path.do` 非 variadic helper 补 executable fixture。下一步继续为 `url/range/slice/bytes/hex` 这批纯函数库补 fixture。
...
- [ ] C5.2 为纯函数库补 `do test` fixture。
  ...
  - [ ] C5.2.3 继续补 `url/range/slice/bytes/hex` 纯函数库 fixture。
