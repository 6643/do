...
2026-09-15 增量: D2 private `descriptor.read-via-stream` bounded byte-reader
gate 已闭环。固定同步 `@host_func` method ABI 为
`(File,u64) -> Tuple<Stream<u8>, Future<Result<nil,FileError>>>`，Core import 为
`(i32,i64,i32) -> nil`；编译器只接受一至三次显式 byte read 和一次 completion
await。ABI、private Component lowering、Rust/Wasmtime ready/pending/error/cancel/
early-drop/repeat 生命周期门禁均通过当前 `wasm-tools 1.258.0` / Wasmtime
`48.0.1` adapter。该 route 仍是 private fixed-shape evidence，不开放通用
...
`do:g6-2-owned-record-two-list-producer@0.1.0` 仅在
`--p3-async-component` 下接受
`TwoListEntry { first: list<u32>, second: list<u32>, ticket: own<Ticket> }`
与 capacity-one `stream<two-list-entry>`; WIT SHA-256 为
`14ba67e070346a127e75c7dfd73c71d6082ad7d9385c7d803834319dabc6404a`。
record size/alignment 为 `20/4`, `first.ptr/len` offset 为 `0/4`,
`second.ptr/len` offset 为 `8/12`, `ticket` offset 为 `16`, list
stride/capacity 为 `4/3`, stream capacity 为 `1`。immutable
`ProducerContract` 保留 `first`/`second` 两条独立 list allocation cleanup path,
ownership mask 仅覆盖 `ticket`, 完整 record 写入后才提交 transfer。

Do/Component 正向 gate、13 个 WAT 前负例、canonical ABI、生成
Rust/Wasmtime 十模式 lifecycle 与 canonical/generated byte parity 均通过。
ready/pending、sink error、transfer 前后 cancel/early-drop、repeat、invalid
均验证 cleanup。静态 WAT 门禁固定两个 allocation call site、共享
release-counter path 与四个终态 cleanup call site; runner 直接观测
transferred-list read、ticket drop 和空 ResourceTable。runner 输出的 list
allocation/release 总数按有效调用数推导, 不表示
直接读取 Component 内部 Core global; repeat 推导值为 `4/4` list, ticket 直接观测
为 `2/2`, invalid 为 `0/0`; 所有模式 `table-empty=true`, 生成 WAT 不含
`__arc_` 且不跨 Wasm GC reference boundary。
fresh harness 为 `14/14 steps; 53/53 tests`, `zig test main.zig` 为
`1583/1583`, ReleaseSmall build 通过。该项仍是 private fixed-shape
evidence, 不新增 GC inventory row, 不开放 public `own<T>`/`borrow<T>`/
`ref<T>`、generic/arbitrary producer、borrowed/variant payload 或 general
async/resource lowering; inventory 仍为 `complete_rows=15 pending_rows=15`。
