...
当前结论: C3.1 已完成。C1 JSON stringify / from_json 已收口, C2 字段反射 API 收口结束。`src/bytes.do` 当前承载 `[u8]` 序列操作, `src/text.do` 当前是 text / `[u8]` 显式转换和 UTF-8 校验包装层, UTF-8 细节落在 `src/utf8.do`。现有 bytes/text/utf8 直接 ok fixture 仍是 static skip, 只有 JSON 的 text/[u8] 字段解析路径已有 compiled 执行正例。下一步进入 C3.2, 补 bytes/text 转换正例。
...
- [ ] C3.2 补 bytes/text 转换正例。前置: C3.1 已确认 `ok/121_source_text_type.do` 当前只有 static skip, 需要补 compiled 可执行正例或等价 build 回归。
