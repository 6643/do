...
回归均为 `14/14 steps; 53/53 tests`，独立 `zig test main.zig` 为 `1561/1561`，
...
2026-09-09 增量: 私有 async `map<u32,u32>` compiler admission 已闭环，仅由
`--p3-async-map-component` 选择。严格 analyzer 只接受
`demo:map-async-probe/api@0.1.0.submit`、`HashMap<u32,u32>`、两个固定 pair
`[7,70]`/`[9,90]`、一个 helper/root await 拓扑和 pinned WIT hash
`821f5a1d20b600284efca10ad78f16e64d3ca5f42df566d4f045ef5b5348d3a0`；生成的
Core 形状为 `(i32 ptr, i32 len, i32 result_area) -> i32`。WAT/WIT snapshot、
copy-before-call、post-call overwrite、result-area/root return 与 cleanup markers
均通过，错误 value type、动态 map、额外 pair、同步 host marker、第二次 await
五个负例均在 WAT 前返回 `UnsupportedP3AsyncMapComponent`。Component
parse/embed/new/validate 与 Rust/Wasmtime `ready/pending/cancel/drop` gate
通过；默认不带 flag 的同一 fixture 仍返回 `AsyncLoweringUnavailable` 且不写出
WAT/WIT。该 admission 不开放通用 async map、其他 key/value、Stream 跨 poll
buffer、arbitrary producer 或 public ownership syntax。
...
2026-09-04 增量: 两个 bounded GC runtime-oracle gate
`test_async_frame_table.sh` 与 `test_cabi_realloc_budget.sh` 已统一通过
current-only `bin/do-toolchain` 的 parse、GC compile 和 invoke 操作；Zig harness
新增 `GC runtime oracle matrix`，固定观察 async-frame `27815/1/1`、realloc
`4/1` 与 quota rejection trap。根目录及无关 `/tmp` cwd 各自通过，完整
`zig build test --summary all` 为 `14/14 steps、51/51 tests`。这只是 GC runtime
...
| 当前验证基线 | 默认及 `RUN_WASM=1 RUN_GC_CORE=1` 为 `14/14 steps; 53/53 tests`；`zig test main.zig` 为 `1561/1561`；GC default gate `87 fixtures`、semantic-equivalence `26 rows; 0 pending`、ReleaseSmall/release smoke 通过 |
