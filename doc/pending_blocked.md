...
capacity-one `stream<two-list-entry>`、同步 source `(i32) -> (i32)` 和固定
producer 输入; WIT hash 为
`14ba67e070346a127e75c7dfd73c71d6082ad7d9385c7d803834319dabc6404a`, record
`20/4`, `first.ptr/len=0/4`, `second.ptr/len=8/12`, `ticket=16`, list
`stride/capacity=4/3`。`first` 与 `second` 是两条独立 allocation cleanup path,
ticket 是唯一 ownership leaf, 完整 record 写入后才 transfer。

Do/Component、13 个 WAT 前负例、canonical ABI、生成 Rust/Wasmtime 十模式和
canonical/generated byte parity 均通过。静态 WAT 门禁固定两个 allocation call
site、共享 release-counter path 与四个终态 cleanup call site; runner 直接观测
transferred-list read、ticket drop 和空 ResourceTable。runner 输出的 list
allocation/release 总数按有效调用数推导,
不表示直接读取 Component 内部 Core global; repeat 推导值为 `4/4`, ticket 直接
观测为 `2/2`, invalid 不创建资源, 所有模式 `table-empty=true`。fresh harness 为
`14/14 steps; 53/53 tests`,
`zig test main.zig` 为 `1583/1583`, ReleaseSmall build 通过, 生成 WAT 无
`__arc_` 且无 Wasm GC reference boundary。该项不关闭通用 producer、arbitrary
expression、borrowed/variant payload、general async/resource lowering 或 public
`own<T>`/`borrow<T>`/`ref<T>` syntax, 也不新增 inventory row。
...
`write-via-stream`, `append-via-stream`,
