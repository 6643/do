...
状态: v1 子集发布候选已收口; G5c residual capability matrix 已完成且当前 exact candidate 已通过专用与全局门禁; G6 generic consumer、bounded nested resource paths、私有双 owned-field record producer、参数化双 owned-field record producer 与固定三字段 `ResourceTriple` private compiler admission 已闭环, D2 私有 descriptor slices 与私有有界 `stream<list<u32>>` producer promotion 已闭环, 剩余 producer/resource residual
更新时间: 2026-08-28
...
- 最新 fresh release-candidate 回归: `./src/build/test/run_tests.sh` → `pass=1424 fail=0 skip=3`; `zig test main.zig` → `695/695`; ReleaseSmall/release smoke 均通过。default GC gate 与 semantic-equivalence 的既有基线保持不变；`RUN_WASM=1` 的最后已核实 checkpoint 为 `pass=1269 fail=0 skip=3`（2026-08-20），不作为当前默认基线；inventory 仍为 `complete_rows=15 pending_rows=15`、预期 exit `1`。
