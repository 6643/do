...
状态: v1 子集发布候选已收口; G6 generic consumer 与 bounded nested resource paths 已闭环, D2 私有 `descriptor.get-type`/`descriptor.sync`/`descriptor.get-flags` 已闭环, 私有有界 `stream<list<u32>>` producer promotion 已闭环, 剩余 producer/resource residual
更新时间: 2026-08-09
...
- 最近回归: Bun Node-compatible runner 下 `./src/build/test/run_tests.sh` → `pass=1167 fail=0 skip=3`; `RUN_WASM=1 SKIP_BUILD=1` → `pass=1169 fail=0 skip=3` (WASM smoke `6/6`); ReleaseSmall smoke 与 `cd src && zig test main.zig` (`319/319`) 均通过。
