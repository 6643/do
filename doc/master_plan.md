...
状态: v1 子集发布候选已收口; G6 generic consumer 与 bounded nested resource paths 已闭环, D2 私有 `descriptor.get-type`/`descriptor.sync`/`descriptor.get-flags`/`descriptor.stat`/`descriptor.sync-data`/`descriptor.metadata-hash` 已闭环, 私有有界 `stream<list<u32>>` producer promotion 已闭环, 剩余 producer/resource residual
更新时间: 2026-08-10
...
- 阶段 A–F、H 已完成; D 可推进项与 D2.1 已收口; D2 真实本地 file/dir/CLI stream、compiler-generated TCP/UDP socket create/bind/drop loopback smoke 与私有 `descriptor.get-type`/`descriptor.sync`/`descriptor.get-flags`/`descriptor.stat`/`descriptor.sync-data`/`descriptor.metadata-hash` async slices 已收口，但总项仍受通用 filesystem async/external HTTP 阻断; G1–G5、G6.4 已完成; **阶段 I (I1+I2) 已关闭**。
- 架构扁平拆分已落地: `type_name` / `sema_error` / `diagnostics` / `gen_*` 域竖切 / `sema_*` 域竖切 (见 `AGENTS.md`)。
- 最近回归: Bun Node-compatible runner 下 `./src/build/test/run_tests.sh` → `pass=1199 fail=0 skip=3`; `RUN_WASM=1 SKIP_BUILD=1` → `pass=1201 fail=0 skip=3` (WASM smoke `6/6`); ReleaseSmall smoke 与 `cd src && zig test main.zig` (`347/347`) 均通过。
...
- D2 `descriptor.metadata-hash` 的私有记录固定 upstream WIT hash
  ...
  cancel/Store-disposal early-drop oracle matrix 均通过；`metadata-hash-at`、
  generic filesystem async and host-future-drop cancellation remain pending.
...
3. 维护 D2 真实 host matrix：local filesystem preopen/read-directory、CLI pipe、socket create/bind/drop loopback 与私有 `descriptor.get-type`/`descriptor.sync`/`descriptor.get-flags`/`descriptor.stat`/`descriptor.sync-data`/`descriptor.metadata-hash` 已有 gate；保留通用 filesystem async、external HTTP 的明确阻断。
