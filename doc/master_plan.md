...
状态: v1 子集发布候选已收口; G5c residual capability matrix 已完成且当前 exact candidate 已通过专用与全局门禁; G6 generic consumer、bounded nested resource paths 与私有双 owned-field record producer 已闭环, D2 私有 descriptor slices 与私有有界 `stream<list<u32>>` producer promotion 已闭环, 剩余 producer/resource residual
更新时间: 2026-08-28
...
- 最新文档记录的回归: `./src/build/test/run_tests.sh` → `pass=1400 fail=0 skip=3`; `zig test main.zig` → `688/688`; default GC gate → `86 fixtures`；semantic-equivalence → `26 rows; 0 pending`；ReleaseSmall/release smoke 均通过。`RUN_WASM=1` 的最后已核实 checkpoint 为 `pass=1269 fail=0 skip=3`（2026-08-20），不作为当前默认基线；inventory 仍为 `complete_rows=15 pending_rows=15`、预期 exit `1`。
...
4. G6.2 direct owned-record producer 与本轮 bounded two-owned-field record producer 的独立 lifecycle gates 均已闭环；下一步仍只能为新的 producer/resource shape 建立独立 design gate，并按 design → manifest/hash/ABI → negative → Component → Rust/Wasmtime ready/pending/error/cancel/drop → ownership invariant 顺序闭环，再考虑 default promotion。
