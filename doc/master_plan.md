...
capability inventory 和公开语言能力均未改变；test-only counter world 通过
`runtime-counter-event` callback 在 `$frame-alloc/$frame-free` 观察十模式精确
alloc/free delta。`ready`、`pending`、四种 sink/cancel/early-drop mode 为 `1/1`，
`repeat` 为 `2/2`，`invalid` 为 `0/0`。A 裁定要求 `mode=255` 在 frame allocation
前直接返回，因而不创建 frame/stream/resource。旧 `runtime-counters` Core-global
tuple 在 Wasmtime async Component 边界保留为诊断，不能作为 runtime 证据。
12-route migration、generic/arbitrary producers、public ownership syntax、semantic-parity
rewrite 和 D2 general async 保持 deferred。详见
`doc/superpowers/specs/2026-09-14-g6-2-pilot-runtime-counter-gate-design.md`。
...
## 0.4 2026-09-11 list-owned-record producer compiler admission

G6.2 新增的 private list-owned-record producer compiler/runtime gate 已闭环。
精确 descriptor `do:g6-2-owned-record-list-producer@0.1.0` 仅在
...
Do/Component、10 个 WAT 前负例、canonical ABI、生成 Rust/Wasmtime 十模式
lifecycle、canonical/generated parity、完整 harness 与 release smoke 均通过。
完整 harness 为 `14/14 steps; 53/53 tests`，独立 `zig test main.zig` 为
`1575/1575`；valid/invalid、cancel/early-drop、repeat 均满足 exactly-once
list/resource cleanup，所有模式 `table-empty=true`，生成 WAT 不含 `__arc_`。
该项不新增 GC inventory row，不开放 public ownership syntax、generic/arbitrary
producer 或 general async/resource lowering。
...
- 最新 fresh release-candidate 回归: 默认及 `RUN_WASM=1 RUN_GC_CORE=1` 组合的 `./src/build/test/run_tests.sh` 均为 `14/14` steps、`56/56` tests；`zig test main.zig` 为 `1743/1743`。ReleaseSmall/release smoke、GC default gate（87 fixtures）、ARC inventory 当前 post-cutover（`rows=55 matches=492 unclassified=0 normal_route_matches=0`）、生产依赖闭包（`modules=162 forbidden=0`）和 semantic-equivalence（26 rows; 0 pending）均通过；inventory 仍为 `complete_rows=15 pending_rows=15`、预期 exit `1`。
