...
sha1_f0(b u32, c u32, d u32) -> u32 {
    return @or(@and(b, c), @and(bit_not_u32(b), d))
}
...
.pad(bytes [u8]) -> List<u8> {
    ...
    loop {
        if @eq(@rem(list_len(out), 64), 56) return append_u64_be(out, @mul(@to_u64(@len(bytes)), 8))
        out = list_add(out, 0)
    }
}
...
.block(h0 u32, h1 u32, h2 u32, h3 u32, h4 u32, block List<u8>, base usize) -> u32, u32, u32, u32, u32 {
    ...
    loop {
        ...
        idx usize = @rem(i, 16)
        if @ge(i, 16) {
            s u32 = @xor(@xor(@get(w, @rem(@sub(i, 3), 16)), @get(w, @rem(@sub(i, 8), 16))), @xor(@get(w, @rem(@sub(i, 14), 16)), @get(w, idx)))
            w = @set(w, idx, @rotl(s, 1))
        }
        ...
        t u32 = add_wrap_u32(add_wrap_u32(add_wrap_u32(add_wrap_u32(@rotl(a, 5), f), e), wi), k)
        e = d
        d = c
        c = @rotl(b, 30)
        b = a
