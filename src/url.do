...
url_decode(bytes [u8]) -> [u8] | UrlError {
    out [u8] = .{}
    i usize = 0
    loop {
        ...
        if @ne(byte, _percent) {
            ...
            if @ge(@add(i, 2), @len(bytes)) return UrlInvalidEscape
            hi = hex_value(@get(bytes, @add(i, 1)))
            if @is(hi, UrlError) return hi
            lo = hex_value(@get(bytes, @add(i, 2)))
