...
pub const Error = error{UnsupportedP3WasiFilesystemSyncComponent};

const locator = "wasi:filesystem/types@0.3.0-rc-2025-09-16";
const member = "descriptor.sync";
...
pub fn emit_component_wat(
    ...
    const plan = try SyncPlan.analyze(tokens, registry);
    if (!std.mem.eql(u8, plan.descriptor.canonical.async_import_module, locator) or
        !std.mem.eql(u8, plan.descriptor.canonical.async_import_name, "[async-lower][method]descriptor.sync"))
        return error.UnsupportedP3WasiFilesystemSyncComponent;
    return generated_text.alloc_block(allocator, 0, @embedFile("wasi_filesystem_sync_component_template.wat"));
}
...
pub fn emit_component_wit(
    ...
    var registry = try p3_async_manifest.Registry.load(allocator, @embedFile("p3_async_registry.json"));
    defer registry.deinit(allocator);
    _ = try SyncPlan.analyze(tokens, registry);
...
pub const SyncPlan = struct {
    ...
    pub fn analyze(tokens: []const lexer.Token, registry: p3_async_manifest.Registry) Error!SyncPlan {
        const descriptor = registry.find(locator, member) orelse return error.UnsupportedP3WasiFilesystemSyncComponent;
        switch (p3_async_manifest.lowering_shape(descriptor) orelse return error.UnsupportedP3WasiFilesystemSyncComponent) {
            .filesystem_sync => {},
            else => return error.UnsupportedP3WasiFilesystemSyncComponent,
        }
        ...
        while (idx + 9 < tokens.len) : (idx += 1) {
            ...
            if (host != null or !signature_matches(tokens, idx + 9)) return error.UnsupportedP3WasiFilesystemSyncComponent;
            host = .{ .name = tokens[idx].lexeme };
        }
        const host_binding = host orelse return error.UnsupportedP3WasiFilesystemSyncComponent;
        if (host_count != 1 or !has_exact_resource_decl(tokens) or !has_exact_sync_error_decl(tokens))
            return error.UnsupportedP3WasiFilesystemSyncComponent;

        const function = find_run_function(tokens) orelse return error.UnsupportedP3WasiFilesystemSyncComponent;
...
fn signature_matches(tokens: []const lexer.Token, signature_start: usize) bool {
    ...
    return host_close > params_close + 3 and
        compact_token_range_equals(tokens, params_close + 3, host_close, "nil|SyncError");
}
...
fn has_exact_sync_error_decl(tokens: []const lexer.Token) bool {
    return has_exact_token_sequence(tokens, "SyncError", "SyncErrorerror=Io|NoEntry");
}
...
fn find_run_function(tokens: []const lexer.Token) ?RunFunction {
    ...
    while (idx + 8 < tokens.len) : (idx += 1) {
        ...
        if (!compact_token_range_equals(
            ...
            body_open + 2,
            body_close,
            "pendingFuture<nil|SyncError>=sync_descriptor(file)resultnil|SyncError=@await(pending)",
        )) return null;
        return .{
...
test "filesystem descriptor sync source shape admits the bounded direct await" {
    ...
    try std.testing.expect(has_exact_sync_error_decl(tokens));
