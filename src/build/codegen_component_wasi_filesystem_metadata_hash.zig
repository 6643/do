...
pub const Error = error{UnsupportedP3WasiFilesystemMetadataHashComponent};

const locator = "wasi:filesystem/types@0.3.0-rc-2025-09-16";
const member = "descriptor.metadata-hash";
...
pub fn emit_component_wat(
    ...
    const plan = try MetadataHashPlan.analyze(tokens, registry);
...
pub const MetadataHashPlan = struct {
    ...
    pub fn analyze(tokens: []const lexer.Token, registry: p3_async_manifest.Registry) Error!MetadataHashPlan {
        const descriptor = registry.find(locator, member) orelse return error.UnsupportedP3WasiFilesystemMetadataHashComponent;
        switch (p3_async_manifest.lowering_shape(descriptor) orelse return error.UnsupportedP3WasiFilesystemMetadataHashComponent) {
            .filesystem_metadata_hash => {},
            else => return error.UnsupportedP3WasiFilesystemMetadataHashComponent,
        }
        ...
        while (idx + 9 < tokens.len) : (idx += 1) {
            ...
            if (host != null or !signature_matches(tokens, idx + 9)) return error.UnsupportedP3WasiFilesystemMetadataHashComponent;
            host = .{ .name = tokens[idx].lexeme };
        }
        const host_binding = host orelse return error.UnsupportedP3WasiFilesystemMetadataHashComponent;
        ...
        const function = find_run_function(tokens) orelse return error.UnsupportedP3WasiFilesystemMetadataHashComponent;
...
fn find_run_function(tokens: []const lexer.Token) ?RunFunction {
    ...
    while (idx + 8 < tokens.len) : (idx += 1) {
        ...
        if (!compact_token_range_equals(tokens, body_open + 4, body_close,
            "pendingFuture<MetadataHash|HashError>=metadata_hash(file)return@await(pending)")) return null;
        return .{
...
test "metadata-hash source shape admits the bounded direct await" {
    ...
    const plan = try MetadataHashPlan.analyze(tokens, registry);
    try std.testing.expectEqualStrings("metadata_hash", plan.host_name);
...
test "metadata-hash source shape rejects a reordered record" {
    const source = @embedFile("test/compile_err/518_wasi_filesystem_metadata_hash_wrong_record.do");
    const tokens = try lexer.tokenize(std.testing.allocator, source);
    defer std.testing.allocator.free(tokens);
    var registry = try p3_async_manifest.Registry.load(std.testing.allocator, @embedFile("p3_async_registry.json"));
    defer registry.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedP3WasiFilesystemMetadataHashComponent, MetadataHashPlan.analyze(tokens, registry));
}
