//! A private manifest-backed GC list<u32> lower probe.
//!
...
const descriptor_id = "demo:marshal-u32-equivalence/api.send@1.0.0/lower";
...
pub fn emit_u32_list_lower_module(
    io: std.Io,
...
test "manifest-backed u32 list probe emits a lower entry" {
    const wat = try emit_u32_list_lower_module(std.testing.io, std.testing.allocator, "..");
    ...
    try std.testing.expect(std.mem.indexOf(u8, wat, "(param $input (ref null $do_u32))") != null);
...
pub fn main(init: std.process.Init) !void {
    ...
    if (args.len != 2 and args.len != 3) return error.InvalidGcMarshalU32ProbeArgs;
    const repository_root = if (args.len == 3) args[2] else ".";
    const wat = try emit_u32_list_lower_module(init.io, init.gpa, repository_root);
