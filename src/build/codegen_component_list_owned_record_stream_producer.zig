...
const canonical_core_wat = @embedFile("list_owned_record_stream_producer_template.wat");

pub const ProducerError = error{UnsupportedP3ListOwnedRecordStreamProducer};

pub const ListOwnedRecordStreamProducerPlan = struct {
    ...
    list_layout: p3_async_manifest.RecordListLayout,
    ...
    pub fn analyze(
    ...
    ) ProducerError!ListOwnedRecordStreamProducerPlan {
        const descriptor = registry.find(
            "do:g6-2-owned-record-list-producer@0.1.0",
            "consume-via-stream",
        ) orelse return error.UnsupportedP3ListOwnedRecordStreamProducer;
        ...
        {
            .record_resource_list_owned_record_stream_producer => |value| value,
            else => return error.UnsupportedP3ListOwnedRecordStreamProducer,
        };
        ...
        if (count_host_bindings(tokens) != 2 or
            !std.mem.eql(u8, source.locator, "do:g6-2-owned-record-list-producer/source@0.1.0") or
        ...
        if (count_resource_decls(tokens) != 1 or
            ...
            !std.mem.eql(u8, ticket.path, "do:g6-2-owned-record-list-producer/source/ticket") or
            !resource_decl_is_exact(tokens, "Ticket", ticket.path))
            return error.UnsupportedP3ListOwnedRecordStreamProducer;

        if (count_record_decls(tokens, "ListEntry") != 1 or
            !find_record_decl(tokens, "ListEntry") or
        ...
        if (!shape_matches(shape)) return error.UnsupportedP3ListOwnedRecordStreamProducer;
        ...
        return .{
            ...
            .record_type_name = "ListEntry",
            .error_type_name = "ProducerError",
            .root_name = "produce",
            .mode_name = "mode",
            .layout = shape.record_layout,
            .list_layout = shape.record_list_layout,
...
pub fn emit_component_wat(
    ...
    plan: ListOwnedRecordStreamProducerPlan,
) ![]u8 {
...
pub fn emit_component_wat_for_tokens(
    ...
    const plan = try ListOwnedRecordStreamProducerPlan.analyze(tokens, registry);
...
pub fn emit_component_wit(
    ...
    try validate_internal_plan(plan);
    if (!std.mem.eql(u8, plan.descriptor.wit.world, "owned-record-list-producer")) {
        return error.UnsupportedP3ListOwnedRecordStreamProducer;
    }
...
fn validate_internal_plan(plan: ListOwnedRecordStreamProducerPlan) ProducerError!void {
    ...
    if (!std.mem.eql(u8, record.name, "list-entry") or record.byte_size != 12 or
        record.alignment != 4 or record.fields.len != 2 or
        !field_matches(record.fields[0], "values", "i32", 0) or
        !field_matches(record.fields[1], "ticket", "i32", 8) or
        !source_list_matches(record.source_fields[0]) or
        !source_ticket_matches(record.source_fields[1]))
        return error.UnsupportedP3ListOwnedRecordStreamProducer;

    if (plan.layout.byte_size != 12 or plan.layout.alignment != 4 or
        plan.layout.fields.len != 2 or
        !field_matches(plan.layout.fields[0], "values", "i32", 0) or
        !field_matches(plan.layout.fields[1], "ticket", "i32", 8) or
        !record_layout_sources_match(plan.layout))
        return error.UnsupportedP3ListOwnedRecordStreamProducer;
    if (plan.list_layout.pointer_offset != 0 or plan.list_layout.length_offset != 4 or
        plan.list_layout.element_stride != 4 or plan.list_layout.max_items != 3)
        return error.UnsupportedP3ListOwnedRecordStreamProducer;

    if (plan.contract.list_allocations.len != 1) return error.UnsupportedP3ListOwnedRecordStreamProducer;
    const allocation = plan.contract.list_allocations[0];
    if (allocation.path.len != 1 or !std.mem.eql(u8, allocation.path[0], "values") or
        !std.mem.eql(u8, allocation.element_core_type, "u32") or
        allocation.pointer_offset != 0 or allocation.length_offset != 4 or
        allocation.element_stride != 4 or allocation.max_items != 3 or
    ...
    if (leaf.path.len != 1 or !std.mem.eql(u8, leaf.path[0], "ticket") or
        !std.mem.eql(u8, leaf.resource, "ticket") or leaf.handle_offset != 8 or
        !std.mem.eql(u8, leaf.drop_import, "[resource-drop]ticket") or leaf.bit != 0)
        return error.UnsupportedP3ListOwnedRecordStreamProducer;
}

fn shape_matches(shape: p3_async_manifest.ListOwnedRecordStreamProducerShape) bool {
    const layout = shape.record_layout;
    return std.mem.eql(u8, shape.element, "list-entry") and
        std.mem.eql(u8, layout.name, "list-entry") and
        layout.byte_size == 12 and layout.alignment == 4 and layout.fields.len == 2 and
        layout.source_fields.len == 2 and
        field_matches(layout.fields[0], "values", "i32", 0) and
        field_matches(layout.fields[1], "ticket", "i32", 8) and
        record_layout_sources_match(layout) and
        shape.record_list_layout.pointer_offset == 0 and
        shape.record_list_layout.length_offset == 4 and
        shape.record_list_layout.element_stride == 4 and
        shape.record_list_layout.max_items == 3 and
        shape.producer.stream_capacity == 1 and
        std.mem.eql(u8, shape.producer.source_module, "do:g6-2-owned-record-list-producer/source@0.1.0") and
        ...
        stream_matches(shape.stream);
}

fn stream_matches(stream: p3_async_manifest.StreamCanonical) bool {
    return std.mem.eql(u8, stream.element, "list-entry") and
        named_operation_matches(stream.new, "[stream-new-0]consume-via-stream", &.{}, &.{"i64"}) and
        named_operation_matches(stream.cancel_read, "[stream-cancel-read-0]consume-via-stream", &.{"i32"}, &.{"i32"}) and
        named_operation_matches(stream.cancel_write, "[stream-cancel-write-0]consume-via-stream", &.{"i32"}, &.{"i32"}) and
        named_operation_matches(stream.drop_readable, "[stream-drop-readable-0]consume-via-stream", &.{"i32"}, &.{}) and
        named_operation_matches(stream.drop_writable, "[stream-drop-writable-0]consume-via-stream", &.{"i32"}, &.{}) and
        named_operation_matches(stream.read, "[async-lower][stream-read-0]consume-via-stream", &.{"i32", "i32", "i32"}, &.{"i32"}) and
        named_operation_matches(stream.write, "[async-lower][stream-write-0]consume-via-stream", &.{"i32", "i32", "i32"}, &.{"i32"});
}
...
fn record_layout_sources_match(layout: p3_async_manifest.RecordLayout) bool {
    return source_list_matches(layout.source_fields[0]) and source_ticket_matches(layout.source_fields[1]);
}

fn source_list_matches(field: p3_async_manifest.RecordSourceField) bool {
    return std.mem.eql(u8, field.name, "values") and
        std.mem.eql(u8, field.source_type, "list<u32>") and
        field.storage.len == 1 and std.mem.eql(u8, field.storage[0], "values") and
        field.ownership == .none and field.resource == null and field.drop_import == null and
        field.nested_fields.len == 0;
}

fn source_ticket_matches(field: p3_async_manifest.RecordSourceField) bool {
...
fn find_host_binding(tokens: []const lexer.Token, wanted: BindingKind) ?HostBinding {
    ...
    while (idx + 10 < tokens.len) : (idx += 1) {
        ...
        const kind: BindingKind = if (tok_eq(tokens[idx + 3], "host_func")) .source else if
            (tok_eq(tokens[idx + 3], "host_async_func")) .sink else continue;
...
fn count_host_bindings(tokens: []const lexer.Token) usize {
    ...
    for (tokens, 0..) |token, idx| {
        if (token.kind == .ident and idx + 3 < tokens.len and tok_eq(tokens[idx + 1], "=") and
...
fn count_resource_decls(tokens: []const lexer.Token) usize {
    var count: usize = 0;
    for (tokens, 0..) |token, idx| {
        if (token.kind == .ident and idx + 5 < tokens.len and tok_eq(tokens[idx + 1], "=") and
            tok_eq(tokens[idx + 2], "@") and tok_eq(tokens[idx + 3], "wasi_resource") and
            tok_eq(tokens[idx + 4], "(") and tokens[idx + 5].kind == .string) count += 1;
    }
    return count;
}
...
fn sink_signature_is_exact(tokens: []const lexer.Token, open: usize) bool {
    ...
    return close == open + 5 and tok_eq(tokens[open + 1], "StreamWriter") and
        tok_eq(tokens[open + 2], "<") and tok_eq(tokens[open + 3], "ListEntry") and
...
fn find_record_decl(tokens: []const lexer.Token, name: []const u8) bool {
    ...
    while (idx + 9 < tokens.len) : (idx += 1) {
        if (!tok_eq(tokens[idx], name) or !tok_eq(tokens[idx + 1], "{") or
            !tok_eq(tokens[idx + 2], ".values") or !tok_eq(tokens[idx + 3], "[") or
            !tok_eq(tokens[idx + 4], "u32") or !tok_eq(tokens[idx + 5], "]") or
            !tok_eq(tokens[idx + 6], ".ticket") or !tok_eq(tokens[idx + 7], "Ticket") or
