...
pub fn validate_host_import_build_uses(tokens: []const lexer.Token, host_imports: []const HostImport) !void {
    ...
    while (i + 1 < tokens.len) : (i += 1) {
        ...
        if (is_bare_host_call_statement(tokens, i, close_paren) and host_import.result != null) return error.NoMatchingCall;
        if (is_typed_binding_rhs_call(tokens, i) and host_import.result == null) return error.NoMatchingCall;
        i = close_paren;
    }
}
...
pub fn validate_reachable_wasi_host_import_stack(
    ...
    while (stack.items.len != 0) {
        ...
        defer {
            free_wasi_host_imports(allocator, module_wasi_imports.items);
            module_wasi_imports.deinit(allocator);
        }
        try collect_wasi_host_imports(allocator, module.tokens, module.path, &module_wasi_imports);
        if (find_wasi_host_import(module_wasi_imports.items, visit.name)) |import| {
            if (visit.call_idx) |call_idx| {
                if (wasi_host_import_use_is_lowerable_at_call(module.tokens, call_idx, import)) continue;
            }
...
pub fn find_wasi_host_import_for_tokens(ctx: CodegenContext, tokens: []const lexer.Token, alias: []const u8) ?WasiHostImport {
    ...
    return find_wasi_host_import_by_source(ctx.wasi_imports, source, alias);
}
...
pub fn push_reach_visit(
    ...
    if (is_core_wasm_call_name(visit.name)) return;
...
pub fn collect_function_body_calls(
    ...
    while (i + 1 < tokens.len) : (i += 1) {
        ...
        if (depth_brace != 0) continue;
        if (tokens[i].kind != .ident) continue;
        if (!std.mem.eql(u8, public_decl_name(tokens[i].lexeme), func_name)) continue;
...
pub fn is_value_enum_decl_start(tokens: []const lexer.Token, idx: usize) bool {
    return idx + 2 < tokens.len and
        ...
        is_public_type_name(public_decl_name(tokens[idx].lexeme)) and
        !is_error_type_name(public_decl_name(tokens[idx].lexeme)) and
...
pub fn is_payload_enum_decl_start(tokens: []const lexer.Token, idx: usize) bool {
    ...
    if (!is_public_type_name(public_decl_name(tokens[idx].lexeme))) return false;
    if (is_error_type_name(public_decl_name(tokens[idx].lexeme))) return false;
    ...
    while (j < line_end) {
        ...
        if (!is_public_type_name(public_decl_name(tokens[j].lexeme))) return false;
        j += 1;
...
pub fn find_payload_enum_decl_line_by_name(tokens: []const lexer.Token, name: []const u8) ?usize {
    ...
    while (i + 2 < tokens.len) : (i += 1) {
        ...
        if (std.mem.eql(u8, public_decl_name(tokens[i].lexeme), name)) return i;
        i = find_line_end(tokens, i) - 1;
    }
    return null;
}
...
pub fn value_enum_line_has_branch(tokens: []const lexer.Token, enum_idx: usize, branch_name: []const u8) bool {
    ...
    while (j + 3 < line_end) {
        ...
        if (tokens[j].kind == .ident and std.mem.eql(u8, public_decl_name(tokens[j].lexeme), branch_name)) return true;
        j += 4;
    }
    return false;
}
...
pub fn collect_string_data_for_wasi_host_calls(
    ...
    while (i + 1 < tokens.len) : (i += 1) {
        if (tokens[i].kind != .ident) continue;
        const import = find_wasi_host_import_by_source(wasi_imports, source, tokens[i].lexeme) orelse continue;
        const lowering = wasi_lowering(import) orelse continue;
        if (!lowering.result_link_at_error) continue;
        if (!tok_eq(tokens[i + 1], "(")) continue;

        const close_paren = find_matching(tokens, i + 1, "(", ")") catch return error.InvalidCallArgList;
        const args = parse_wasi_link_at_args(tokens, i + 2, close_paren) orelse return error.NoMatchingCall;
        if (string_literal_arg_lexeme(tokens, args.old_path_start, args.old_path_end)) |old_path| {
        ...
        if (string_literal_arg_lexeme(tokens, args.new_path_start, args.new_path_end)) |new_path| {
...
pub fn call_head_at(tokens: []const lexer.Token, idx: usize, limit: usize) ?ExprCallHead {
    ...
    if (is_intrinsic and !is_core_wasm_call_name(tokens[name_idx].lexeme)) return null;
    return .{
        .name_idx = name_idx,
