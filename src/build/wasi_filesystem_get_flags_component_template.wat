(module
  ;; Measured from wasm-tools 1.255.0 and reproduced by 1.254.0.
  ...
  (import "[export]wasi:filesystem/probe@0.3.0-rc-2025-09-16"
    "[task-return]run" (func $task-return-run (type $root-join)))

  (memory $memory 1)
  (global $frame-next (mut i32) (i32.const 1024))
  (global $heap-next (mut i32) (i32.const 65536))
  ;; Frame: waitable @0, descriptor @4, result tag @8, flags/error @9,
  ;; subtask @16. Six flags use a one-byte canonical memory payload; the
  ;; task-return flat ABI promotes that payload to i32.
  ;; [get-flags-ready] immediate status=2 carries the result in the frame.
  ;; [get-flags-pending] non-terminal status joins the waitable set.
  ;; [get-flags-error] callback status stores the error-code payload.
  ;; [get-flags-cancel] cancellation is represented by subtask cleanup.
  ...
  (func $wait-on-get-flags (param $frame i32) (result i32)
    local.get $frame
    i32.const 16
    i32.add
    i32.load
  ...
  (func $start-get-flags (param $frame i32) (result i32)
    (local $code i32)
    local.get $frame
    i32.const 4
    i32.add
