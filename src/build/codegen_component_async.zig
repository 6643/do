...
const codegen_component_wasi_filesystem_open_at = @import("codegen_component_wasi_filesystem_open_at.zig");
...
pub fn emit_component_wat(
    ...
    return switch (try target_for_tokens_with_graph(allocator, tokens, module_graph)) {
        ...
        .wasi_filesystem_open_at => codegen_component_wasi_filesystem_open_at.emit_component_wat(allocator, program, tokens, module_graph) catch |err| switch (err) {
            error.UnsupportedP3WasiFilesystemOpenAtComponent => error.UnsupportedP3AsyncComponent,
            else => err,
        },
...
pub fn emit_component_wit_with_graph(
    ...
    return switch (try target_for_tokens_with_graph(allocator, tokens, module_graph)) {
        ...
        .record_resource_list_owned_record_stream_producer => codegen_component_list_owned_record_stream_producer.emit_component_wit_for_tokens(allocator, tokens) catch |err| switch (err) {
            error.UnsupportedP3ListOwnedRecordStreamProducer => error.UnsupportedP3AsyncComponent,
            else => err,
        },
        ...
        .wasi_filesystem_metadata_hash_at => codegen_component_wasi_filesystem_metadata_hash_at.emit_component_wit(allocator, tokens) catch |err| switch (err) {
            error.UnsupportedP3WasiFilesystemMetadataHashAtComponent => error.UnsupportedP3AsyncComponent,
            else => err,
        },
        .wasi_filesystem_open_at => codegen_component_wasi_filesystem_open_at.emit_component_wit(allocator, tokens) catch |err| switch (err) {
            error.UnsupportedP3WasiFilesystemOpenAtComponent => error.UnsupportedP3AsyncComponent,
            else => err,
        },
...
pub fn target_for_tokens_with_graph(
    ...
    if (codegen_component_list_owned_record_stream_producer.ListOwnedRecordStreamProducerPlan.analyze(tokens, registry)) |_| {
    ...
    if (codegen_component_owned_record_stream_producer.OwnedRecordStreamProducerPlan.analyze(tokens, registry)) |_| {
    ...
    while (idx + 8 < tokens.len) : (idx += 1) {
        ...
        const next: Target = switch (shape) {
            ...
            .record_resource_list_stream_reader => if (binding.kind == .host_func and list_resource_stream_signature_at(tokens, idx))
                ...
                return error.UnsupportedP3AsyncComponent,
            .owned_record_stream_producer => if (binding.kind == .host_async_func) blk: {
            ...
            .record_resource_triple_stream_producer => if (binding.kind == .host_async_func) blk: {
                _ = codegen_component_owned_record_triple_stream_producer.analyze(tokens, registry) catch
                    return error.UnsupportedP3AsyncComponent;
                break :blk .owned_record_triple_stream_producer;
            } else return error.UnsupportedP3AsyncComponent,
            ...
            .record_resource_list_owned_record_stream_producer => if (binding.kind == .host_async_func) blk: {
            ...
            .filesystem_metadata_hash_at => if (binding.kind == .host_async_func) blk: {
                _ = codegen_component_wasi_filesystem_metadata_hash_at.MetadataHashAtPlan.analyze(tokens, registry) catch
                    return error.UnsupportedP3AsyncComponent;
                break :blk .wasi_filesystem_metadata_hash_at;
            } else return error.UnsupportedP3AsyncComponent,
            .filesystem_open_at => if (binding.kind == .host_async_func) blk: {
                _ = codegen_component_wasi_filesystem_open_at.FilesystemOpenAtPlan.analyze(tokens, registry) catch
                    return error.UnsupportedP3AsyncComponent;
                break :blk .wasi_filesystem_open_at;
            } else return error.UnsupportedP3AsyncComponent,
...
fn target_for_descriptor(descriptor: p3_async_manifest.Descriptor) !Target {
    ...
    return switch (shape) {
        ...
        .async_map_u32_u32 => error.UnsupportedP3AsyncComponent,
        ...
        .filesystem_open_at => .wasi_filesystem_open_at,
...
test "generic Component async target classifies pinned filesystem descriptor metadata-hash-at" {
    const source = @embedFile("test/compile_ok/520_wasi_filesystem_metadata_hash_at_component.do");
    const tokens = try lexer.tokenize(std.testing.allocator, source);
    defer std.testing.allocator.free(tokens);

    try std.testing.expectEqual(Target.wasi_filesystem_metadata_hash_at, try target_for_tokens(std.testing.allocator, tokens));
    const wit = try emit_component_wit(std.testing.allocator, tokens);
    defer std.testing.allocator.free(wit);
...
test "filesystem descriptor metadata-hash-at target rejects non-linear and async-root sources" {
    const cases = [_][]const u8{
        @embedFile("test/compile_err/524_wasi_filesystem_metadata_hash_at_wrong_result.do"),
...
test "generic Component async target classifies pinned filesystem descriptor open-at" {
    const source = @embedFile("test/compile_ok/540_wasi_filesystem_open_at_component.do");
    const tokens = try lexer.tokenize(std.testing.allocator, source);
    defer std.testing.allocator.free(tokens);

    try std.testing.expectEqual(Target.wasi_filesystem_open_at, try target_for_tokens(std.testing.allocator, tokens));
    const wit = try emit_component_wit(std.testing.allocator, tokens);
    defer std.testing.allocator.free(wit);
...
test "generic Component async target classifies the list-owned-record producer" {
...
test "generic Component async target classifies the direct owned-record producer" {
    const source =
...
test "generic Component async target classifies the direct owned-record pair producer" {
    const source =
    ...
    defer std.testing.allocator.free(tokens);
    try std.testing.expectEqual(
        Target.owned_record_pair_stream_producer,
        try target_for_tokens(std.testing.allocator, tokens),
    );
}

test "generic Component async direct owned-record pair dispatch emits pinned markers" {
    const source =
        \\make_ticket = @host_func("do:g6-2-owned-record-pair-producer/source@0.1.0", "make-ticket", (u32) -> Ticket)
        \\consume = @host_async_func("do:g6-2-owned-record-pair-producer@0.1.0", "consume-via-stream", (StreamWriter<ResourcePair>) -> Result<nil, ProducerError>)
        \\Ticket = @wasi_resource("do:g6-2-owned-record-pair-producer/source/ticket", { .id i64 })
...
test "generic Component async target classifies the direct owned-record triple producer" {
    const source =
...
test "generic Component async direct owned-record triple dispatch emits pinned contracts" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, wit, "middle: own<ticket>") != null);
}
