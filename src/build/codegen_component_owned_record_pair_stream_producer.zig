...
const canonical_core_wat = @embedFile("owned_record_pair_stream_producer_template.wat");

pub const ProducerError = error{UnsupportedP3OwnedRecordPairStreamProducer};

/// The pair producer is a deliberately closed compiler slice.  Its analyzer
/// accepts only the pinned descriptor and the exact two-owned-field source
/// shape; it is not a generic record/resource lowering entry point.
pub const OwnedRecordPairStreamProducerPlan = struct {
    ...
    pub fn analyze(
    ...
    ) ProducerError!OwnedRecordPairStreamProducerPlan {
        const descriptor = registry.find(
            "do:g6-2-owned-record-pair-producer@0.1.0",
            "consume-via-stream",
        ) orelse return error.UnsupportedP3OwnedRecordPairStreamProducer;
        ...
        if (count_host_bindings(tokens) != 2 or
            ...
            !std.mem.eql(u8, source.locator, "do:g6-2-owned-record-pair-producer/source@0.1.0") or
        ...
        if (count_resource_decls(tokens) != 1 or
            ...
            !std.mem.eql(u8, ticket.path, "do:g6-2-owned-record-pair-producer/source/ticket") or
        ...
        if (!std.mem.eql(u8, shape.element, "resource-pair") or
            ...
            !std.mem.eql(u8, shape.producer.source_module, "do:g6-2-owned-record-pair-producer/source@0.1.0") or
...
pub fn emit_component_wat(allocator: std.mem.Allocator, plan: OwnedRecordPairStreamProducerPlan) ![]u8 {
...
pub fn emit_component_wat_for_tokens(allocator: std.mem.Allocator, tokens: []const lexer.Token) ![]u8 {
    ...
    const plan = try OwnedRecordPairStreamProducerPlan.analyze(tokens, registry);
    return emit_component_wat(allocator, plan);
}

pub fn emit_component_wit(allocator: std.mem.Allocator, plan: OwnedRecordPairStreamProducerPlan) ![]u8 {
    try validate_internal_plans(allocator, plan);
    if (!std.mem.eql(u8, plan.descriptor.wit.world, "owned-record-pair-producer"))
        return error.UnsupportedP3OwnedRecordPairStreamProducer;
...
fn validate_internal_plans(allocator: std.mem.Allocator, plan: OwnedRecordPairStreamProducerPlan) ProducerError!void {
    ...
    }) catch return error.UnsupportedP3OwnedRecordPairStreamProducer;
    defer pair.deinit();
...
test "owned record pair producer plan and emitter accept only the pinned shape" {
...
test "owned record pair producer rejects a single-field or borrowed record" {
    var registry = try p3_async_manifest.Registry.load(std.testing.allocator, @embedFile("p3_async_registry.json"));
    defer registry.deinit(std.testing.allocator);

    const single =
        \\make_ticket = @host_func("do:g6-2-owned-record-pair-producer/source@0.1.0", "make-ticket", (u32) -> Ticket)
        \\consume = @host_async_func("do:g6-2-owned-record-pair-producer@0.1.0", "consume-via-stream", (StreamWriter<ResourceEntry>) -> Result<nil, ProducerError>)
        \\Ticket = @wasi_resource("do:g6-2-owned-record-pair-producer/source/ticket", { .id i64 })
    ...
    for ([_][]const u8{ single, borrowed }) |source| {
        ...
        try std.testing.expectError(
            error.UnsupportedP3OwnedRecordPairStreamProducer,
            OwnedRecordPairStreamProducerPlan.analyze(tokens, registry),
        );
    }
}
