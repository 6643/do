...
pub const ListOwnedRecordStreamProducerShape = struct {
    element: []const u8,
    stream_index: usize,
    method: StreamOperation,
    stream: StreamCanonical,
...
/// ABI facts for the private direct `stream<resource-entry>` producer. This
...
/// ABI facts for the private direct `stream<resource-pair>` producer. This is
...
/// ABI facts for the private fixed one-level nested `stream<outer>` producer.
/// The canonical slot is flattened, but the source ownership path remains
/// `outer.inner.ticket` and is validated separately from the core layout.
pub const OwnedRecordNestedStreamProducerShape = struct {
    element: []const u8,
    stream_index: usize,
    method: StreamOperation,
    stream: StreamCanonical,
...
pub const RecordListLayout = struct {
    pointer_offset: u32,
    length_offset: u32,
    element_stride: u32,
    max_items: u32,
...
/// The private `descriptor.stat-at` slice combines the measured path lowering
/// with the canonical descriptor-stat record returned by `descriptor.stat`.
/// Keep it separate from both descriptor-only stat and metadata-hash-at so a
/// method-specific emitter cannot silently consume the wrong call ABI.
pub const FilesystemStatAtShape = struct {
    receiver: []const u8,
    path_flags: []const u8,
    path: []const u8,
    source_result: []const u8,
...
pub fn lowering_shape(descriptor: Descriptor) ?LoweringShape {
    ...
    if (std.mem.eql(u8, descriptor.effect, "record-resource-nested-stream-producer")) {
        if (valid_owned_record_nested_stream_producer_descriptor(descriptor)) |shape| return .{ .record_resource_nested_stream_producer = shape };
        return null;
    }
    ...
    if (valid_filesystem_stat_at_descriptor(descriptor)) |shape| {
        return .{ .filesystem_stat_at = shape };
    }
...
fn valid_filesystem_stat_at_descriptor(descriptor: Descriptor) ?FilesystemStatAtShape {
    const record_layout = descriptor.canonical.record_layout orelse return null;
    if (!std.mem.eql(u8, descriptor.locator, "wasi:filesystem/types@0.3.0-rc-2025-09-16") or
        !std.mem.eql(u8, descriptor.member, "descriptor.stat-at") or
        !std.mem.eql(u8, descriptor.effect, "async") or
        descriptor.params.len != 3 or
        !std.mem.eql(u8, descriptor.params[0], "descriptor") or
        !std.mem.eql(u8, descriptor.params[1], "path-flags") or
        ...
        descriptor.canonical.result_area_payload != null or
        !std.mem.eql(u8, descriptor.canonical.async_import_module, "wasi:filesystem/types@0.3.0-rc-2025-09-16") or
        !std.mem.eql(u8, descriptor.canonical.async_import_name, "[async-lower][method]descriptor.stat-at") or
        !std.mem.eql(u8, descriptor.wit.package, "wasi:filesystem@0.3.0-rc-2025-09-16") or
        !std.mem.eql(u8, descriptor.wit.interface, "types") or
        !std.mem.eql(u8, descriptor.wit.operation, "descriptor.stat-at") or
        !std.mem.eql(u8, descriptor.wit.world, "imports") or
        descriptor.wit.parameter.len != 0 or
...
pub const Canonical = struct {
    ...
    record_list_layout: ?RecordListLayout = null,
...
fn parse_canonical(allocator: std.mem.Allocator, value: ?std.json.Value) !Canonical {
    ...
    const list_resource_layout = if (canonical.get("list_resource_layout")) |layout_value|
        ...
        null;
    const record_list_layout = if (canonical.get("record_list_layout")) |layout_value|
        try parse_record_list_layout(layout_value)
    else
        null;
...
fn parse_record_list_layout(value: std.json.Value) !RecordListLayout {
    const object = object_value(value) orelse return error.InvalidP3AsyncManifest;
    if (!object_has_only_fields(object, &.{ "pointer_offset", "length_offset", "element_stride", "max_items" })) {
        return error.InvalidP3AsyncManifest;
    }
    const pointer_offset = unsigned_value(object.get("pointer_offset")) orelse return error.InvalidP3AsyncManifest;
    const length_offset = unsigned_value(object.get("length_offset")) orelse return error.InvalidP3AsyncManifest;
    const element_stride = unsigned_value(object.get("element_stride")) orelse return error.InvalidP3AsyncManifest;
    const max_items = unsigned_value(object.get("max_items")) orelse return error.InvalidP3AsyncManifest;
    if (pointer_offset > std.math.maxInt(u32) or
        length_offset > std.math.maxInt(u32) or
    ...
    const pointer: u32 = @intCast(pointer_offset);
    const length: u32 = @intCast(length_offset);
    if (pointer % 4 != 0 or length % 4 != 0 or
        element_stride == 0 or max_items == 0 or
        (@as(u64, pointer) < @as(u64, length) + 4 and
            @as(u64, length) < @as(u64, pointer) + 4)) return error.InvalidP3AsyncManifest;
    return .{
...
fn valid_owned_record_stream_producer_descriptor(descriptor: Descriptor) ?OwnedRecordStreamProducerShape {
    const stream = descriptor.canonical.stream orelse return null;
    const record_layout = descriptor.canonical.record_layout orelse return null;
    const producer = descriptor.canonical.producer orelse return null;

    if (!std.mem.eql(u8, descriptor.effect, "record-resource-stream-producer") or
        !std.mem.eql(u8, descriptor.locator, "do:g6-2-owned-record-producer@0.1.0") or
        !std.mem.eql(u8, descriptor.member, "consume-via-stream") or
        descriptor.params.len != 1 or
        !std.mem.eql(u8, descriptor.params[0], "stream<resource-entry>") or
        descriptor.resource != null or
        !std.mem.eql(u8, descriptor.result, "Result<nil,error-code>") or
        descriptor.wit_sha256 == null or
        !std.mem.eql(u8, descriptor.wit_sha256.?, "6c1406962ee4c4e3eec5b3b4a866acfd1d8eb6ee159ce5b4077df113063d1ace") or
        !std.mem.eql(u8, descriptor.wit.package, "do:g6-2-owned-record-producer@0.1.0") or
        !std.mem.eql(u8, descriptor.wit.interface, "sink") or
        !std.mem.eql(u8, descriptor.wit.operation, "consume-via-stream") or
        !std.mem.eql(u8, descriptor.wit.world, "owned-record-producer") or
        !std.mem.eql(u8, descriptor.wit.parameter, "data") or
        !equal_core_types(descriptor.canonical.core_params, &.{ "i32", "i32" }) or
        !equal_core_types(descriptor.canonical.core_results, &.{"i32"}) or
        !equal_core_types(descriptor.canonical.completion_params, &.{ "i32", "i32" }) or
        !std.mem.eql(u8, descriptor.canonical.completion, "task-return") or
        !std.mem.eql(u8, descriptor.canonical.async_import_module, "do:g6-2-owned-record-producer/sink@0.1.0") or
        ...
        !valid_single_ticket_record_layout(record_layout) or
        !std.mem.eql(u8, stream.element, "resource-entry") or
        !std.mem.eql(u8, producer.source_module, "do:g6-2-owned-record-producer/source@0.1.0") or
...
fn valid_owned_record_pair_stream_producer_descriptor(descriptor: Descriptor) ?OwnedRecordPairStreamProducerShape {
    ...
    if (!std.mem.eql(u8, descriptor.effect, "record-resource-pair-stream-producer") or
        !std.mem.eql(u8, descriptor.locator, "do:g6-2-owned-record-pair-producer@0.1.0") or
        !std.mem.eql(u8, descriptor.member, "consume-via-stream") or
        descriptor.params.len != 1 or
        ...
        !std.mem.eql(u8, descriptor.result, "Result<nil,error-code>") or
        descriptor.wit_sha256 == null or
        !std.mem.eql(u8, descriptor.wit_sha256.?, "89345a5213936735d7f065cd54ed42b83d159b80305a1a900ae00df2e811704d") or
        !std.mem.eql(u8, descriptor.wit.package, "do:g6-2-owned-record-pair-producer@0.1.0") or
        !std.mem.eql(u8, descriptor.wit.interface, "sink") or
        !std.mem.eql(u8, descriptor.wit.operation, "consume-via-stream") or
        !std.mem.eql(u8, descriptor.wit.world, "owned-record-pair-producer") or
        !std.mem.eql(u8, descriptor.wit.parameter, "data") or
        !equal_core_types(descriptor.canonical.core_params, &.{ "i32", "i32" }) or
        !equal_core_types(descriptor.canonical.core_results, &.{ "i32" }) or
        !equal_core_types(descriptor.canonical.completion_params, &.{ "i32", "i32" }) or
        !std.mem.eql(u8, descriptor.canonical.completion, "task-return") or
        !std.mem.eql(u8, descriptor.canonical.async_import_module, "do:g6-2-owned-record-pair-producer/sink@0.1.0") or
        !std.mem.eql(u8, descriptor.canonical.async_import_name, "[async-lower]consume-via-stream") or
        descriptor.canonical.result_payload != null or
        descriptor.canonical.result_area_payload != null or
        descriptor.canonical.future_owned != null or
        ...
        !std.mem.eql(u8, producer.source_module, "do:g6-2-owned-record-pair-producer/source@0.1.0") or
        !std.mem.eql(u8, producer.source_import_name, "make-ticket") or
        !equal_core_types(producer.source_core_params, &.{ "i32" }) or
        !equal_core_types(producer.source_core_results, &.{ "i32" }) or
        !std.mem.eql(u8, producer.resource_drop_import, "[resource-drop]ticket") or
...
fn valid_owned_record_nested_stream_producer_descriptor(descriptor: Descriptor) ?OwnedRecordNestedStreamProducerShape {
    const stream = descriptor.canonical.stream orelse return null;
    const record_layout = descriptor.canonical.record_layout orelse return null;
    const producer = descriptor.canonical.producer orelse return null;
    if (!std.mem.eql(u8, descriptor.locator, "do:g6-2-owned-record-nested-producer@0.1.0") or
        !std.mem.eql(u8, descriptor.member, "consume-via-stream") or
        !std.mem.eql(u8, descriptor.effect, "record-resource-nested-stream-producer") or
        descriptor.params.len != 1 or !std.mem.eql(u8, descriptor.params[0], "stream<outer>") or
        descriptor.resource != null or !std.mem.eql(u8, descriptor.result, "Result<nil,error-code>") or
        descriptor.wit_sha256 == null or !std.mem.eql(u8, descriptor.wit_sha256.?, "9662440709b01044544d4c4350f3e6f783a8f06aaa5c884a96a1e43a935a7543") or
        !std.mem.eql(u8, descriptor.wit.package, "do:g6-2-owned-record-nested-producer@0.1.0") or
        !std.mem.eql(u8, descriptor.wit.interface, "sink") or !std.mem.eql(u8, descriptor.wit.operation, "consume-via-stream") or
        !std.mem.eql(u8, descriptor.wit.world, "owned-record-nested-producer") or !std.mem.eql(u8, descriptor.wit.parameter, "data") or
        !equal_core_types(descriptor.canonical.core_params, &.{ "i32", "i32" }) or
        !equal_core_types(descriptor.canonical.core_results, &.{ "i32" }) or
        !equal_core_types(descriptor.canonical.completion_params, &.{ "i32", "i32" }) or
        !std.mem.eql(u8, descriptor.canonical.completion, "task-return") or
        !std.mem.eql(u8, descriptor.canonical.async_import_module, "do:g6-2-owned-record-nested-producer/sink@0.1.0") or
        !std.mem.eql(u8, descriptor.canonical.async_import_name, "[async-lower]consume-via-stream") or
        descriptor.canonical.result_payload != null or descriptor.canonical.result_area_payload != null or
        descriptor.canonical.future_owned != null or descriptor.canonical.error_variants.len != 0 or
        descriptor.canonical.list_resource_layout != null or descriptor.canonical.scalar_list_layout != null or
        descriptor.canonical.scalar_list_producer != null or descriptor.canonical.future_input != null or
        descriptor.canonical.future != null or descriptor.canonical.parameterized_owned_record_pair_producer != null or
        descriptor.canonical.variant_stream != null or descriptor.canonical.variant_future != null or
        descriptor.canonical.event_layout != null or descriptor.canonical.ticket_drop_import != null or
        !valid_owned_nested_ticket_record_layout(record_layout) or !std.mem.eql(u8, stream.element, "outer") or
        !std.mem.eql(u8, producer.source_module, "do:g6-2-owned-record-nested-producer/source@0.1.0") or
        !std.mem.eql(u8, producer.source_import_name, "make-ticket") or
        !equal_core_types(producer.source_core_params, &.{ "i32" }) or
        !equal_core_types(producer.source_core_results, &.{ "i32" }) or
        !std.mem.eql(u8, producer.resource_drop_import, "[resource-drop]ticket") or
        producer.stream_capacity != 1 or !std.mem.eql(u8, producer.terminal, "task-return") or
        producer.runtime_count_param != null or producer.runtime_max != null or producer.runtime_mode_param == null or
        !std.mem.eql(u8, producer.runtime_mode_param.?, "u32") or producer.batch_count != null or producer.batch_lengths != null) return null;
    if (!valid_named_stream_operation(stream.new, "[stream-new-0]consume-via-stream", &.{}, &.{ "i64" }) or
        !valid_named_stream_operation(stream.cancel_read, "[stream-cancel-read-0]consume-via-stream", &.{ "i32" }, &.{ "i32" }) or
        !valid_named_stream_operation(stream.cancel_write, "[stream-cancel-write-0]consume-via-stream", &.{ "i32" }, &.{ "i32" }) or
        !valid_named_stream_operation(stream.drop_readable, "[stream-drop-readable-0]consume-via-stream", &.{ "i32" }, &.{}) or
...
fn valid_record_resource_list_owned_record_stream_producer_descriptor(descriptor: Descriptor) ?ListOwnedRecordStreamProducerShape {
    const stream = descriptor.canonical.stream orelse return null;
    const record_layout = descriptor.canonical.record_layout orelse return null;
    const record_list_layout = descriptor.canonical.record_list_layout orelse return null;
    const producer = descriptor.canonical.producer orelse return null;
    ...
    if (!std.mem.eql(u8, descriptor.effect, "record-resource-list-owned-record-stream-producer") or
        ...
        !std.mem.eql(u8, descriptor.member, "consume-via-stream") or
        descriptor.params.len != 1 or
        !std.mem.eql(u8, descriptor.params[0], "stream<list-entry>") or
        descriptor.resource != null or
        !std.mem.eql(u8, descriptor.result, "Result<nil,error-code>") or
        descriptor.wit_sha256 == null or
        ...
        descriptor.canonical.event_layout != null or
        descriptor.canonical.ticket_drop_import != null or
        !valid_list_owned_record_layout(record_layout) or
...
fn valid_record_list_layout(layout: RecordListLayout) bool {
...
fn valid_list_owned_record_layout(layout: RecordLayout) bool {
    if (!std.mem.eql(u8, layout.name, "list-entry") or
        ...
        !record_field_matches(layout.fields[0], "values", "i32", 0) or
        !record_field_matches(layout.fields[1], "ticket", "i32", 8)) return false;
    ...
    return std.mem.eql(u8, values.name, "values") and
        ...
        std.mem.eql(u8, ticket.source_type, "ticket") and
        ticket.storage.len == 1 and
        std.mem.eql(u8, ticket.storage[0], "ticket") and
        ticket.ownership == .own and
        ticket.resource != null and
        std.mem.eql(u8, ticket.resource.?, "ticket") and
...
fn record_field_matches(field: RecordField, name: []const u8, core_type: []const u8, offset: u32) bool {
    return std.mem.eql(u8, field.name, name) and
        std.mem.eql(u8, field.core_type, core_type) and
        field.offset == offset;
}
...
fn valid_pair_ticket_record_layout(layout: RecordLayout) bool {
    ...
    const left = layout.fields[0];
    const right = layout.fields[1];
    const left_source = layout.source_fields[0];
    const right_source = layout.source_fields[1];
    return std.mem.eql(u8, left.name, "left") and
        std.mem.eql(u8, left.core_type, "i32") and
        left.offset == 0 and
        std.mem.eql(u8, right.name, "right") and
        std.mem.eql(u8, right.core_type, "i32") and
        right.offset == 4 and
...
fn valid_owned_nested_ticket_record_layout(layout: RecordLayout) bool {
    ...
    return std.mem.eql(u8, leaf.name, "ticket") and
        std.mem.eql(u8, leaf.source_type, "ticket") and
        leaf.storage.len == 1 and std.mem.eql(u8, leaf.storage[0], "ticket") and
        leaf.ownership == .own and leaf.resource != null and
        std.mem.eql(u8, leaf.resource.?, "ticket") and leaf.drop_import != null and
        std.mem.eql(u8, leaf.drop_import.?, "[resource-drop]ticket") and
        leaf.nested_fields.len == 0;
}
...
fn valid_owned_ticket_source_field(field: RecordSourceField, name: []const u8) bool {
    return std.mem.eql(u8, field.name, name) and
        ...
        field.storage.len == 1 and
        std.mem.eql(u8, field.storage[0], name) and
        field.ownership == .own and
        field.resource != null and
        std.mem.eql(u8, field.resource.?, "ticket") and
        field.drop_import != null and
...
fn is_record_source_type(value: []const u8) bool {
    return std.mem.eql(u8, value, "string") or
        std.mem.eql(u8, value, "list<u32>") or
...
test "checked-in registry admits the list-owned-record producer shape" {
    var registry = try Registry.load(std.testing.allocator, @embedFile("p3_async_registry.json"));
    defer registry.deinit(std.testing.allocator);
    ...
    try std.testing.expectEqualStrings("record-resource-list-owned-record-stream-producer", descriptor.effect);
    try std.testing.expectEqualStrings("stream<list-entry>", descriptor.params[0]);
    try std.testing.expectEqualStrings(
    ...
    switch (lowering_shape(descriptor) orelse return error.TestUnexpectedResult) {
        .record_resource_list_owned_record_stream_producer => |shape| {
            ...
            try std.testing.expectEqual(@as(u32, 0), shape.record_list_layout.pointer_offset);
            try std.testing.expectEqual(@as(u32, 4), shape.record_list_layout.length_offset);
            try std.testing.expectEqual(@as(u32, 4), shape.record_list_layout.element_stride);
            try std.testing.expectEqual(@as(u32, 3), shape.record_list_layout.max_items);
...
test "list-owned-record producer descriptor rejects measured drift" {
    var registry = try Registry.load(std.testing.allocator, @embedFile("p3_async_registry.json"));
    defer registry.deinit(std.testing.allocator);
    ...
    var wrong_package = original;
    wrong_package.wit.package = "do:other@0.1.0";
    try std.testing.expect(lowering_shape(wrong_package) == null);

    var wrong_world = original;
    wrong_world.wit.world = "other-world";
    try std.testing.expect(lowering_shape(wrong_world) == null);

    var wrong_element = original;
    wrong_element.canonical.stream.?.element = "list<resource-entry>";
    try std.testing.expect(lowering_shape(wrong_element) == null);

    var wrong_byte_size = original;
    wrong_byte_size.canonical.record_layout.?.byte_size = 8;
    try std.testing.expect(lowering_shape(wrong_byte_size) == null);

    var wrong_ticket_offset = original;
    var wrong_fields: [2]RecordField = .{
        original.canonical.record_layout.?.fields[0],
        original.canonical.record_layout.?.fields[1],
    };
    wrong_fields[1].offset = 4;
    var wrong_ticket_layout = original.canonical.record_layout.?;
    wrong_ticket_layout.fields = &wrong_fields;
    wrong_ticket_offset.canonical.record_layout = wrong_ticket_layout;
    try std.testing.expect(lowering_shape(wrong_ticket_offset) == null);

    var wrong_list_offset = original;
    wrong_list_offset.canonical.record_list_layout.?.length_offset = 8;
    try std.testing.expect(lowering_shape(wrong_list_offset) == null);

    var wrong_list_stride = original;
    wrong_list_stride.canonical.record_list_layout.?.element_stride = 8;
    try std.testing.expect(lowering_shape(wrong_list_stride) == null);

    var wrong_list_capacity = original;
    wrong_list_capacity.canonical.record_list_layout.?.max_items = 4;
    try std.testing.expect(lowering_shape(wrong_list_capacity) == null);

    var source_fields: [2]RecordSourceField = .{
    ...
    borrowed_layout.source_fields = &source_fields;
    ...
    wrong_source.canonical.producer.?.source_module = "do:other/source@0.1.0";
    try std.testing.expect(lowering_shape(wrong_source) == null);

    var wrong_write = original.canonical.stream.?;
    wrong_write.write.import_name = "[wrong]stream-write";
    var wrong_stream_descriptor = original;
    wrong_stream_descriptor.canonical.stream = wrong_write;
    try std.testing.expect(lowering_shape(wrong_stream_descriptor) == null);

    var wrong_write_signature = original.canonical.stream.?;
    wrong_write_signature.write.core_params = &.{ "i32", "i32" };
    var wrong_signature_descriptor = original;
    wrong_signature_descriptor.canonical.stream = wrong_write_signature;
    try std.testing.expect(lowering_shape(wrong_signature_descriptor) == null);

    var extra_canonical = original;
    extra_canonical.canonical.list_resource_layout = .{
        .result_pointer_offset = 64,
        .result_length_offset = 68,
        .element_stride = 4,
        .ticket_offset = 0,
        .max_items = 3,
    };
    try std.testing.expect(lowering_shape(extra_canonical) == null);
...
test "checked-in registry admits the pinned filesystem descriptor stat-at ABI" {
    var registry = try Registry.load(std.testing.allocator, @embedFile("p3_async_registry.json"));
    defer registry.deinit(std.testing.allocator);
    const descriptor = registry.find("wasi:filesystem/types@0.3.0-rc-2025-09-16", "descriptor.stat-at") orelse return error.TestUnexpectedResult;
    const shape = switch (lowering_shape(descriptor) orelse return error.TestUnexpectedResult) {
        .filesystem_stat_at => |value| value,
        else => return error.TestUnexpectedResult,
    };
    try std.testing.expectEqualStrings("descriptor", shape.receiver);
    try std.testing.expectEqualStrings("path-flags", shape.path_flags);
    try std.testing.expectEqualStrings("string", shape.path);
    try std.testing.expectEqualStrings("Result<descriptor-stat,error-code>", shape.source_result);
    try std.testing.expectEqualStrings("descriptor-stat", shape.record_layout.name);
    try std.testing.expectEqual(@as(u32, 96), shape.record_layout.byte_size);
    try std.testing.expectEqualStrings("[async-lower][method]descriptor.stat-at", descriptor.canonical.async_import_name);
    try std.testing.expectEqualStrings("[resource-drop]descriptor", shape.resource_drop_import);
    try std.testing.expectEqual(@as(usize, 5), descriptor.canonical.core_params.len);
    try std.testing.expectEqual(@as(usize, 13), descriptor.canonical.completion_params.len);
}

test "filesystem descriptor stat-at lowering rejects ABI drift" {
    var registry = try Registry.load(std.testing.allocator, @embedFile("p3_async_registry.json"));
    defer registry.deinit(std.testing.allocator);
    const descriptor = registry.find("wasi:filesystem/types@0.3.0-rc-2025-09-16", "descriptor.stat-at") orelse return error.TestUnexpectedResult;
    ...
    drifted = descriptor;
    drifted.canonical.async_import_name = "[async-lower][method]descriptor.stat-at-drift";
    try std.testing.expect(lowering_shape(drifted) == null);
}
...
test "owned record producer descriptor is pinned before lowering" {
    var registry = try Registry.load(std.testing.allocator, @embedFile("p3_async_registry.json"));
    defer registry.deinit(std.testing.allocator);
...
test "owned record producer lowering rejects descriptor drift" {
    ...
    var params = [_][]const u8{"stream<list<resource-entry>>"};
    wrong_element.params = &params;
    try std.testing.expect(lowering_shape(wrong_element) == null);

    var wrong_byte_size = descriptor;
    wrong_byte_size.canonical.record_layout.?.byte_size = 8;
    try std.testing.expect(lowering_shape(wrong_byte_size) == null);
    ...
    wrong_import.canonical.async_import_name = "[async-lower]consume-via-stream-drift";
    try std.testing.expect(lowering_shape(wrong_import) == null);

    var wrong_completion = descriptor;
    wrong_completion.canonical.completion = "result-area";
    try std.testing.expect(lowering_shape(wrong_completion) == null);

    const neighboring = registry.find(
        "do:g6-2-c-min-producer@0.1.0",
        "consume-via-stream",
    ) orelse return error.TestUnexpectedResult;
    try std.testing.expect(lowering_shape(neighboring) != null);
}

test "owned record pair producer descriptor is pinned before lowering" {
    var registry = try Registry.load(std.testing.allocator, @embedFile("p3_async_registry.json"));
    defer registry.deinit(std.testing.allocator);
    ...
    switch (lowering_shape(descriptor) orelse return error.TestUnexpectedResult) {
        .record_resource_pair_stream_producer => |shape| {
...
test "owned record pair producer lowering rejects descriptor and ownership drift" {
    ...
    var wrong_offset = descriptor;
    var fields = [_]RecordField{
        wrong_offset.canonical.record_layout.?.fields[0],
        wrong_offset.canonical.record_layout.?.fields[1],
    };
    fields[1].offset = 8;
    var layout = wrong_offset.canonical.record_layout.?;
    layout.fields = &fields;
    wrong_offset.canonical.record_layout = layout;
    try std.testing.expect(lowering_shape(wrong_offset) == null);

    var borrowed = descriptor;
    var source_fields = [_]RecordSourceField{
        borrowed.canonical.record_layout.?.source_fields[0],
        borrowed.canonical.record_layout.?.source_fields[1],
    };
    source_fields[1].ownership = .borrow;
    layout = borrowed.canonical.record_layout.?;
    layout.source_fields = &source_fields;
    borrowed.canonical.record_layout = layout;
    try std.testing.expect(lowering_shape(borrowed) == null);

    var wrong_import = descriptor;
    wrong_import.canonical.stream.?.write.import_name = "[async-lower][stream-write-0]drift";
    try std.testing.expect(lowering_shape(wrong_import) == null);
...
test "owned record nested producer descriptor is registered before lowering" {
    var registry = try Registry.load(std.testing.allocator, @embedFile("p3_async_registry.json"));
    defer registry.deinit(std.testing.allocator);
    const descriptor = registry.find("do:g6-2-owned-record-nested-producer@0.1.0", "consume-via-stream") orelse return error.TestUnexpectedResult;
    try std.testing.expectEqualStrings("9662440709b01044544d4c4350f3e6f783a8f06aaa5c884a96a1e43a935a7543", descriptor.wit_sha256.?);
}

test "owned record nested producer lowering rejects descriptor and nested ABI drift" {
    var registry = try Registry.load(std.testing.allocator, @embedFile("p3_async_registry.json"));
    defer registry.deinit(std.testing.allocator);
    const descriptor = registry.find("do:g6-2-owned-record-nested-producer@0.1.0", "consume-via-stream") orelse return error.TestUnexpectedResult;
    ...
    try std.testing.expect(lowering_shape(wrong_world) == null);
    var wrong_element = descriptor;
    wrong_element.params = &.{ "stream<resource-entry>" };
    try std.testing.expect(lowering_shape(wrong_element) == null);

    var wrong_field = descriptor;
    var field = descriptor.canonical.record_layout.?.fields[0];
    field.offset = 4;
    var fields = [_]RecordField{field};
    var field_layout = descriptor.canonical.record_layout.?;
    field_layout.fields = &fields;
    wrong_field.canonical.record_layout = field_layout;
    try std.testing.expect(lowering_shape(wrong_field) == null);

    var wrong_size = descriptor;
    var size_layout = descriptor.canonical.record_layout.?;
    size_layout.byte_size = 8;
    wrong_size.canonical.record_layout = size_layout;
    try std.testing.expect(lowering_shape(wrong_size) == null);

    var wrong_path = descriptor;
    var path_source = descriptor.canonical.record_layout.?.source_fields[0];
    path_source.name = "wrong";
    var path_sources = [_]RecordSourceField{path_source};
    var path_layout = descriptor.canonical.record_layout.?;
    path_layout.source_fields = &path_sources;
    wrong_path.canonical.record_layout = path_layout;
    try std.testing.expect(lowering_shape(wrong_path) == null);

    var wrong_leaf_name = descriptor;
    var leaf_name = descriptor.canonical.record_layout.?.source_fields[0].nested_fields[0];
    leaf_name.name = "wrong";
    var leaf_names = [_]RecordNestedField{leaf_name};
    var leaf_name_source = descriptor.canonical.record_layout.?.source_fields[0];
    leaf_name_source.nested_fields = &leaf_names;
    var leaf_name_sources = [_]RecordSourceField{leaf_name_source};
    var leaf_name_layout = descriptor.canonical.record_layout.?;
    leaf_name_layout.source_fields = &leaf_name_sources;
    wrong_leaf_name.canonical.record_layout = leaf_name_layout;
    try std.testing.expect(lowering_shape(wrong_leaf_name) == null);

    var wrong_ownership = descriptor;
    var borrowed_leaf = descriptor.canonical.record_layout.?.source_fields[0].nested_fields[0];
    borrowed_leaf.ownership = .borrow;
    var borrowed_leaves = [_]RecordNestedField{borrowed_leaf};
    var borrowed_source = descriptor.canonical.record_layout.?.source_fields[0];
    borrowed_source.nested_fields = &borrowed_leaves;
    var borrowed_sources = [_]RecordSourceField{borrowed_source};
    var borrowed_layout = descriptor.canonical.record_layout.?;
    borrowed_layout.source_fields = &borrowed_sources;
    wrong_ownership.canonical.record_layout = borrowed_layout;
    try std.testing.expect(lowering_shape(wrong_ownership) == null);

    var wrong_resource = descriptor;
    var resource_leaf = descriptor.canonical.record_layout.?.source_fields[0].nested_fields[0];
    resource_leaf.resource = "other";
    var resource_leaves = [_]RecordNestedField{resource_leaf};
    var resource_source = descriptor.canonical.record_layout.?.source_fields[0];
    resource_source.nested_fields = &resource_leaves;
    var resource_sources = [_]RecordSourceField{resource_source};
    var resource_layout = descriptor.canonical.record_layout.?;
    resource_layout.source_fields = &resource_sources;
    wrong_resource.canonical.record_layout = resource_layout;
    try std.testing.expect(lowering_shape(wrong_resource) == null);

    var wrong_source = descriptor;
    wrong_source.canonical.producer.?.source_module = "do:wrong/source@0.1.0";
    try std.testing.expect(lowering_shape(wrong_source) == null);
    var wrong_source_result = descriptor;
    wrong_source_result.canonical.producer.?.source_core_results = &.{ "i64" };
    try std.testing.expect(lowering_shape(wrong_source_result) == null);
    var wrong_import = descriptor;
    wrong_import.canonical.stream.?.write.import_name = "[stream-write-drift]";
    try std.testing.expect(lowering_shape(wrong_import) == null);
    var wrong_signature = descriptor;
    ...
    var direct_leaf = descriptor;
    var direct_source = descriptor.canonical.record_layout.?.source_fields[0];
    direct_source.name = "ticket";
    direct_source.source_type = "ticket";
    direct_source.storage = descriptor.canonical.record_layout.?.source_fields[0].nested_fields[0].storage;
    direct_source.nested_fields = &.{};
    var direct_sources = [_]RecordSourceField{direct_source};
    var direct_layout = descriptor.canonical.record_layout.?;
    direct_layout.source_fields = &direct_sources;
    direct_leaf.canonical.record_layout = direct_layout;
    try std.testing.expect(lowering_shape(direct_leaf) == null);
}
