...
pub fn error_summary(err: anyerror) []const u8 {
    return switch (err) {
        ...
        error.UnsupportedGcCoreLowering => "此 Core Wasm GC target 只支持已实现的受限 text/list/managed-struct/Tuple<text, [u8]> 源码形态",
...
pub fn error_hint(err: anyerror) []const u8 {
    return switch (err) {
        ...
        error.UnsupportedGcCoreLowering => "使用已登记的 text/list/managed-struct 或 `rewrite(pair Tuple<text, [u8]>) -> Tuple<text, [u8]>` 受限源码形态，并使用空 `start()`；其他 GC lowering 尚未接入该 target",
