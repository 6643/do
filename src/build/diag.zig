...
pub fn errorSummary(err: anyerror) []const u8 {
    return switch (err) {
        ...
        error.InvalidImportDecl => "导入使用 `name = @lib(\"file.do\", symbol)`, `name = @lib(\"./file.do\", symbol)`, `name = @lib(\"~/vendor.name.do\", symbol)`；host import 左侧使用 `LowerIdent` 或 `.LowerIdent`，右侧使用 `@env(\"name\", (...) -> Type)` 或 `@wasi_func(\"path/member\", (...) -> Type)`",
...
pub fn errorHint(err: anyerror) []const u8 {
    return switch (err) {
        ...
        error.DuplicateHostImportAlias => "`@env` / `@wasi_func` alias 是当前模块内的 host binding 身份; 同名 binding 要放在不同 source 模块或改名",
        error.DuplicateStructFieldName => "结构体字段名按去掉私有标记后的名字唯一; 每个字段名保留 1 个声明",
        error.MultiReturnInIfCondition => "先接收多返回值, 再在 if 使用单值变量",
        error.MultiReturnInIfBindRhs => "if 条件语法使用单值 bool 表达式",
        error.MultiReturnInLoopCondition => "先接收多返回值, 再在 loop 条件使用单值变量",
        ...
        error.InvalidImportDecl => "导入语法: `name = @lib(\"file.do\", symbol)`, `name = @lib(\"./file.do\", symbol)`, `name = @lib(\"~/vendor.name.do\", symbol)`; host import 左侧使用 `LowerIdent` 或 `.LowerIdent`，右侧使用 `@env(\"name\", (...) -> Type)` 或 `@wasi_func(\"path/member\", (...) -> Type)`",
