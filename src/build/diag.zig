...
pub fn error_summary(err: anyerror) []const u8 {
    return switch (err) {
        ...
        error.UnsupportedP3AsyncMapComponent => "此私有 P3 Component 目标只支持 pinned HashMap<u32, u32> async map 源码形态",
        ...
        error.UnsupportedP3AsyncHostArgComponent => "此私有 P3 Component target 只支持已登记的单个 u32 异步 host 参数源码形态",
        ...
        error.UnsupportedGcCoreLowering => "此 Core Wasm GC token-profile target 只支持已实现的受限 text/managed-struct/Tuple<text, [u8]> 源码形态",
...
pub fn error_hint(err: anyerror) []const u8 {
    return switch (err) {
        ...
        error.UnsupportedP3AsyncMapComponent => "使用 `--p3-async-map-component` 时必须匹配 pinned `HashMap<u32, u32>`、两个固定键值对、单个 async host `submit` 与两处 await；其他 map、producer、Stream 或 ownership 形态保持拒绝",
        ...
        error.UnsupportedP3AsyncHostArgComponent => "使用 `--p3-async-host-arg-component` 时必须匹配固定 `work(u32)`、单个 helper、根字面量 `@async(helper(7))` 与两处 `@await`；其他 producer、payload、resource、list、stream 或 ownership 形态保持拒绝",
        ...
        error.UnsupportedGcCoreLowering => "使用已登记的 text/managed-struct 或 `rewrite(pair Tuple<text, [u8]>) -> Tuple<text, [u8]>` 受限源码形态，并使用空 `start()`；parsed `[u8] @set` 仅由非 CLI G5a test entry 验证，其他 GC lowering 尚未接入该 target",
