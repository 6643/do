(module
  ;; The pinned tool selects the indirect async import ABI because the
  ;; receiver plus four open-at parameters exceed four flat async arguments.
  ...
  ;; [open-at-call] params_ptr points at the canonical six-word block:
  ;; descriptor, path-flags, path-ptr, path-len, open-flags, descriptor-flags.
  ;; result_ptr points at result<descriptor,error-code>: tag:u32, payload:u32.
  ...
  ;; Frame: waitable @0, parent descriptor @4, path @8, path-len @12,
  ;; status @16, subtask @20, result tag @32, result payload @36,
  ;; canonical indirect params @48..71.
  ;; [open-at-result-area] tag/payload are read before parent cleanup.
  ;; [open-at-path-copy] the host adapter copies the UTF-8 path at call start.
  ...
  (func $write-error (param $frame i32) (param $code i32)
    ;; [open-at-error] no child handle is fabricated on Err.
    local.get $frame
    i32.const 32
    i32.add
    i32.const 1
  ...
  (func $finish (param $frame i32) (result i32)
    ...
    (local $payload i32)
    (local $subtask i32)
    ;; [open-at-ready] task-return receives the owned child handle on Ok.
    local.get $frame
    i32.const 32
    i32.add
    i32.load
    ...
    ;; [descriptor-drop] the parent is consumed exactly once.
    local.get $frame
    i32.const 4
    i32.add
    i32.load
