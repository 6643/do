...
test "future-owned emitter emits the measured private frame markers" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, wat, "[abi-plan] arguments=0 results=1") != null);
    try std.testing.expect(std.mem.indexOf(u8, wat, "[resource-terminal] drop_owned") != null);
