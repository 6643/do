...
const canonical_core_wat = @embedFile("cmin_dynamic_list_resource_producer_template.wat");

pub const ProducerError = error{UnsupportedP3DynamicListResourceProducer};

pub const DynamicListResourceProducerPlan = struct {
    ...
    pub fn analyze(tokens: []const lexer.Token, registry: p3_async_manifest.Registry) ProducerError!DynamicListResourceProducerPlan {
        ...
        const shape = switch (p3_async_manifest.lowering_shape(descriptor) orelse return error.UnsupportedP3DynamicListResourceProducer) {
            .record_resource_list_stream_dynamic_producer => |value| value,
            else => return error.UnsupportedP3DynamicListResourceProducer,
        };

        const source = find_host_binding(tokens, .source) orelse return error.UnsupportedP3DynamicListResourceProducer;
        const sink = find_host_binding(tokens, .sink) orelse return error.UnsupportedP3DynamicListResourceProducer;
        if (count_host_bindings(tokens) != 2 or
            !std.mem.eql(u8, source.locator, "do:g6-2-c-min-dynamic-producer/source@0.1.0") or
            ...
            !sink_signature_is_exact(tokens, sink.signature_open)) return error.UnsupportedP3DynamicListResourceProducer;

        const ticket = find_resource_decl(tokens) orelse return error.UnsupportedP3DynamicListResourceProducer;
        if (!std.mem.eql(u8, ticket.name, "Ticket") or
            !std.mem.eql(u8, ticket.path, "do:g6-2-c-min-dynamic-producer/source/ticket")) return error.UnsupportedP3DynamicListResourceProducer;
        if (!find_record_decl(tokens, "ResourceEntry", "Ticket") or
            !find_error_decl(tokens, "ProducerError") or
            !find_producer_function(tokens, "produce", "ProducerError")) return error.UnsupportedP3DynamicListResourceProducer;
        if (count_top_level_functions(tokens) != 2 or
            ...
            count_intrinsic(tokens, "cancel") != 0) return error.UnsupportedP3DynamicListResourceProducer;
        ...
        if (!std.mem.eql(u8, shape.element, "resource-entry") or
            ...
            !std.mem.eql(u8, shape.producer.source_module, "do:g6-2-c-min-dynamic-producer/source@0.1.0") or
...
pub fn emit_component_wat(allocator: std.mem.Allocator, plan: DynamicListResourceProducerPlan) ![]u8 {
    ...
    if (plan.layout.result_pointer_offset != 64 or plan.layout.result_length_offset != 68 or
        ...
        plan.layout.max_items != 3 or plan.producer.stream_capacity != 1) return error.UnsupportedP3DynamicListResourceProducer;
    ...
    const metadata = try generated_text.alloc_fmt_block(
        ...
        \\  ;; [producer-plan-layout] pointer={[pointer]d} length={[length]d} stride={[stride]d} ticket-offset={[ticket_offset]d} capacity={[capacity]d}
        \\
    ,
        .{
    ...
    const close = std.mem.lastIndexOf(u8, core_wat, "\n)") orelse return error.UnsupportedP3DynamicListResourceProducer;
...
pub fn emit_component_wat_for_tokens(allocator: std.mem.Allocator, tokens: []const lexer.Token) ![]u8 {
    ...
    const plan = try DynamicListResourceProducerPlan.analyze(tokens, registry);
    return emit_component_wat(allocator, plan);
}

pub fn emit_component_wit(allocator: std.mem.Allocator, plan: DynamicListResourceProducerPlan) ![]u8 {
    try validate_internal_plans(allocator, plan);
    if (!std.mem.eql(u8, plan.descriptor.wit.world, "dynamic-list-producer")) return error.UnsupportedP3DynamicListResourceProducer;
...
fn validate_internal_plans(allocator: std.mem.Allocator, plan: DynamicListResourceProducerPlan) ProducerError!void {
    ...
    }) catch return error.UnsupportedP3DynamicListResourceProducer;
    defer entry.deinit();
    var list = wit_abi_types.AbiType.list(allocator, &entry) catch return error.UnsupportedP3DynamicListResourceProducer;
    defer list.deinit();
    ...
    for (accepted_lengths) |length| {
        var slots = layout.owned_slot_iterator(length) catch return error.UnsupportedP3DynamicListResourceProducer;
        var slot_count: u32 = 0;
        while (slots.next() != null) slot_count += 1;
        if (slot_count != length) return error.UnsupportedP3DynamicListResourceProducer;
        ...
        ownership.apply(.{ .allocate = {} }) catch return error.UnsupportedP3DynamicListResourceProducer;
        var index: u32 = 0;
        while (index < length) : (index += 1) {
            ownership.apply(.{ .create_ticket = index }) catch return error.UnsupportedP3DynamicListResourceProducer;
        }
        ownership.apply(.{ .enqueue = {} }) catch return error.UnsupportedP3DynamicListResourceProducer;
        ownership.apply(.{ .transfer = {} }) catch return error.UnsupportedP3DynamicListResourceProducer;
        index = 0;
        while (index < length) : (index += 1) {
            ownership.apply(.{ .clear_source_slot = index }) catch return error.UnsupportedP3DynamicListResourceProducer;
        }
        ownership.apply(.{ .release_list = {} }) catch return error.UnsupportedP3DynamicListResourceProducer;
        ownership.apply(.{ .terminal_finalize = {} }) catch return error.UnsupportedP3DynamicListResourceProducer;
    }

    var frame = wit_abi_async.ListProducerFramePlan.init(allocator);
    defer frame.deinit(allocator);
    frame.apply(.{ .allocate = {} }, allocator) catch return error.UnsupportedP3DynamicListResourceProducer;
    frame.apply(.{ .queue = {} }, allocator) catch return error.UnsupportedP3DynamicListResourceProducer;
    frame.apply(.{ .transfer = {} }, allocator) catch return error.UnsupportedP3DynamicListResourceProducer;
    frame.apply(.{ .await_sink = {} }, allocator) catch return error.UnsupportedP3DynamicListResourceProducer;
    frame.apply(.{ .complete = .ready }, allocator) catch return error.UnsupportedP3DynamicListResourceProducer;
    frame.apply(.{ .finalize = {} }, allocator) catch return error.UnsupportedP3DynamicListResourceProducer;
}
...
fn find_producer_function(tokens: []const lexer.Token, name: []const u8, error_name: []const u8) bool {
    ...
    while (idx + 10 < tokens.len) : (idx += 1) {
        ...
        if (params_close != idx + 4 or !tok_eq(tokens[idx + 2], "count") or !tok_eq(tokens[idx + 3], "u32") or
...
test "dynamic list producer plan rejects a missing fixed source body" {
    ...
    try std.testing.expectError(error.UnsupportedP3DynamicListResourceProducer, DynamicListResourceProducerPlan.analyze(tokens, registry));
}
...
test "dynamic list producer plan accepts the exact fixed Do source" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, wit, "export produce: async func(count: u32)") != null);
}

fn expect_dynamic_plan_error(source: []const u8) !void {
    var registry = try p3_async_manifest.Registry.load(std.testing.allocator, @embedFile("p3_async_registry.json"));
    defer registry.deinit(std.testing.allocator);
    const tokens = try lexer.tokenize(std.testing.allocator, source);
    defer std.testing.allocator.free(tokens);
...
test "dynamic list producer plan rejects a non-u32 runtime count" {
    try expect_dynamic_plan_error(
        ...
        \\ResourceEntry { .ticket Ticket }
        \\ProducerError error = Io | Pipe | InvalidMode
        \\produce(count i64) -> Result<nil, ProducerError> { return Ok() }
        \\start() {}
    );
}
...
test "dynamic list producer plan rejects source async syntax" {
    try expect_dynamic_plan_error(
        ...
        \\ResourceEntry { .ticket Ticket }
        \\ProducerError error = Io | Pipe | InvalidMode
        \\produce(count u32) -> Result<nil, ProducerError> { @await(count) }
        \\start() {}
    );
}
