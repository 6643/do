...
const canonical_core_wat = @embedFile("mixed_owned_record_stream_producer_template.wat");

pub const ProducerError = error{UnsupportedP3MixedOwnedRecordStreamProducer};

/// The mixed producer is a deliberately closed compiler slice. Its analyzer
/// accepts only the pinned scalar-plus-owned-field source shape; it is not a
/// generic record/resource lowering entry point.
pub const MixedOwnedRecordStreamProducerPlan = struct {
    ...
    pub fn analyze(
    ...
    ) ProducerError!MixedOwnedRecordStreamProducerPlan {
        const descriptor = registry.find(
            "do:g6-2-owned-record-mixed-producer@0.1.0",
            "consume-via-stream",
        ) orelse return error.UnsupportedP3MixedOwnedRecordStreamProducer;
        const shape = switch (p3_async_manifest.lowering_shape(descriptor) orelse
            return error.UnsupportedP3MixedOwnedRecordStreamProducer) {
            .record_resource_mixed_stream_producer => |value| value,
            else => return error.UnsupportedP3MixedOwnedRecordStreamProducer,
        };
        ...
        if (count_host_bindings(tokens) != 2 or
            ...
            !std.mem.eql(u8, source.locator, "do:g6-2-owned-record-mixed-producer/source@0.1.0") or
        ...
        if (count_resource_decls(tokens) != 1 or
            ...
            !std.mem.eql(u8, ticket.path, "do:g6-2-owned-record-mixed-producer/source/ticket") or
            !resource_decl_is_exact(tokens, "Ticket", ticket.path))
            return error.UnsupportedP3MixedOwnedRecordStreamProducer;
        if (count_record_decls(tokens, "MixedEntry") != 1 or
            !find_mixed_record_decl(tokens, "MixedEntry", "Ticket") or
        ...
        if (!std.mem.eql(u8, shape.element, "mixed-entry") or
            shape.record_layout.byte_size != 8 or
            shape.record_layout.alignment != 4 or
            shape.record_layout.fields.len != 2 or
            shape.record_layout.source_fields.len != 2 or
            !std.mem.eql(u8, shape.record_layout.name, "mixed-entry") or
            !field_is(shape.record_layout.fields[0], "code", "i32", 0) or
            !field_is(shape.record_layout.fields[1], "ticket", "i32", 4) or
            !scalar_source_field_is(shape.record_layout.source_fields[0]) or
            !source_field_is(shape.record_layout.source_fields[1], "ticket") or
            shape.producer.stream_capacity != 1 or
            !std.mem.eql(u8, shape.producer.source_module, "do:g6-2-owned-record-mixed-producer/source@0.1.0") or
...
pub fn emit_component_wat(allocator: std.mem.Allocator, plan: MixedOwnedRecordStreamProducerPlan) ![]u8 {
...
pub fn emit_component_wat_for_tokens(allocator: std.mem.Allocator, tokens: []const lexer.Token) ![]u8 {
    ...
    const plan = try MixedOwnedRecordStreamProducerPlan.analyze(tokens, registry);
    return emit_component_wat(allocator, plan);
}

pub fn emit_component_wit(allocator: std.mem.Allocator, plan: MixedOwnedRecordStreamProducerPlan) ![]u8 {
    try validate_internal_plans(allocator, plan);
    if (!std.mem.eql(u8, plan.descriptor.wit.world, "owned-record-mixed-producer"))
        return error.UnsupportedP3MixedOwnedRecordStreamProducer;
...
fn validate_internal_plans(allocator: std.mem.Allocator, plan: MixedOwnedRecordStreamProducerPlan) ProducerError!void {
    producer_contract.validate_contract(plan.contract) catch return error.UnsupportedP3MixedOwnedRecordStreamProducer;
    if (!std.mem.eql(u8, plan.contract.descriptor_id, plan.descriptor.locator) or
        plan.contract.sink.capacity != plan.producer.stream_capacity) return error.UnsupportedP3MixedOwnedRecordStreamProducer;
    ...
    var code = wit_abi_types.AbiType.scalar(allocator, .u32);
    defer code.deinit();
    ...
    }) catch return error.UnsupportedP3MixedOwnedRecordStreamProducer;
    defer entry.deinit();
    ...
    var layout = wit_abi_layout.LayoutPlan.record(allocator, &entry, .{
        ...
        .fields = &.{
            ...
            .{ .name = "ticket", .offset = 4, .byte_size = 4, .alignment = 4, .indirect = null },
        },
    }) catch return error.UnsupportedP3MixedOwnedRecordStreamProducer;
    defer layout.deinit();
    if (layout.byte_size != 8 or layout.alignment != 4 or layout.record_fields.len != 2 or
        ...
        !std.mem.eql(u8, layout.record_fields[0].name, "code") or
        !std.mem.eql(u8, layout.record_fields[1].name, "ticket"))
        return error.UnsupportedP3MixedOwnedRecordStreamProducer;
}
...
fn sink_signature_is_exact(tokens: []const lexer.Token, open: usize) bool {
    ...
    return close == open + 5 and tok_eq(tokens[open + 1], "StreamWriter") and tok_eq(tokens[open + 2], "<") and
        tok_eq(tokens[open + 3], "MixedEntry") and tok_eq(tokens[open + 4], ">") and
...
fn find_mixed_record_decl(tokens: []const lexer.Token, name: []const u8, field_type: []const u8) bool {
    var idx: usize = 0;
    while (idx + 6 < tokens.len) : (idx += 1) {
        if (!tok_eq(tokens[idx], name) or !tok_eq(tokens[idx + 1], "{") or
            !tok_eq(tokens[idx + 2], ".code") or !tok_eq(tokens[idx + 3], "u32") or
            !tok_eq(tokens[idx + 4], ".ticket") or !tok_eq(tokens[idx + 5], field_type) or
...
fn scalar_source_field_is(field: p3_async_manifest.RecordSourceField) bool {
    return std.mem.eql(u8, field.name, "code") and
        std.mem.eql(u8, field.source_type, "u32") and
        field.storage.len == 1 and std.mem.eql(u8, field.storage[0], "code") and
...
test "mixed owned record producer plan and emitter accept only the pinned shape" {
    ...
    const plan = try MixedOwnedRecordStreamProducerPlan.analyze(tokens, registry);
    try std.testing.expectEqualStrings("MixedEntry", plan.record_type_name);
    ...
    try std.testing.expectEqual(@as(u32, 4), plan.contract.ownership.leaves[0].handle_offset);
...
test "mixed owned record producer rejects a single-field or borrowed record" {
    var registry = try p3_async_manifest.Registry.load(std.testing.allocator, @embedFile("p3_async_registry.json"));
    defer registry.deinit(std.testing.allocator);

    const single =
        \\make_ticket = @host_func("do:g6-2-owned-record-mixed-producer/source@0.1.0", "make-ticket", (u32) -> Ticket)
        \\consume = @host_async_func("do:g6-2-owned-record-mixed-producer@0.1.0", "consume-via-stream", (StreamWriter<ResourceEntry>) -> Result<nil, ProducerError>)
        \\Ticket = @wasi_resource("do:g6-2-owned-record-mixed-producer/source/ticket", { .id i64 })
    ...
    for ([_][]const u8{ single, borrowed }) |source| {
        ...
        try std.testing.expectError(
            error.UnsupportedP3MixedOwnedRecordStreamProducer,
            MixedOwnedRecordStreamProducerPlan.analyze(tokens, registry),
        );
    }
}
