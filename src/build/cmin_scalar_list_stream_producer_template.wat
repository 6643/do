(module
  ...
  (import "do:g6-2-scalar-list-producer/sink@0.1.0" "[async-lower]consume-via-stream"
    (func $sink-call (type $async-lower)))
  (import "do:g6-2-scalar-list-producer/sink@0.1.0" "[stream-new-0]consume-via-stream"
    (func $stream-new (type $stream-new)))
  (import "do:g6-2-scalar-list-producer/sink@0.1.0" "[stream-cancel-read-0]consume-via-stream"
    (func $stream-cancel-read (type $stream-cancel)))
  (import "do:g6-2-scalar-list-producer/sink@0.1.0" "[stream-cancel-write-0]consume-via-stream"
    (func $stream-cancel-write (type $stream-cancel)))
  (import "do:g6-2-scalar-list-producer/sink@0.1.0" "[stream-drop-readable-0]consume-via-stream"
    (func $stream-drop-readable (type $stream-drop)))
  (import "do:g6-2-scalar-list-producer/sink@0.1.0" "[stream-drop-writable-0]consume-via-stream"
    (func $stream-drop-writable (type $stream-drop)))
  (import "do:g6-2-scalar-list-producer/sink@0.1.0" "[async-lower][stream-read-0]consume-via-stream"
    (func $stream-read (type $stream-io)))
  (import "do:g6-2-scalar-list-producer/sink@0.1.0" "[async-lower][stream-write-0]consume-via-stream"
  ...
  ;; Guest owns the list while state=1. State=2 means the stream owns the
  ;; copied scalar values and the guest only releases its backing allocation.
  ...
  (func $assert-list-release (param $frame i32)
  ...
  (func $cleanup (param $frame i32) (param $tag i32) (param $payload i32) (result i32)
    ...
    ;; Child subtask, waitable set, stream handles, list storage, then frame.
    local.get $frame
    i32.const 8
    i32.add
    i32.load
  ...
  (func $make-list (param $frame i32) (param $count-input i32) (result i32)
    (local $count i32)
    (local $ptr i32)
    (local $index i32)
  ...
  (func (export "[async-lift]produce") (type $async-run-u32) (param $count i32) (result i32)
    (local $frame i32)
    (local $pair i64)
    (local $status i32)
    (local $subtask i32)
