...
pub const Error = error{UnsupportedP3WasiFilesystemSyncDataComponent};

const locator = "wasi:filesystem/types@0.3.0-rc-2025-09-16";
const member = "descriptor.sync-data";
...
pub fn emit_component_wat(
    ...
    const plan = try SyncDataPlan.analyze(tokens, registry);
    if (!std.mem.eql(u8, plan.descriptor.canonical.async_import_module, locator) or
        !std.mem.eql(u8, plan.descriptor.canonical.async_import_name, "[async-lower][method]descriptor.sync-data"))
        return error.UnsupportedP3WasiFilesystemSyncDataComponent;
    return generated_text.alloc_block(allocator, 0, @embedFile("wasi_filesystem_sync_data_component_template.wat"));
}
...
pub fn emit_component_wit(
    ...
    var registry = try p3_async_manifest.Registry.load(allocator, @embedFile("p3_async_registry.json"));
    defer registry.deinit(allocator);
    _ = try SyncDataPlan.analyze(tokens, registry);
    return generated_text.alloc_block(allocator, 0, component_wit);
}

pub const SyncDataPlan = struct {
    ...
    pub fn analyze(tokens: []const lexer.Token, registry: p3_async_manifest.Registry) Error!SyncDataPlan {
        const descriptor = registry.find(locator, member) orelse return error.UnsupportedP3WasiFilesystemSyncDataComponent;
        switch (p3_async_manifest.lowering_shape(descriptor) orelse return error.UnsupportedP3WasiFilesystemSyncDataComponent) {
            .filesystem_sync_data => {},
            else => return error.UnsupportedP3WasiFilesystemSyncDataComponent,
        }
        ...
        while (idx + 9 < tokens.len) : (idx += 1) {
            ...
            if (host != null or !signature_matches(tokens, idx + 9)) return error.UnsupportedP3WasiFilesystemSyncDataComponent;
            host = .{ .name = tokens[idx].lexeme };
        }
        const host_binding = host orelse return error.UnsupportedP3WasiFilesystemSyncDataComponent;
        if (host_count != 1 or !has_exact_resource_decl(tokens) or !has_exact_sync_data_error_decl(tokens))
            return error.UnsupportedP3WasiFilesystemSyncDataComponent;

        const function = find_run_function(tokens) orelse return error.UnsupportedP3WasiFilesystemSyncDataComponent;
...
fn signature_matches(tokens: []const lexer.Token, signature_start: usize) bool {
    ...
    return host_close > params_close + 3 and
        compact_token_range_equals(tokens, params_close + 3, host_close, "nil|SyncDataError");
}

fn has_exact_resource_decl(tokens: []const lexer.Token) bool {
    const expected = "Dir=@wasi_resource(\"filesystem/types/descriptor\",{.idi64})";
    return has_exact_token_sequence(tokens, "Dir", expected);
}

fn has_exact_sync_data_error_decl(tokens: []const lexer.Token) bool {
    return has_exact_token_sequence(tokens, "SyncDataError", "SyncDataErrorerror=Io|NoEntry");
}
...
fn find_run_function(tokens: []const lexer.Token) ?RunFunction {
    ...
    while (idx + 8 < tokens.len) : (idx += 1) {
        ...
        if (body_open + 1 >= tokens.len or !tok_eq(tokens[body_open], "nil") or
            !tok_eq(tokens[body_open + 1], "{")) continue;
        const body_close = find_matching(tokens, body_open + 1, "{", "}") catch return null;
        if (!compact_token_range_equals(
            tokens,
            body_open + 2,
            body_close,
            "pendingFuture<nil|SyncDataError>=sync_data_descriptor(file)resultnil|SyncDataError=@await(pending)",
        )) return null;
        return .{
            .root_name = tokens[idx].lexeme,
            .file_name = tokens[idx + 2].lexeme,
            .pending_name = tokens[body_open + 2].lexeme,
        };
    }
    return null;
}
...
test "filesystem descriptor sync-data source shape admits the bounded direct await" {
    ...
    try std.testing.expect(has_exact_sync_data_error_decl(tokens));
