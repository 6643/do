...
const canonical_core_wat = @embedFile("parameterized_owned_record_pair_stream_producer_template.wat");

pub const ProducerError = error{UnsupportedP3ParameterizedOwnedRecordPairStreamProducer};

/// The parameterized pair producer is a deliberately closed compiler slice.
/// Its analyzer accepts only the pinned descriptor, explicit three-argument
/// producer, and exact two-owned-field source shape; it is not a generic
/// record/resource lowering entry point.
pub const ParameterizedOwnedRecordPairStreamProducerPlan = struct {
    ...
    root_name: []const u8,
    mode_name: []const u8,
    left_seed_name: []const u8,
    right_seed_name: []const u8,
    ...
    pub fn analyze(
    ...
    ) ProducerError!ParameterizedOwnedRecordPairStreamProducerPlan {
        const descriptor = registry.find(
            "do:g6-2-owned-record-pair-parameterized-producer@0.1.0",
            "consume-via-stream",
        ) orelse return error.UnsupportedP3ParameterizedOwnedRecordPairStreamProducer;
        const shape = switch (p3_async_manifest.lowering_shape(descriptor) orelse
            return error.UnsupportedP3ParameterizedOwnedRecordPairStreamProducer)
        {
            .record_resource_pair_parameterized_stream_producer => |value| value,
            else => return error.UnsupportedP3ParameterizedOwnedRecordPairStreamProducer,
        };
        ...
        if (count_host_bindings(tokens) != 2 or
            ...
            !std.mem.eql(u8, source.locator, "do:g6-2-owned-record-pair-parameterized-producer/source@0.1.0") or
        ...
        if (count_resource_decls(tokens) != 1 or
            ...
            !std.mem.eql(u8, ticket.path, "do:g6-2-owned-record-pair-parameterized-producer/source/ticket") or
            !resource_decl_is_exact(tokens, "Ticket", ticket.path))
            return error.UnsupportedP3ParameterizedOwnedRecordPairStreamProducer;
        if (count_record_decls(tokens, "ResourcePair") != 1 or
            !find_pair_record_decl(tokens, "ResourcePair", "Ticket") or
            !find_error_decl(tokens, "ProducerError") or
            count_error_decls(tokens, "ProducerError") != 1 or
            !find_parameterized_producer_function(tokens, "produce", "ProducerError") or
        ...
        if (!std.mem.eql(u8, shape.element, "resource-pair") or
            shape.record_layout.byte_size != 8 or
            shape.record_layout.fields.len != 2 or
            shape.record_layout.source_fields.len != 2 or
            !std.mem.eql(u8, shape.record_layout.name, "resource-pair") or
            !field_is(shape.record_layout.fields[0], "left", "i32", 0) or
            !field_is(shape.record_layout.fields[1], "right", "i32", 4) or
            !source_field_is(shape.record_layout.source_fields[0], "left") or
            !source_field_is(shape.record_layout.source_fields[1], "right") or
            shape.producer.stream_capacity != 1 or
            !std.mem.eql(u8, shape.producer.source_module, "do:g6-2-owned-record-pair-parameterized-producer/source@0.1.0") or
            ...
            !std.mem.eql(u8, shape.producer.left_seed_param, "u32") or
            !std.mem.eql(u8, shape.producer.right_seed_param, "u32") or
            !equal_core_types(shape.producer.source_core_params, &.{ "i32" }) or
            !equal_core_types(shape.producer.source_core_results, &.{ "i32" }) or
            !equal_core_types(shape.producer.producer_core_params, &.{ "i32", "i32", "i32" }) or
            !equal_core_types(shape.producer.producer_core_results, &.{ "i32" }))
            return error.UnsupportedP3ParameterizedOwnedRecordPairStreamProducer;
        ...
        return .{
            ...
            .record_type_name = "ResourcePair",
            .error_type_name = "ProducerError",
            .root_name = "produce",
            .mode_name = "mode",
            .left_seed_name = "left_seed",
            .right_seed_name = "right_seed",
...
pub fn emit_component_wat(allocator: std.mem.Allocator, plan: ParameterizedOwnedRecordPairStreamProducerPlan) ![]u8 {
    try validate_internal_plans(allocator, plan);
    if (plan.layout.byte_size != 8 or plan.layout.fields.len != 2 or
        plan.layout.fields[0].offset != 0 or plan.layout.fields[1].offset != 4 or
        plan.producer.stream_capacity != 1)
        return error.UnsupportedP3ParameterizedOwnedRecordPairStreamProducer;
...
pub fn emit_component_wat_for_tokens(allocator: std.mem.Allocator, tokens: []const lexer.Token) ![]u8 {
    ...
    const plan = try ParameterizedOwnedRecordPairStreamProducerPlan.analyze(tokens, registry);
    return emit_component_wat(allocator, plan);
}

pub fn emit_component_wit(allocator: std.mem.Allocator, plan: ParameterizedOwnedRecordPairStreamProducerPlan) ![]u8 {
    try validate_internal_plans(allocator, plan);
    if (!std.mem.eql(u8, plan.descriptor.wit.world, "owned-record-pair-parameterized-producer"))
        return error.UnsupportedP3ParameterizedOwnedRecordPairStreamProducer;
...
fn validate_internal_plans(allocator: std.mem.Allocator, plan: ParameterizedOwnedRecordPairStreamProducerPlan) ProducerError!void {
    producer_contract.validate_contract(plan.contract) catch return error.UnsupportedP3ParameterizedOwnedRecordPairStreamProducer;
    if (!std.mem.eql(u8, plan.contract.descriptor_id, plan.descriptor.locator) or
        plan.contract.sink.capacity != plan.producer.stream_capacity) return error.UnsupportedP3ParameterizedOwnedRecordPairStreamProducer;
    ...
    var ticket = wit_abi_types.AbiType.resource(allocator, "ticket", .own) catch
        return error.UnsupportedP3ParameterizedOwnedRecordPairStreamProducer;
    defer ticket.deinit();
    var pair = wit_abi_types.AbiType.record(allocator, &.{
        .{ .name = "left", .value = &ticket },
        .{ .name = "right", .value = &ticket },
    }) catch return error.UnsupportedP3ParameterizedOwnedRecordPairStreamProducer;
    defer pair.deinit();

    var layout = wit_abi_layout.LayoutPlan.record(allocator, &pair, .{
        .byte_size = plan.layout.byte_size,
        .alignment = 4,
        .fields = &.{
            .{ .name = "left", .offset = 0, .byte_size = 4, .alignment = 4, .indirect = null },
            .{ .name = "right", .offset = 4, .byte_size = 4, .alignment = 4, .indirect = null },
        },
    }) catch return error.UnsupportedP3ParameterizedOwnedRecordPairStreamProducer;
    defer layout.deinit();
    if (layout.byte_size != 8 or layout.alignment != 4 or layout.record_fields.len != 2 or
        layout.record_fields[0].offset != 0 or layout.record_fields[1].offset != 4 or
        !std.mem.eql(u8, layout.record_fields[0].name, "left") or
        !std.mem.eql(u8, layout.record_fields[1].name, "right"))
        return error.UnsupportedP3ParameterizedOwnedRecordPairStreamProducer;
}
...
fn sink_signature_is_exact(tokens: []const lexer.Token, open: usize) bool {
    ...
    return close == open + 5 and tok_eq(tokens[open + 1], "StreamWriter") and tok_eq(tokens[open + 2], "<") and
        tok_eq(tokens[open + 3], "ResourcePair") and tok_eq(tokens[open + 4], ">") and
...
fn find_pair_record_decl(tokens: []const lexer.Token, name: []const u8, field_type: []const u8) bool {
    var idx: usize = 0;
    while (idx + 6 < tokens.len) : (idx += 1) {
        if (!tok_eq(tokens[idx], name) or !tok_eq(tokens[idx + 1], "{") or
            !tok_eq(tokens[idx + 2], ".left") or !tok_eq(tokens[idx + 3], field_type) or
            !tok_eq(tokens[idx + 4], ".right") or !tok_eq(tokens[idx + 5], field_type) or
            !tok_eq(tokens[idx + 6], "}")) continue;
        return true;
    }
    return false;
}
...
fn find_parameterized_producer_function(tokens: []const lexer.Token, name: []const u8, error_name: []const u8) bool {
    var idx: usize = 0;
    while (idx + 16 < tokens.len) : (idx += 1) {
        ...
        if (params_close != idx + 10 or
            !tok_eq(tokens[idx + 2], "mode") or !tok_eq(tokens[idx + 3], "u32") or
            !tok_eq(tokens[idx + 4], ",") or !tok_eq(tokens[idx + 5], "left_seed") or
            !tok_eq(tokens[idx + 6], "u32") or !tok_eq(tokens[idx + 7], ",") or
            !tok_eq(tokens[idx + 8], "right_seed") or !tok_eq(tokens[idx + 9], "u32") or
...
fn source_field_is(field: p3_async_manifest.RecordSourceField, name: []const u8) bool {
    return std.mem.eql(u8, field.name, name) and std.mem.eql(u8, field.source_type, "ticket") and
        ...
        field.resource != null and std.mem.eql(u8, field.resource.?, "ticket") and field.drop_import != null and
...
test "parameterized owned record pair producer plan and emitter accept only the pinned shape" {
    ...
    try std.testing.expectEqualStrings("u32", plan.contract.runtime_mode_param.?);

    const wat = try emit_component_wat(std.testing.allocator, plan);
    defer std.testing.allocator.free(wat);
    try std.testing.expect(std.mem.indexOf(u8, wat, "[producer-record-byte-size] 8") != null);
    try std.testing.expect(std.mem.indexOf(u8, wat, "[producer-record-left-offset] 0") != null);
    try std.testing.expect(std.mem.indexOf(u8, wat, "[producer-record-right-offset] 4") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, wit, "record resource-pair") != null);
...
test "parameterized owned record pair producer rejects a single-field or borrowed record" {
    var registry = try p3_async_manifest.Registry.load(std.testing.allocator, @embedFile("p3_async_registry.json"));
    defer registry.deinit(std.testing.allocator);

    const single =
        \\make_ticket = @host_func("do:g6-2-owned-record-pair-parameterized-producer/source@0.1.0", "make-ticket", (u32) -> Ticket)
        \\consume = @host_async_func("do:g6-2-owned-record-pair-parameterized-producer@0.1.0", "consume-via-stream", (StreamWriter<ResourceEntry>) -> Result<nil, ProducerError>)
        \\Ticket = @wasi_resource("do:g6-2-owned-record-pair-parameterized-producer/source/ticket", { .id i64 })
    ...
    const borrowed =
        ...
        \\start() {}
    ;
    for ([_][]const u8{ single, borrowed }) |source| {
        const tokens = try lexer.tokenize(std.testing.allocator, source);
        defer std.testing.allocator.free(tokens);
        try std.testing.expectError(
            error.UnsupportedP3ParameterizedOwnedRecordPairStreamProducer,
            ParameterizedOwnedRecordPairStreamProducerPlan.analyze(tokens, registry),
        );
    }
}
