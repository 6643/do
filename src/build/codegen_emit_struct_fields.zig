...
pub const field_static_values_equal = codegen_collect_reflection.field_static_values_equal;
pub const field_reflection_local_visible = context.field_reflection_local_visible;
pub const field_reflection_local_name_prefix = context.field_reflection_local_name_prefix;
pub const field_reflection_if_parts = codegen_collect_reflection.field_reflection_if_parts;
pub const field_static_bool_expr = codegen_collect_reflection.field_static_bool_expr;
pub const field_static_value = codegen_collect_reflection.field_static_value;
pub const field_visible_from_tokens = codegen_collect_reflection.field_visible_from_tokens;
pub const is_private_field_name = codegen_collect_reflection.is_private_field_name;
pub const typed_struct_binding = codegen_collect_reflection.typed_struct_binding;
pub const inferred_struct_binding = codegen_collect_reflection.inferred_struct_binding;
pub const borrowed_field_meta_local_set = context.borrowed_field_meta_local_set;
pub const single_field_meta_arg = codegen_collect_reflection.single_field_meta_arg;
pub const collect_field_reflection_static_if = codegen_collect_reflection.collect_field_reflection_static_if;
pub const collect_field_reflection_static_branch = codegen_collect_reflection.collect_field_reflection_static_branch;
...
pub fn emit_field_reflection_loop_block(allocator: std.mem.Allocator, tokens: []const lexer.Token, header: FieldReflectionLoopHeader, body_start: usize, locals: *const LocalSet, return_cleanup_locals: *const LocalSet, control_cleanup_locals: *const LocalSet, ctx: CodegenContext, result_tys: []const []const u8, result_items: []const FuncResultItem, result_struct: ?[]const u8, result_union: ?UnionLayout, loop_ctx: ?LoopControl, defer_ctx: ?*const DeferContext, return_label: ?[]const u8, out: *std.ArrayList(u8)) CodegenError!bool {
    ...
    try append_fmt(allocator, out, "    ;; field-reflect-loop type={[type]s}\n", .{ .type = header.decl.name });
...
pub fn emit_managed_struct_field_set(allocator: std.mem.Allocator, tokens: []const lexer.Token, value_start: usize, value_end: usize, body_end: usize, allow_last_use_move: bool, target_name: []const u8, field_name: []const u8, field_offset: usize, field_ty: []const u8, locals: *const LocalSet, defer_ctx: ?*const DeferContext, ctx: CodegenContext, out: *std.ArrayList(u8)) CodegenError!void {
    ...
    try append_fmt(allocator, out, "      ;; arc-managed-struct-reuse {[target]s}.{[field]s}\n", .{ .target = target_name, .field = field_name });
    try append_fmt(allocator, out, "      ;; arc-overwrite-release {[target]s}.{[field]s}\n", .{ .target = target_name, .field = field_name });
    try append_fmt(allocator, out, "      local.get ${[name]s}\n", .{ .name = STORAGE_OVERWRITE_TMP_LOCAL });
    ...
    try append_fmt(allocator, out, "      ;; arc-managed-struct-clone-set {[target]s}.{[field]s}\n", .{ .target = target_name, .field = field_name });
    ...
    if (move_source) |source| {
        try append_fmt(allocator, out, "    ;; field-set-move {[name]s}\n", .{ .name = source.source_name });
...
pub fn emit_struct_field_meta_set_assignment(allocator: std.mem.Allocator, tokens: []const lexer.Token, start_idx: usize, end_idx: usize, body_end: usize, allow_last_use_move: bool, locals: *const LocalSet, defer_ctx: ?*const DeferContext, ctx: CodegenContext, out: *std.ArrayList(u8)) !bool {
    ...
    try append_fmt(allocator, out, "    ;; field-set name={[target]s} field={[field]s}\n", .{
...
pub fn emit_managed_struct_clone_with_field_set(allocator: std.mem.Allocator, target_name: []const u8, target_field_name: []const u8, decl: StructDecl, struct_ty: []const u8, layout: StructLayout, out: *std.ArrayList(u8)) CodegenError!void {
    ...
    try append_fmt(allocator, out, "      local.set ${[name]s}\n", .{ .name = STORAGE_WRITE_TARGET_TMP_LOCAL });
...
pub fn emit_field_get_call(allocator: std.mem.Allocator, tokens: []const lexer.Token, start_idx: usize, end_idx: usize, locals: *const LocalSet, ctx: CodegenContext, move_ctx: ?*const CallLastUseMoveContext, out: *std.ArrayList(u8)) CodegenError!bool {
    ...
    if (find_struct_layout(ctx.struct_layouts, struct_local.ty)) |layout| {
        ...
        if (move_source) |source| {
            try append_fmt(allocator, out, "    ;; field-get-move {[source]s}.{[field]s}\n", .{ .source = source.source_name, .field = field_name });
...
pub fn append_union_payload_local_set(allocator: std.mem.Allocator, out: *std.ArrayList(u8), base: []const u8, idx: usize) !void {
    try append_fmt(allocator, out, "    local.set ${[name]s}.__union_payload_{[index]d}\n", .{ .name = base, .index = idx });
}
