(module
  ;; Private descriptor.metadata-hash lowering. The method call is
  ;; (descriptor handle, result-area pointer) -> readiness status and the
  ;; task-return callback is (result tag, lower u64, upper u64).
  ...
  (import "wasi:filesystem/types@0.3.0-rc-2025-09-16"
    "[async-lower][method]descriptor.metadata-hash" (func $metadata-hash (type $method)))
  (import "wasi:filesystem/types@0.3.0-rc-2025-09-16"
  ...
  ;; Frame: 0 waitable, 4 descriptor, 8 tag:u8, 16 lower:u64,
  ;; 24 upper:u64, 32 status, 36 callback subtask.
  ;; [metadata-hash-result-area] tag is frame+8 and the aligned record starts
  ;; at frame+16. Err stores its error-code payload in the first slot.
  ...
  (func $finish (param $frame i32) (result i32)
    ...
    (local $upper i64)
    (local $subtask i32)
    ;; [metadata-hash-ready] read both u64 words before dropping ownership.
    local.get $frame
    i32.const 8
    i32.add
    i32.load8_u
    ...
    if
      ...
      ;; [metadata-hash-error] no Ok upper word is fabricated for Err.
      local.get $frame
      i32.const 16
      i32.add
      i32.load8_u
