...
TEST_DIR="$ROOT_DIR/src/build/test"
STDLIB_DIR="$ROOT_DIR/lib"
...
run_ok_compiled_must_pass_case() {
    ...
    if ! DO_LIB_ROOT="$STDLIB_DIR" "$DO_BIN" test "$case_file" --compiled -o "$wat_file" >"$stdout_file" 2>"$stderr_file"; then
...
run_std_src_case() {
    ...
    if DO_LIB_ROOT="$STDLIB_DIR" "$DO_BIN" test "$case_file" >"$stdout_file" 2>"$stderr_file"; then
...
for case_file in "$STDLIB_DIR"/*.do; do
    [[ -e "$case_file" ]] || continue
