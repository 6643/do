...
fs.writeFileSync(
  ...
  [
    ...
    '  ;; wasi-bind source="lib/time.do" alias="host_now" target="clocks/system-clock/now" params="" result="Datetime"',
    '  ;; wasi-bind source="lib/time.do" alias="host_resolution" target="clocks/system-clock/get-resolution" params="" result="u64"',
    "",
  ].join("\n"),
);
...
assert.deepEqual(parsed.bindings, [
  ...
  },
  {
    ...
    params: [],
    result: "u64",
    identity: "lib/time.do/host_resolution",
    known: true,
    resolved: {
      package: "clocks",
      interface: "system-clock",
...
assert.deepEqual(
  ...
  ["entry/host_now", "lib/time.do/host_now"],
);
assert.deepEqual(
...
fs.writeFileSync(
  ...
  [
    '  ;; wasi-bind source="lib/file.do" alias="host_file_drop" target="filesystem/types/descriptor.drop" params="descriptor" result="nil"',
    "",
  ].join("\n"),
);
...
fs.writeFileSync(
  ...
  [
    '  ;; wasi-bind source="lib/random.do" alias="host_random_bytes" target="random/random/get-random-bytes" params="u64" result="list<u8>"',
    '  ;; wasi-bind source="lib/random.do" alias="host_random_u64" target="random/random/get-random-u64" params="" result="u64"',
    "",
  ].join("\n"),
);
...
assert.equal(
  ...
  [
    ...
    '  (import "cm32p2|wasi:random/random" "get-random-u64" (func $__wasi_import_random_random_get_random_u64 (result i64)))',
    "",
    "  (func $__wasi_shim_lib_random_do_host_random_bytes (param $p0 i64) (param $__result_area i32)",
    "    local.get $p0",
    "    local.get $__result_area",
    "    call $__wasi_import_random_random_get_random_bytes",
    "  )",
    "  (func $__wasi_shim_lib_random_do_host_random_u64 (result i64)",
...
fs.writeFileSync(
  ...
  [
    '  ;; wasi-bind source="lib/file.do" alias="host_file_read" target="filesystem/types/descriptor.read" params="descriptor,filesize,filesize" result="result<tuple<list<u8>,bool>,error-code>"',
    "",
  ].join("\n"),
);
...
assert.equal(
  ...
  [
    '  (import "cm32p2|wasi:filesystem/types" "[method]descriptor.read" (func $__wasi_import_filesystem_types_descriptor_read (param i32 i64 i64 i32)))',
    "",
    "  (func $__wasi_shim_lib_file_do_host_file_read (param $p0 i32) (param $p1 i64) (param $p2 i64) (param $__result_area i32)",
    "    local.get $p0",
    "    local.get $p1",
    "    local.get $p2",
    "    local.get $__result_area",
...
assert.match(
  ...
  /\$__wasi_shim_lib_time_do_host_now/,
);
assert.equal(
