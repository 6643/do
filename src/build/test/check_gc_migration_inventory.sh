...
host_wit_marshalling	complete	doc/wit/gc_descriptor_manifest.json,examples/gc-p3-runtime/wasi-random-gc-lift-package/worlds.wit,examples/gc-p3-runtime/marshal-record-assembly.wit,examples/gc-p3-runtime/marshal-record-host.wit,examples/gc-p3-runtime/marshal-record-lower-assembly.wit,examples/gc-p3-runtime/marshal-record-indirect-lower-assembly.wit	examples/gc-p3-runtime/test_gc_wasi_random_list_lift.sh,examples/gc-p3-runtime/test_gc_marshal_record_component.sh,examples/gc-p3-runtime/test_gc_marshal_record_host.sh,examples/gc-p3-runtime/test_gc_marshal_record_lower_component.sh,examples/gc-p3-runtime/test_gc_marshal_record_lower_host.sh,examples/gc-p3-runtime/test_gc_marshal_record_indirect_lower_host.sh	complete	examples/gc-p3-runtime/test_gc_wasi_random_list_lift_equivalence.sh,examples/gc-p3-runtime/test_gc_marshal_record_equivalence.sh,examples/gc-p3-runtime/test_gc_marshal_record_lower_equivalence.sh,examples/gc-p3-runtime/test_gc_marshal_record_indirect_lower_equivalence.sh	pending	-	current implementation record: the manifest-backed WASI random list<u8> lift now has a pinned Component gate and one GC/ARC canonical-boundary equivalence row; bounded parser-backed scalar-record lift, flat scalar-record lower, and the pinned 17-field indirect scalar-record lower also have measured canonical ABI, no-GC-reference boundary checks, pinned Component assembly, generated host execution, and GC/flat equivalence; nested/general aggregates, indirect layouts beyond these shapes, default host/WIT routing, and G5c cutover remain pending
