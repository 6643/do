...
pub const Error = error{UnsupportedP3WasiFilesystemSetSizeComponent};

const locator = "wasi:filesystem/types@0.3.0-rc-2025-09-16";
const member = "descriptor.set-size";
...
pub fn emit_component_wat(
    ...
    const plan = try FilesystemSetSizePlan.analyze(tokens, registry);
    if (!std.mem.eql(u8, plan.descriptor.canonical.async_import_module, locator) or
        !std.mem.eql(u8, plan.descriptor.canonical.async_import_name, "[async-lower][method]descriptor.set-size"))
        return error.UnsupportedP3WasiFilesystemSetSizeComponent;
    return generated_text.alloc_block(allocator, 0, @embedFile("wasi_filesystem_set_size_component_template.wat"));
}
...
pub fn emit_component_wit(
    ...
    var registry = try p3_async_manifest.Registry.load(allocator, @embedFile("p3_async_registry.json"));
    defer registry.deinit(allocator);
    _ = try FilesystemSetSizePlan.analyze(tokens, registry);
    return generated_text.alloc_block(allocator, 0, component_wit);
}

pub const FilesystemSetSizePlan = struct {
    ...
    pub fn analyze(tokens: []const lexer.Token, registry: p3_async_manifest.Registry) Error!FilesystemSetSizePlan {
        const descriptor = registry.find(locator, member) orelse return error.UnsupportedP3WasiFilesystemSetSizeComponent;
        switch (p3_async_manifest.lowering_shape(descriptor) orelse return error.UnsupportedP3WasiFilesystemSetSizeComponent) {
            .filesystem_set_size => {},
            else => return error.UnsupportedP3WasiFilesystemSetSizeComponent,
        }
        ...
        while (idx + 9 < tokens.len) : (idx += 1) {
            ...
            if (host != null or !signature_matches(tokens, idx + 9)) return error.UnsupportedP3WasiFilesystemSetSizeComponent;
            host = .{ .name = tokens[idx].lexeme };
        }
        const host_binding = host orelse return error.UnsupportedP3WasiFilesystemSetSizeComponent;
        if (host_count != 1 or !has_exact_resource_decl(tokens) or !has_exact_set_size_error_decl(tokens))
            return error.UnsupportedP3WasiFilesystemSetSizeComponent;

        const function = find_run_function(tokens, host_binding.name) orelse return error.UnsupportedP3WasiFilesystemSetSizeComponent;
...
fn signature_matches(tokens: []const lexer.Token, signature_start: usize) bool {
    ...
    if (params_close != signature_start + 4 or tokens[signature_start + 1].kind != .ident or
        !std.mem.eql(u8, tokens[signature_start + 1].lexeme, "File") or
        !tok_eq(tokens[signature_start + 2], ",") or tokens[signature_start + 3].kind != .ident or
        !std.mem.eql(u8, tokens[signature_start + 3].lexeme, "u64") or
    ...
    return host_close > params_close + 3 and
        compact_token_range_equals(tokens, params_close + 3, host_close, "nil|SetSizeError");
}

fn has_exact_resource_decl(tokens: []const lexer.Token) bool {
    return has_exact_token_sequence(tokens, "File", "File=@wasi_resource(\"filesystem/types/descriptor\",{.idi64})");
}

fn has_exact_set_size_error_decl(tokens: []const lexer.Token) bool {
    return has_exact_token_sequence(tokens, "SetSizeError", "SetSizeErrorerror=Io|NoEntry");
}
...
fn find_run_function(tokens: []const lexer.Token, host_name: []const u8) ?RunFunction {
    ...
    while (idx + 10 < tokens.len) : (idx += 1) {
        ...
        if (params_close != idx + 7 or tokens[idx + 2].kind != .ident or
            tokens[idx + 3].kind != .ident or !std.mem.eql(u8, tokens[idx + 3].lexeme, "File") or
            !tok_eq(tokens[idx + 4], ",") or tokens[idx + 5].kind != .ident or
            tokens[idx + 6].kind != .ident or !std.mem.eql(u8, tokens[idx + 6].lexeme, "u64") or
            !tok_eq(tokens[params_close + 1], "-") or !tok_eq(tokens[params_close + 2], ">")) continue;
        const body_open = params_close + 3;
        if (body_open + 3 >= tokens.len or !tok_eq(tokens[body_open], "nil") or
            !tok_eq(tokens[body_open + 1], "|") or !tok_eq(tokens[body_open + 2], "SetSizeError") or
            !tok_eq(tokens[body_open + 3], "{")) continue;
        const body_close = find_matching(tokens, body_open + 3, "{", "}") catch return null;
        const pending_name = tokens[body_open + 4].lexeme;
        if (!body_matches(tokens, body_open + 4, body_close, host_name, tokens[idx + 2].lexeme, tokens[idx + 5].lexeme, pending_name)) return null;
        return .{
            .root_name = tokens[idx].lexeme,
            .file_name = tokens[idx + 2].lexeme,
            .size_name = tokens[idx + 5].lexeme,
...
fn body_matches(
    ...
    if (end < start or end - start != 20 or tokens[start].kind != .ident or
        !std.mem.eql(u8, tokens[start].lexeme, pending_name)) return false;
    const fixed = [_][]const u8{ "Future", "<", "nil", "|", "SetSizeError", ">", "=" };
    var idx = start + 1;
    for (fixed) |expected| {
        if (!tok_eq(tokens[idx], expected)) return false;
        idx += 1;
    }
    if (tokens[idx].kind != .ident or !std.mem.eql(u8, tokens[idx].lexeme, host_name)) return false;
    idx += 1;
    if (!tok_eq(tokens[idx], "(") or tokens[idx + 1].kind != .ident or
        !std.mem.eql(u8, tokens[idx + 1].lexeme, file_name) or !tok_eq(tokens[idx + 2], ",") or
        tokens[idx + 3].kind != .ident or !std.mem.eql(u8, tokens[idx + 3].lexeme, size_name) or
        !tok_eq(tokens[idx + 4], ")")) return false;
    idx += 5;
    const result_fixed = [_][]const u8{ "return", "@", "await", "(", pending_name, ")" };
    for (result_fixed) |expected| {
        if (!tok_eq(tokens[idx], expected)) return false;
        idx += 1;
    }
    return idx == end;
...
const component_wit =
    ...
    \\  resource descriptor { set-size: async func(size: filesize) -> result<_, error-code>; }
    \\}
    \\
    \\interface probe { use types.{descriptor, error-code, filesize}; run: async func(file: own<descriptor>, size: filesize) -> result<_, error-code>; }
    \\
    \\world set-size-probe { import types; export probe; }
    \\
    ;

test "filesystem descriptor set-size source shape admits the bounded direct await" {
    ...
    const plan = try FilesystemSetSizePlan.analyze(tokens, registry);
    try std.testing.expectEqualStrings("set_size_descriptor", plan.host_name);
    try std.testing.expectEqualStrings("file", plan.file_name);
    try std.testing.expectEqualStrings("size", plan.size_name);
    try std.testing.expectEqualStrings("pending", plan.pending_name);
}

test "filesystem descriptor set-size source rejects a same-shape call to another host" {
    const source =
        ...
        \\    pending Future<nil | SetSizeError> = other_host(file, size)
    ...
    var registry = try p3_async_manifest.Registry.load(std.testing.allocator, @embedFile("p3_async_registry.json"));
    defer registry.deinit(std.testing.allocator);
    try std.testing.expectError(error.UnsupportedP3WasiFilesystemSetSizeComponent, FilesystemSetSizePlan.analyze(tokens, registry));
}
