...
pub const GcModuleEmitter = struct {
    ...
    pub fn emit_byte_list_set(self: *GcModuleEmitter, plan: ByteListSetPlan) !void {
        try self.ensure_open();
        try self.emit_bytes_prelude();
        try emit_byte_list_set_fragment(self.allocator, self.out, plan);
    }
...
fn emit_byte_list_set_fragment(
    allocator: std.mem.Allocator,
    out: *std.ArrayList(u8),
    plan: ByteListSetPlan,
) !void {
    ...
    try append_fmt(allocator, out, "    i32.const {s}\n", .{plan.index_expr});
    try append_fmt(allocator, out, "    i32.const {s}\n", .{plan.value_expr});
    try out.appendSlice(allocator, "    array.set $do_bytes\n    local.get $__gc_next)\n");
}
