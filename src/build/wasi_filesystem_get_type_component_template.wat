(module
  ...
  (import "wasi:filesystem/types@0.3.0-rc-2025-09-16" "[async-lower][method]descriptor.get-type"
    (func $get-type (type $method)))
  ...
  ;; get-type result buffer: tag at frame+8, payload at frame+9.
  ;; The method call is (descriptor handle, completion context) -> status.
  ...
  (func $wait-on-get-type (param $frame i32) (result i32)
  ...
  (func $finish (param $frame i32) (result i32)
    ...
    i32.const 0)
  (func $start-get-type (param $frame i32) (result i32)
    (local $code i32)
    local.get $frame i32.const 4 i32.add i32.load
    local.get $frame i32.const 8 i32.add call $get-type local.set $code
    ...
    else local.get $frame call $wait-on-get-type end)
  (func $run (type $async-run) (local $frame i32)
    ...
    local.get $frame call $start-get-type)
