(module
  ...
  (import "wasi:filesystem/types@0.3.0-rc-2025-09-16" "[async-lower][method]descriptor.sync-data"
  ...
  (func $start-sync-data (param $frame i32) (result i32)
    ...
    local.get $frame i32.const 8 i32.add call $sync-data local.set $code
    ...
    else local.get $frame call $wait-on-sync-data end)
  (func $run (type $async-run) (local $frame i32)
    ...
    local.get $frame call $start-sync-data)
