(module
  (type $method (func (param i32 i64 i32) (result i32)))
  ...
  (type $async-run (func (param i32 i64) (result i32)))
  (type $async-callback (func (param i32 i32 i32) (result i32)))
  (type $cabi-realloc (func (param i32 i32 i32 i32) (result i32)))
  ;; [set-size-call] descriptor:i32, size:i64, result-area:i32 -> status:i32.
  (import "wasi:filesystem/types@0.3.0-rc-2025-09-16" "[async-lower][method]descriptor.set-size"
    (func $set-size (type $method)))
  ...
  ;; [set-size-result-area] tag=u8 at frame+8, error-code payload=u8 at frame+9, size=u64 at frame+16.
  (global $result-buffer i32 (i32.const 1024))
  ...
  ;; [set-size-pending] retain the waitable and descriptor until callback completion.
  (func $wait-on-set-size (param $frame i32) (result i32)
    local.get $frame i32.const 24 i32.add i32.load i32.const 4 i32.shr_u
  ...
  (func $finish (param $frame i32) (result i32)
    ...
    local.get $frame i32.const 24 i32.add i32.load i32.const 2 i32.ne
    if
      local.get $frame i32.const 24 i32.add i32.load i32.const 4 i32.shr_u call $subtask-drop
    end
    ...
    i32.const 0)
  ;; [set-size-ready] status=2 means the Result is already complete.
  (func $start-set-size (param $frame i32) (result i32)
    (local $code i32)
    local.get $frame i32.const 4 i32.add i32.load
    local.get $frame i32.const 16 i32.add i64.load
    local.get $frame i32.const 8 i32.add
    call $set-size local.set $code
    local.get $frame i32.const 24 i32.add local.get $code i32.store
    local.get $code i32.const 2 i32.eq
    if (result i32) local.get $frame call $finish
    else local.get $frame call $wait-on-set-size end)
  (func $run (type $async-run) (local $frame i32)
    ...
    local.get $frame i32.const 16 i32.add local.get 1 i64.store
    local.get $frame i32.const 24 i32.add i32.const 0 i32.store
    local.get $frame call $start-set-size)
  ;; [set-size-error] completion event=1 with a non-2 payload stores error-code.
