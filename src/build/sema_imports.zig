...
pub fn check_p3_async_host_imports(allocator: std.mem.Allocator, tokens: []const lexer.Token) !void {
    ...
    for (tokens, 0..) |token, idx| {
        ...
        const is_stream_effect = if (shape) |resolved_shape| switch (resolved_shape) {
            .http_request_constructor, .http_stream_reader, .stream_reader_acquire, .stream_writer, .record_stream_reader, .record_resource_list_stream_reader, .owned_record_stream_producer, .record_resource_mixed_stream_producer, .record_resource_pair_stream_producer, .record_resource_nested_stream_producer, .record_resource_triple_stream_producer, .record_resource_pair_parameterized_stream_producer, .record_resource_list_owned_record_stream_producer, .record_resource_list_stream_producer, .record_resource_list_stream_dynamic_producer, .record_resource_list_stream_batched_producer, .scalar_list_stream_producer, .variant_resource_stream_reader => true,
            else => false,
        } else false;
...
fn descriptor_is_async_invocation(descriptor: p3_async_manifest.Descriptor) bool {
    ...
    return std.mem.eql(u8, descriptor.effect, "record-resource-list-stream-producer") or
        ...
        std.mem.eql(u8, descriptor.effect, "record-resource-mixed-stream-producer") or
        ...
        std.mem.eql(u8, descriptor.effect, "record-resource-list-owned-record-stream-producer") or
...
fn p3_async_signature_matches(tokens: []const lexer.Token, start_idx: usize, end_idx: usize, descriptor: p3_async_manifest.Descriptor) bool {
    ...
    if (shape) |resolved_shape| {
        switch (resolved_shape) {
            ...
            .record_resource_mixed_stream_producer => return mixed_owned_record_stream_producer_signature_matches(tokens, close_idx, end_idx, descriptor),
            ...
            .record_resource_list_owned_record_stream_producer => return list_owned_record_stream_producer_signature_matches(tokens, close_idx, end_idx, descriptor),
...
fn list_owned_record_stream_producer_signature_matches(
    tokens: []const lexer.Token,
    params_close_idx: usize,
    end_idx: usize,
    descriptor: p3_async_manifest.Descriptor,
...
fn owned_record_stream_producer_signature_matches(
    tokens: []const lexer.Token,
    params_close_idx: usize,
    end_idx: usize,
    descriptor: p3_async_manifest.Descriptor,
...
fn mixed_owned_record_stream_producer_signature_matches(
    tokens: []const lexer.Token,
    params_close_idx: usize,
    end_idx: usize,
    descriptor: p3_async_manifest.Descriptor,
...
fn owned_record_pair_stream_producer_signature_matches(
    ...
    if (!source_type_matches_element(tokens[params_close_idx - 2].lexeme, shape.element)) {
        // Let the Component planner classify the deliberate old-descriptor
        // ResourceTriple probe as UnsupportedP3AsyncComponent. The planner
        // still rejects the pair descriptor after checking the full source
        // topology; other element mismatches remain signature errors here.
...
fn owned_record_triple_stream_producer_signature_matches(
    tokens: []const lexer.Token,
    params_close_idx: usize,
    end_idx: usize,
    descriptor: p3_async_manifest.Descriptor,
...
fn parameterized_owned_record_pair_stream_producer_signature_matches(
    tokens: []const lexer.Token,
    params_close_idx: usize,
    end_idx: usize,
    descriptor: p3_async_manifest.Descriptor,
...
test "list-owned-record producer host_async_func imports accept the exact record writer signature" {
    const source =
...
test "owned record producer host_async_func imports accept the exact direct writer signature" {
    const source =
...
test "owned record producer host_async_func imports reject a list writer signature" {
    const source =
        \\consume = @host_async_func("do:g6-2-owned-record-producer@0.1.0", "consume-via-stream", (StreamWriter<[ResourceEntry]>) -> Result<nil, ProducerError>)
        \\ProducerError error = Io | Pipe | InvalidMode
    ;
...
test "parameterized owned record pair producer host_async_func imports accept the exact direct writer signature" {
    const source =
...
test "parameterized owned record pair producer host_async_func imports reject a drifted writer" {
    const source =
