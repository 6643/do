...
pub fn check_p3_async_host_imports(allocator: std.mem.Allocator, tokens: []const lexer.Token) !void {
    ...
    for (tokens, 0..) |token, idx| {
        ...
        const is_stream_effect = if (shape) |resolved_shape| switch (resolved_shape) {
            .http_request_constructor, .http_stream_reader, .stream_reader_acquire, .stream_writer, .record_stream_reader, .record_resource_list_stream_reader, .owned_record_stream_producer, .record_resource_mixed_stream_producer, .record_resource_pair_stream_producer, .record_resource_nested_stream_producer, .record_resource_triple_stream_producer, .record_resource_pair_parameterized_stream_producer, .record_resource_list_stream_producer, .record_resource_list_stream_dynamic_producer, .record_resource_list_stream_batched_producer, .scalar_list_stream_producer, .variant_resource_stream_reader => true,
            else => false,
        } else false;
