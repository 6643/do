...
pub fn parse_build(args: []const []const u8) !Args {
    ...
    while (i < args.len) : (i += 1) {
        ...
        if (std.mem.eql(u8, args[i], "--p3-async-map-component")) {
            p3_async_map_component = true;
            continue;
        }
        ...
        if (std.mem.eql(u8, args[i], "--p3-wasi-filesystem-stat-component")) {
            p3_wasi_filesystem_stat_component = true;
            continue;
        }
        ...
        if (std.mem.eql(u8, args[i], "--p3-owned-future-component")) {
            p3_owned_future_component = true;
            continue;
        }
        if (std.mem.eql(u8, args[i], "--p3-async-component-v2")) {
            p3_async_component_v2 = true;
            continue;
        }
        if (std.mem.eql(u8, args[i], "--p3-async-v2-scalar-i64")) {
            p3_async_v2_scalar_i64_component = true;
            continue;
        }
...
test "parse_build accepts the private async map component target" {
    const args = [_][]const u8{ "build", "app.do", "--p3-async-map-component", "--p3-wit-output", "app.wit", "-o", "app.wat" };
    const parsed = try parse_build(&args);
    try std.testing.expect(parsed.p3_async_map_component);
    try std.testing.expectEqualStrings("app.wit", parsed.p3_wit_output_path.?);
}

test "parse_build rejects async map component target combinations" {
    const another_target = [_][]const u8{ "build", "app.do", "--p3-async-map-component", "--p3-async-component" };
    try std.testing.expectError(error.UnexpectedCliArg, parse_build(&another_target));

    const component_core = [_][]const u8{ "build", "app.do", "--p3-async-map-component", "--component-core" };
    try std.testing.expectError(error.UnexpectedCliArg, parse_build(&component_core));

    const host_export = [_][]const u8{ "build", "app.do", "--p3-async-map-component", "--host-export" };
...
test "parse_build rejects the private filesystem stat target combinations" {
    const unified = [_][]const u8{ "build", "app.do", "--p3-wasi-filesystem-stat-component", "--p3-async-component" };
    try std.testing.expectError(error.UnexpectedCliArg, parse_build(&unified));

    const core = [_][]const u8{ "build", "app.do", "--p3-wasi-filesystem-stat-component", "--component-core" };
    try std.testing.expectError(error.UnexpectedCliArg, parse_build(&core));

    const host = [_][]const u8{ "build", "app.do", "--p3-wasi-filesystem-stat-component", "--host-export" };
    try std.testing.expectError(error.UnexpectedCliArg, parse_build(&host));
}
...
test "parse_build rejects owned future target combinations" {
    const args = [_][]const u8{ "build", "app.do", "--p3-owned-future-component", "--p3-async-component-v2" };
    try std.testing.expectError(error.UnexpectedCliArg, parse_build(&args));
}

test "parse_build rejects async call target combinations" {
    const v1 = [_][]const u8{ "build", "app.do", "--p3-async-call-component", "--p3-async-component" };
    try std.testing.expectError(error.UnexpectedCliArg, parse_build(&v1));

    const v2 = [_][]const u8{ "build", "app.do", "--p3-async-call-component", "--p3-async-component-v2" };
    try std.testing.expectError(error.UnexpectedCliArg, parse_build(&v2));

    const core = [_][]const u8{ "build", "app.do", "--p3-async-call-component", "--component-core" };
    try std.testing.expectError(error.UnexpectedCliArg, parse_build(&core));

    const host = [_][]const u8{ "build", "app.do", "--p3-async-call-component", "--host-export" };
    try std.testing.expectError(error.UnexpectedCliArg, parse_build(&host));
}
