...
pub const Error = error{UnsupportedP3WasiFilesystemGetTypeComponent};

const locator = "wasi:filesystem/types@0.3.0-rc-2025-09-16";
const member = "descriptor.get-type";
...
pub fn emit_component_wat(
    ...
    const plan = try GetTypePlan.analyze(tokens, registry);
...
pub const GetTypePlan = struct {
    ...
    pub fn analyze(tokens: []const lexer.Token, registry: p3_async_manifest.Registry) Error!GetTypePlan {
        const descriptor = registry.find(locator, member) orelse return error.UnsupportedP3WasiFilesystemGetTypeComponent;
        switch (p3_async_manifest.lowering_shape(descriptor) orelse return error.UnsupportedP3WasiFilesystemGetTypeComponent) {
            .filesystem_get_type => {},
            else => return error.UnsupportedP3WasiFilesystemGetTypeComponent,
        }
        ...
        while (idx + 9 < tokens.len) : (idx += 1) {
            ...
            if (host != null or !signature_matches(tokens, idx + 9)) return error.UnsupportedP3WasiFilesystemGetTypeComponent;
            host = .{ .name = tokens[idx].lexeme };
        }
        const host_binding = host orelse return error.UnsupportedP3WasiFilesystemGetTypeComponent;
        if (!has_exact_resource_decl(tokens) or !has_exact_descriptor_type_decl(tokens) or !has_exact_file_error_decl(tokens))
            return error.UnsupportedP3WasiFilesystemGetTypeComponent;

        const function = find_run_function(tokens) orelse return error.UnsupportedP3WasiFilesystemGetTypeComponent;
...
fn signature_matches(tokens: []const lexer.Token, signature_start: usize) bool {
    ...
    return host_close > params_close + 3 and compact_token_range_equals(tokens, params_close + 3, host_close, "DescriptorType|FileError");
}
...
fn has_exact_descriptor_type_decl(tokens: []const lexer.Token) bool {
    return has_exact_token_sequence(tokens, "DescriptorType", "DescriptorType=Unknown|Directory|RegularFile");
}

fn has_exact_file_error_decl(tokens: []const lexer.Token) bool {
    return has_exact_token_sequence(tokens, "FileError", "FileErrorerror=Io|NoEntry");
}
...
fn find_run_function(tokens: []const lexer.Token) ?RunFunction {
    ...
    while (idx + 8 < tokens.len) : (idx += 1) {
        ...
        if (body_open + 3 >= tokens.len or !tok_eq(tokens[body_open], "DescriptorType") or !tok_eq(tokens[body_open + 1], "|") or
            !tok_eq(tokens[body_open + 2], "FileError") or !tok_eq(tokens[body_open + 3], "{")) continue;
        const body_close = find_matching(tokens, body_open + 3, "{", "}") catch return null;
        if (!compact_token_range_equals(tokens, body_open + 4, body_close, "pendingFuture<DescriptorType|FileError>=get_type(directory)return@await(pending)")) return null;
        return .{ .root_name = tokens[idx].lexeme, .directory_name = tokens[idx + 2].lexeme, .pending_name = tokens[body_open + 4].lexeme };
    }
    return null;
}
...
test "get-type source shape admits the bounded direct await" {
    ...
    try std.testing.expect(has_exact_descriptor_type_decl(tokens));
    try std.testing.expect(has_exact_file_error_decl(tokens));
