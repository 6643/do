//! Private manifest-backed mixed scalar-record lift probe.
//!
...
const descriptor_id = "demo:marshal-record-mixed-host/api.read@1.0.0/lift";

fn mixed_record_measurement() marshal.MeasuredNode {
    return .{
        .layout = .{ .record = .{
            .byte_size = 24,
            .alignment = 8,
...
pub fn emit_mixed_record_lift_module(
    io: std.Io,
...
test "manifest-backed mixed scalar record lift probe emits a host entry" {
    const wat = try emit_mixed_record_lift_module(std.testing.io, std.testing.allocator, "..");
...
pub fn main(init: std.process.Init) !void {
    ...
    if (args.len != 2 and args.len != 3) return error.InvalidGcMarshalRecordMixedLiftProbeArgs;
    const repository_root = if (args.len == 3) args[2] else ".";
    const wat = try emit_mixed_record_lift_module(init.io, init.gpa, repository_root);
