//! Private manifest-backed three-level nested scalar-record lower probe.
//!
...
const descriptor_id = "demo:marshal-record-nested-lower-deep/api.write@1.0.0/lower";
...
pub fn emit_nested_record_lower_deep_module(
    io: std.Io,
...
test "manifest-backed three-level nested scalar record lower probe emits a host entry" {
    const wat = try emit_nested_record_lower_deep_module(std.testing.io, std.testing.allocator, "..");
...
pub fn main(init: std.process.Init) !void {
    ...
    if (args.len != 2 and args.len != 3) return error.InvalidGcMarshalRecordNestedLowerDeepProbeArgs;
    const repository_root = if (args.len == 3) args[2] else ".";
    const wat = try emit_nested_record_lower_deep_module(init.io, init.gpa, repository_root);
