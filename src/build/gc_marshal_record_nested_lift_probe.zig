//! Private manifest-backed nested scalar-record lift probe.
//!
...
//! only by the bounded Component host/equivalence gates; the default
//! host/WIT route stays on its existing path.
...
const descriptor_id = "demo:marshal-record-nested-host/api.read@1.0.0/lift";
...
pub fn emit_nested_record_lift_module(
    io: std.Io,
...
test "manifest-backed nested scalar record lift probe emits a host entry" {
    const wat = try emit_nested_record_lift_module(std.testing.io, std.testing.allocator, "..");
...
pub fn main(init: std.process.Init) !void {
    ...
    if (args.len != 2 and args.len != 3) return error.InvalidGcMarshalRecordNestedLiftProbeArgs;
    const repository_root = if (args.len == 3) args[2] else ".";
    const wat = try emit_nested_record_lift_module(init.io, init.gpa, repository_root);
