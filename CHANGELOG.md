...
# 2026-09-11 G6.2 private two-list owned-record producer:
  ...
  in a capacity-one `stream<two-list-entry>`. The measured record is 20 bytes
  with alignment 4: `first.ptr/len` offsets are `0/4`, `second.ptr/len` offsets
  are `8/12`, `ticket` is at `16`, list stride/capacity is `4/3`, and the
  pinned WIT SHA-256 is
  `14ba67e070346a127e75c7dfd73c71d6082ad7d9385c7d803834319dabc6404a`.
  The immutable `ProducerContract` keeps `first` and `second` as independent
  list-allocation cleanup paths and `ticket` as the only ownership leaf;
  transfer commits only after the complete record write.
  Do/Component, 13 WAT-before-rejection negatives, canonical ABI, generated
  Rust/Wasmtime lifecycle, and canonical/generated byte parity pass all ten
  ready/pending/error/cancel/early-drop/repeat/invalid modes. Static WAT checks
  lock two allocation call sites, the shared release-counter path, and four
  terminal cleanup call sites. The runner directly
  observes transferred-list reads, ticket drops, and an empty resource table;
  its list allocation/release totals are expected cardinalities derived from
  valid invocations, not direct reads of hidden Core globals. Repeat derives
  `4/4` list accounting and observes `2/2` ticket cleanup, invalid creates
  none, every mode leaves `table-empty=true`, and generated WAT contains no
  `__arc_` or Wasm GC reference boundary. Fresh verification is
  `14/14 steps; 53/53 tests`, `zig test main.zig 1583/1583`, and a passing
  ReleaseSmall build. This remains private fixed-shape evidence: it adds no GC
  inventory row and does not open public ownership syntax, generic/arbitrary
  producer lowering, borrowed/variant payloads, or general async/resource
  lowering.
